...
OPENSSL_OLD_VERSIONS = ['1.0.2u', '1.1.0l']
OPENSSL_RECENT_VERSIONS = ['1.1.1g']
LIBRESSL_OLD_VERSIONS = ['2.9.2']
LIBRESSL_RECENT_VERSIONS = ['3.1.0']
HERE = os.path.dirname(os.path.abspath(__file__))
PYTHONROOT = os.path.abspath(os.path.join(HERE, '..', '..'))
MULTISSL_DIR = os.path.abspath(os.path.join(PYTHONROOT, '..', 'multissl'))
parser = argparse.ArgumentParser(prog='multissl', description='Run CPython tests with multiple OpenSSL and LibreSSL versions.')
parser.add_argument('--debug', action='store_true', help='Enable debug logging')
parser.add_argument('--disable-ancient', action='store_true', help="Don't test OpenSSL and LibreSSL versions without upstream support")
parser.add_argument('--openssl', nargs='+', default=(), help="OpenSSL versions, defaults to '{}' (ancient: '{}') if no OpenSSL and LibreSSL versions are given.".format(OPENSSL_RECENT_VERSIONS, OPENSSL_OLD_VERSIONS))
parser.add_argument('--libressl', nargs='+', default=(), help="LibreSSL versions, defaults to '{}' (ancient: '{}') if no OpenSSL and LibreSSL versions are given.".format(LIBRESSL_RECENT_VERSIONS, LIBRESSL_OLD_VERSIONS))
parser.add_argument('--tests', nargs='*', default=(), help='Python tests to run, defaults to all SSL related tests.')
parser.add_argument('--base-directory', default=MULTISSL_DIR, help='Base directory for OpenSSL / LibreSSL sources and builds.')
parser.add_argument('--no-network', action='store_false', dest='network', help='Disable network tests.')
parser.add_argument('--steps', choices=['library', 'modules', 'tests'], default='tests', help="Which steps to perform. 'library' downloads and compiles OpenSSL or LibreSSL. 'module' also compiles Python modules. 'tests' builds all and runs the test suite.")
parser.add_argument('--system', default='', help='Override the automatic system type detection.')
parser.add_argument('--force', action='store_true', dest='force', help='Force build and installation.')
parser.add_argument('--keep-sources', action='store_true', dest='keep_sources', help='Keep original sources for debugging.')
...
class AbstractBuilder(object):
    ...
    def __init__(self, version, args):
        ...
        self.args = args
        self.install_dir = os.path.join(os.path.join(args.base_directory, self.library.lower()), version)
...
class BuildOpenSSL(AbstractBuilder):
    library = 'OpenSSL'
    url_templates = ('https://www.openssl.org/source/openssl-{v}.tar.gz', 'https://www.openssl.org/source/old/{s}/openssl-{v}.tar.gz')
    ...
    def _post_install_300(self):
        ...
        self._subprocess_call([self.openssl_cli, 'fipsinstall', '-out', fipsinstall_cnf, '-module', fips_mod, '-provider_name', 'fips', '-mac_name', 'HMAC', '-macopt', 'digest:SHA256', '-macopt', 'hexkey:00', '-section_name', 'fips_sect'])
