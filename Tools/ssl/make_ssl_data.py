...
def parse_error_codes(h_file, prefix, libcode):
    pat = re.compile(('#\\s*define\\W+(%s([\\w]+))\\W+(\\d+)\\b' % re.escape(prefix)))
    codes = []
...
if (__name__ == '__main__'):
    ...
    for error_header in glob.glob(os.path.join(glob.escape(openssl_inc), 'include/openssl/*err.h')):
        ...
        if (mnemonic == ''):
            lib_codes = {code: num for (code, (_, _, num)) in parse_error_codes(error_header, 'ERR_LIB_', None)}
        else:
