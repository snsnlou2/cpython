...
class SheetParser():
    ...
    def end_value(self, text):
        if isinstance(self.value, BaseCell):
            ...
            self.cell = NumericCell(self.value, (self.fmt or '%s'), (self.alignment or RIGHT))
...
class NumericCell(BaseCell):
    ...
    def xml(self):
        ...
        return ('<value align="%s" format="%s">%s</value>' % (align2xml[self.alignment], self.fmt, method()))
...
class FormulaCell(BaseCell):
    ...
    def xml(self):
        return ('<formula align="%s" format="%s">%s</formula>' % (align2xml[self.alignment], self.fmt, escape(self.formula)))
