...
try:
    ...
    print('Please run `make venv` to create a virtual environment and install all the dependencies, before running this script.')
    sys.exit(1)
sys.path.insert(0, os.getcwd())
from scripts.test_parse_directory import parse_directory
argparser = argparse.ArgumentParser(prog='benchmark', description='Reproduce the various pegen benchmarks')
argparser.add_argument('--target', action='store', choices=['xxl', 'stdlib'], default='xxl', help='Which target to use for the benchmark (default is xxl.py)')
subcommands = argparser.add_subparsers(title='Benchmarks', dest='subcommand')
command_compile = subcommands.add_parser('compile', help='Benchmark parsing and compiling to bytecode')
...
def run_benchmark_stdlib(subcommand):
    ...
    for _ in range(3):
        parse_directory('../../Lib', verbose=False, excluded_files=['*/bad*', '*/lib2to3/tests/data/*'], short=True, mode=modes[subcommand])
