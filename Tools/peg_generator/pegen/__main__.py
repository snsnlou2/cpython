...
def generate_c_code(args):
    from pegen.build import build_c_parser_and_generator
    verbose = args.verbose
    verbose_tokenizer = (verbose >= 3)
    verbose_parser = ((verbose == 2) or (verbose >= 4))
    try:
        (grammar, parser, tokenizer, gen) = build_c_parser_and_generator(args.grammar_filename, args.tokens_filename, args.output, args.compile_extension, verbose_tokenizer, verbose_parser, args.verbose, keep_asserts_in_extension=(False if args.optimized else True), skip_actions=args.skip_actions)
...
def generate_python_code(args):
    ...
    verbose_parser = ((verbose == 2) or (verbose >= 4))
    try:
        (grammar, parser, tokenizer, gen) = build_python_parser_and_generator(args.grammar_filename, args.output, verbose_tokenizer, verbose_parser, skip_actions=args.skip_actions)
        return (grammar, parser, tokenizer, gen)
    except Exception as err:
        if args.verbose:
            raise
...
c_parser.add_argument('-o', '--output', metavar='OUT', default='parse.c', help='Where to write the generated parser')
c_parser.add_argument('--compile-extension', action='store_true', help='Compile generated C code into an extension module')
c_parser.add_argument('--optimized', action='store_true', help='Compile the extension in optimized mode')
c_parser.add_argument('--skip-actions', action='store_true', help='Suppress code emission for rule actions')
...
python_parser.add_argument('-o', '--output', metavar='OUT', default='parse.py', help='Where to write the generated parser')
python_parser.add_argument('--skip-actions', action='store_true', help='Suppress code emission for rule actions')
...
def main():
    ...
    if args.verbose:
        ...
        for scc in gen.first_sccs:
            print(' ', scc, end='')
            if (len(scc) > 1):
                print('  # Indirectly left-recursive; leaders:', {name for name in scc if grammar.rules[name].leader})
            else:
                name = next(iter(scc))
