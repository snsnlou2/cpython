...
from pegen.grammar import Alt, Cut, Gather, GrammarVisitor, Group, Lookahead, NamedItem, NameLeaf, NegativeLookahead, Opt, PositiveLookahead, Repeat0, Repeat1, Rhs, Rule, StringLeaf
...
class NodeTypes(Enum):
    ...
    SOFT_KEYWORD = 5
    CUT_OPERATOR = 6
BASE_NODETYPES = {'NAME': NodeTypes.NAME_TOKEN, 'NUMBER': NodeTypes.NUMBER_TOKEN, 'STRING': NodeTypes.STRING_TOKEN}
...
class CCallMakerVisitor(GrammarVisitor):
    ...
    def __init__(self, parser_generator, exact_tokens, non_exact_tokens):
    ...
    def soft_keyword_helper(self, value):
...
class CParserGenerator(ParserGenerator, GrammarVisitor):
    ...
    def __init__(self, grammar, tokens, exact_tokens, non_exact_tokens, file, debug=False, skip_actions=False):
    ...
    def out_of_memory_return(self, expr, cleanup_code=None):
    ...
    def out_of_memory_goto(self, expr, goto_target):
        self.print(f'if ({expr}) {{')
        with self.indent():
            self.print('PyErr_NoMemory();')
            self.print(f'goto {goto_target};')
    ...
    def _group_keywords_by_length(self):
    ...
    def _set_up_token_start_metadata_extraction(self):
    ...
    def _set_up_token_end_metadata_extraction(self):
    ...
    def _set_up_rule_memoization(self, node, result_type):
        self.print('{')
        with self.indent():
            self.add_level()
    ...
    def visit_Rhs(self, node, is_loop, is_gather, rulename):
        if is_loop:
            assert (len(node.alts) == 1)
        for alt in node.alts:
            self.visit(alt, is_loop=is_loop, is_gather=is_gather, rulename=rulename)

    def join_conditions(self, keyword, node):
    ...
    def emit_action(self, node, cleanup_code=None):
    ...
    def emit_default_action(self, is_gather, node):
    ...
    def emit_dummy_action(self):
        self.print('_res = _PyPegen_dummy_name(p);')

    def handle_alt_normal(self, node, is_gather, rulename):
    ...
    def handle_alt_loop(self, node, is_gather, rulename):
    ...
    def visit_Alt(self, node, is_loop, is_gather, rulename):
        self.print(f'{{ // {node}')
        with self.indent():
            self._check_for_errors()
            node_str = str(node).replace('"', '\\"')
