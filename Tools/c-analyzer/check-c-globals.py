...
CAPI_REGEX = re.compile('^ *PyAPI_DATA\\([^)]*\\) \\W*(_?Py\\w+(?:, \\w+)*\\w).*;.*$')
IGNORED_VARS = {'_DYNAMIC', '_GLOBAL_OFFSET_TABLE_', '__JCR_LIST__', '__JCR_END__', '__TMC_END__', '__bss_start', '__data_start', '__dso_handle', '_edata', '_end'}

def find_capi_vars(root):
    capi_vars = {}
    for dirname in SOURCE_DIRS:
        for filename in glob.glob(os.path.join(glob.escape(os.path.join(ROOT_DIR, dirname)), '**/*.[hc]'), recursive=True):
...
def _read_global_names(filename):
    with open(filename) as file:
        return {line.partition('#')[0].strip() for line in file if (line.strip() and (not line.startswith('#')))}
...
def _is_autogen_var(name):
    return (name.startswith('PyId_') or ('.' in name) or name.startswith('op_id.') or name.startswith('rop_id.') or name.startswith('arcs_') or name.startswith('states_'))
...
def _is_type_var(name):
    ...
    if name.endswith('_desc'):
        return True
    return (name.startswith('doc_') or name.endswith(('_doc', '__doc__', '_docstring')) or name.endswith('_methods') or name.endswith('_fields') or name.endswith(('_memberlist', '_members')) or name.endswith('_slots') or name.endswith(('_getset', '_getsets', '_getsetlist')) or name.endswith('_as_mapping') or name.endswith('_as_number') or name.endswith('_as_sequence') or name.endswith('_as_buffer') or name.endswith('_as_async'))
...
def _is_compiler(name):
    return (name.endswith('_type') or name.endswith('_singleton') or name.endswith('_attributes'))
...
def find_vars(root, globals_filename=GLOBALS_FILE):
    ...
    if (nm is None):
        ...
        (yield from (var for var in _find_var_symbols(python, nm, capi_vars, globalnames) if (var.name not in IGNORED_VARS)))
NM_FUNCS = set('Tt')
...
def make_sort_key(spec):
    columns = [(col.strip('_'), ('_' if col.startswith('_') else '')) for col in spec]
...
COLUMN_WIDTHS.update({'name': 50, 'scope': 7, 'filename': 40})
...
def parse_args(argv=None):
    ...
    parser.add_argument('--filters', default='-scope', help='[[-]<COLUMN>[=<GLOB>]] ...')
    parser.add_argument('--columns', default=COLUMNS, help='a comma-separated list of columns to show')
    parser.add_argument('--sort', default='filename,_name', help='a comma-separated list of columns to sort')
    parser.add_argument('--group', help='group by the given column name (- to not group)')
...
def main(root=ROOT_DIR, filename=GLOBALS_FILE, filters=None, columns=COLUMN_NAMES, sort=None, group=None, verbosity=VERBOSITY, rc=1):
    log = (lambda msg: ...)
    if (verbosity >= 2):
