...
def get_srclines(filename, *, cache=None, _open=open, _iter_lines=_iter_clean_lines):
    ...
    with _open(filename) as srcfile:
        srclines = [line for (_, line) in _iter_lines(srcfile) if (not line.startswith('#'))]
...
def iter_variables(filename, *, srccache=None, parse_variable=None, _get_srclines=get_srclines, _default_parse_variable=parse_variable):
...
def find_variable(filename, funcname, name, *, ignored=None, srccache=None, parse_variable=None, _iter_variables=iter_variables):
    'Return the matching variable.\n\n    Return None if the variable is not found.\n    '
    for (varid, decl) in _iter_variables(filename, srccache=srccache, parse_variable=parse_variable):
        if _match_varid(varid, name, funcname, ignored):
            return (varid, decl)
    else:
        return None

def find_variables(varids, filenames=None, *, srccache=_NOT_SET, parse_variable=None, _find_symbol=find_variable):
    ...
    for varid in varids:
        ...
        for filename in srcfiles:
            (varid, decl) = _find_varid(filename, varid.funcname, varid.name, ignored=used, srccache=srccache, parse_variable=parse_variable)
            if varid:
                (yield (varid, decl))
                used.add(varid)
                break
