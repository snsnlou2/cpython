...
def _iter_vars(filenames, preprocessed, *, handle_id=None, _iter_decls=declarations.iter_all):
    if (handle_id is None):
        handle_id = ID
    for filename in (filenames or ()):
        for (kind, funcname, name, decl) in _iter_decls(filename, preprocessed=preprocessed):
            if (kind != 'variable'):
                continue
            varid = handle_id(filename, funcname, name)
            (yield (varid, decl))

def variables(*filenames, perfilecache=None, preprocessed=False, known=None, handle_id=None, _iter_vars=_iter_vars):
...
def variable(name, filenames, *, local=False, perfilecache=None, preprocessed=False, handle_id=None, _iter_vars=variables):
    ...
    for (varid, decl) in _iter_vars(filenames, perfilecache=perfilecache, preprocessed=preprocessed):
        if (varid.name != name):
            continue
        if local:
            if varid.funcname:
...
def variable_from_id(id, filenames, *, perfilecache=None, preprocessed=False, handle_id=None, _get_var=variable):
    'Return (varid, decl) for the first found variable that matches.'
    local = False
    ...
    return _get_var(name, filenames, local=local, perfilecache=perfilecache, preprocessed=preprocessed, handle_id=handle_id)
