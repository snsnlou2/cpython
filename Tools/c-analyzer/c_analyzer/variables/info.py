...
class Variable(_NTBase, namedtuple('Variable', 'id storage vartype')):
    'Information about a single variable declaration.'
    __slots__ = ()
    STORAGE = ('static', 'extern', 'implicit', 'local')
    ...
    def __new__(cls, id, storage, vartype):
        self = super().__new__(cls, id=ID.from_raw(id), storage=(str(storage) if storage else None), vartype=(normalize_vartype(vartype) if vartype else None))
        return self
