...
NM_KINDS = {'b': Symbol.KIND.VARIABLE, 'd': Symbol.KIND.VARIABLE, 't': Symbol.KIND.FUNCTION}
SPECIAL_SYMBOLS = {'__bss_start', '__data_start', '__dso_handle', '_DYNAMIC', '_edata', '_end', '__environ@@GLIBC_2.2.5', '_GLOBAL_OFFSET_TABLE_', '__JCR_END__', '__JCR_LIST__', '__TMC_END__'}
...
def iter_symbols(binfile, *, nm=None, handle_id=None, _which=shutil.which, _run=util.run_cmd):
    ...
    for line in output.splitlines():
        (name, kind, external, filename, funcname) = _parse_nm_line(line)
        if (kind != Symbol.KIND.VARIABLE):
            continue
        elif _is_special_symbol(name):
            continue
        (yield Symbol(id=handle_id(filename, funcname, name), kind=kind, external=external))
