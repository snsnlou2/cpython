...
''')
        for i in range(256):
            if ((i in self.decode_map) and (self.prefix in self.decode_map[i])):
                ...
                continue
            self.filler.write('{', ('__%s_decmap' % prefix), '+', ('%d' % m[i]['midx']), ',', ('%d,' % m[i]['min']), ('%d' % m[i]['max']), '},')
        self.filler.printout(self.fp)
        self.fp.write('};\n\n')
...
''')
        for i in range(256):
            if ((i in self.encode_map) and (self.prefix in self.encode_map[i])):
                self.filler.write('{', ('__%s_encmap' % self.prefix), '+', ('%d' % self.encode_map[i]['midx']), ',', ('%d,' % self.encode_map[i]['min']), ('%d' % self.encode_map[i]['max']), '},')
            else:
                self.filler.write('{', '0,', '0,', '0', '},')
                continue
        self.filler.printout(self.fp)
