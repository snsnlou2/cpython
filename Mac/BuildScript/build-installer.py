...
universal_opts_map = {'32-bit': ('i386', 'ppc'), '64-bit': ('x86_64', 'ppc64'), 'intel': ('i386', 'x86_64'), 'intel-32': ('i386',), 'intel-64': ('x86_64',), '3-way': ('ppc', 'i386', 'x86_64'), 'all': ('i386', 'ppc', 'x86_64', 'ppc64')}
default_target_map = {'64-bit': '10.5', '3-way': '10.5', 'intel': '10.5', 'intel-32': '10.4', 'intel-64': '10.5', 'all': '10.5'}
...
def getTargetCompilers():
    target_cc_map = {'10.4': ('gcc-4.0', 'g++-4.0'), '10.5': ('gcc', 'g++'), '10.6': ('gcc', 'g++')}
...
def library_recipes():
    ...
    result.extend([dict(name='NCurses 5.9', url='http://ftp.gnu.org/pub/gnu/ncurses/ncurses-5.9.tar.gz', checksum='8cb9c412e5f2d96bc6f459aa8c6282a1', configure_pre=['--enable-widec', '--without-cxx', '--without-cxx-binding', '--without-ada', '--without-curses-h', '--enable-shared', '--with-shared', '--without-debug', '--without-normal', '--without-tests', '--without-manpages', '--datadir=/usr/share', '--sysconfdir=/etc', '--sharedstatedir=/usr/com', '--with-terminfo-dirs=/usr/share/terminfo', '--with-default-terminfo-dir=/usr/share/terminfo', ('--libdir=/Library/Frameworks/Python.framework/Versions/%s/lib' % (getVersion(),))], patchscripts=[('ftp://ftp.invisible-island.net/ncurses//5.9/ncurses-5.9-20120616-patch.sh.bz2', 'f54bf02a349f96a7c4f0d00922f3a0d4')], useLDFlags=False, install=('make && make install DESTDIR=%s && cd %s/usr/local/lib && ln -fs ../../../Library/Frameworks/Python.framework/Versions/%s/lib/lib* .' % (shellQuote(os.path.join(WORKDIR, 'libraries')), shellQuote(os.path.join(WORKDIR, 'libraries')), getVersion()))), dict(name='SQLite 3.32.3', url='https://sqlite.org/2020/sqlite-autoconf-3320300.tar.gz', checksum='2e3911a3c15e85c2f2d040154bbe5ce3', extra_cflags=('-Os -DSQLITE_ENABLE_FTS5 -DSQLITE_ENABLE_FTS4 -DSQLITE_ENABLE_FTS3_PARENTHESIS -DSQLITE_ENABLE_JSON1 -DSQLITE_ENABLE_RTREE -DSQLITE_TCL=0 %s' % ('', '-DSQLITE_WITHOUT_ZONEMALLOC ')[LT_10_5]), configure_pre=['--enable-threadsafe', '--enable-shared=no', '--enable-static=yes', '--disable-readline', '--disable-dependency-tracking'])])
    if (getDeptargetTuple() < (10, 5)):
        result.extend([dict(name='Bzip2 1.0.6', url='http://bzip.org/1.0.6/bzip2-1.0.6.tar.gz', checksum='00b516f4704d4a7cb50a1d97e6e8e15b', configure=None, install=('make install CC=%s CXX=%s, PREFIX=%s/usr/local/ CFLAGS="-arch %s"' % (CC, CXX, shellQuote(os.path.join(WORKDIR, 'libraries')), ' -arch '.join(ARCHLIST)))), dict(name='ZLib 1.2.3', url='http://www.gzip.org/zlib/zlib-1.2.3.tar.gz', checksum='debc62758716a169df9f62e6ab2bc634', configure=None, install=('make install CC=%s CXX=%s, prefix=%s/usr/local/ CFLAGS="-arch %s"' % (CC, CXX, shellQuote(os.path.join(WORKDIR, 'libraries')), ' -arch '.join(ARCHLIST)))), dict(name='GNU Readline 6.1.2', url='http://ftp.gnu.org/pub/gnu/readline/readline-6.1.tar.gz', checksum='fc2f7e714fe792db1ce6ddc4c9fb4ef3', patchlevel='0', patches=[('http://ftp.gnu.org/pub/gnu/readline/readline-6.1-patches/readline61-001', 'c642f2e84d820884b0bf9fd176bc6c3f'), ('http://ftp.gnu.org/pub/gnu/readline/readline-6.1-patches/readline61-002', '1a76781a1ea734e831588285db7ec9b1')])])
    if (not PYTHON_3):
...
def build_universal_openssl(basedir, archList):
    ...
    def build_openssl_arch(archbase, arch):
        ...
        arch_opts = {'i386': ['darwin-i386-cc'], 'x86_64': ['darwin64-x86_64-cc', 'enable-ec_nistp_64_gcc_128'], 'ppc': ['darwin-ppc-cc'], 'ppc64': ['darwin64-ppc-cc']}
