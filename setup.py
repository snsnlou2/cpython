...
def validate_tzpath():
    ...
    if bad_paths:
        raise ValueError((('TZPATH must contain only absolute paths, ' + f'''found:
{tzpaths!r}
with invalid paths:
''') + f'{bad_paths!r}'))
...
class PyBuildExt(build_ext):
    ...
    def update_sources_depends(self):
        ...
        self.distribution.scripts = [os.path.join(self.srcdir, filename) for filename in self.distribution.scripts]
    ...
    def summary(self, mods_built, mods_disabled):
        ...
        if any((('_ssl' in l) for l in (self.missing, self.failed, self.failed_on_import))):
            ...
            print('Python requires an OpenSSL 1.0.2 or 1.1 compatible libssl with X509_VERIFY_PARAM_set1_host().')
            print('LibreSSL 2.6.4 and earlier do not provide the necessary APIs, https://github.com/libressl-portable/portable/issues/381')
            print()
    ...
    def check_extension_import(self, ext):
        if (ext.name in self.failed):
            self.announce(('WARNING: skipping import check for failed build "%s"' % ext.name), level=1)
            return
        ...
        if CYGWIN:
            ...
            return
        ext_filename = os.path.join(self.build_lib, self.get_ext_filename(self.get_ext_fullname(ext.name)))
        ...
        try:
            ...
            self.announce(('*** WARNING: renaming "%s" since importing it failed: %s' % (ext.name, why)), level=3)
            ...
            self.announce(('*** WARNING: importing extension "%s" failed with %s: %s' % (ext.name, exc_type, why)), level=3)
            self.failed.append(ext.name)
    ...
    def add_multiarch_paths(self):
        ...
        if (not os.path.exists(self.build_temp)):
            os.makedirs(self.build_temp)
        ret = run_command(('dpkg-architecture %s -qDEB_HOST_MULTIARCH > %s 2> /dev/null' % (opt, tmpfile)))
        try:
            if (ret == 0):
                with open(tmpfile) as fp:
                    multiarch_path_component = fp.readline().strip()
    ...
    def add_ldflags_cppflags(self):
        for (env_var, arg_name, dir_list) in (('LDFLAGS', '-R', self.compiler.runtime_library_dirs), ('LDFLAGS', '-L', self.compiler.library_dirs), ('CPPFLAGS', '-I', self.compiler.include_dirs)):
    ...
    def init_inc_lib_dirs(self):
        ...
        if (not CROSS_COMPILING):
            ...
            self.lib_dirs = (self.compiler.library_dirs + sysroot_paths(('LDFLAGS', 'CC'), system_lib_dirs))
    ...
    def detect_simple_extensions(self):
        ...
        self.add(Extension('math', ['mathmodule.c'], extra_compile_args=['-DPy_BUILD_CORE_MODULE'], extra_objects=[shared_math], depends=['_math.h', shared_math], libraries=['m']))
        self.add(Extension('cmath', ['cmathmodule.c'], extra_compile_args=['-DPy_BUILD_CORE_MODULE'], extra_objects=[shared_math], depends=['_math.h', shared_math], libraries=['m']))
        time_libs = []
        lib = sysconfig.get_config_var('TIMEMODULE_LIB')
        if lib:
            time_libs.append(lib)
        self.add(Extension('time', ['timemodule.c'], libraries=time_libs))
        self.add(Extension('_datetime', ['_datetimemodule.c'], libraries=['m'], extra_compile_args=['-DPy_BUILD_CORE_MODULE']))
        (self.add(Extension('_zoneinfo', ['_zoneinfo.c'])),)
        self.add(Extension('_random', ['_randommodule.c'], extra_compile_args=['-DPy_BUILD_CORE_MODULE']))
        self.add(Extension('_bisect', ['_bisectmodule.c']))
        self.add(Extension('_heapq', ['_heapqmodule.c'], extra_compile_args=['-DPy_BUILD_CORE_MODULE']))
        self.add(Extension('_pickle', ['_pickle.c'], extra_compile_args=['-DPy_BUILD_CORE_MODULE']))
        ...
        self.add(Extension('unicodedata', ['unicodedata.c'], depends=['unicodedata_db.h', 'unicodename_db.h']))
        self.add(Extension('_opcode', ['_opcode.c']))
        self.add(Extension('_asyncio', ['_asynciomodule.c'], extra_compile_args=['-DPy_BUILD_CORE_MODULE']))
    ...
    def detect_test_extensions(self):
        ...
        self.add(Extension('_testinternalcapi', ['_testinternalcapi.c'], extra_compile_args=['-DPy_BUILD_CORE_MODULE']))
        self.add(Extension('_testbuffer', ['_testbuffer.c']))
        self.add(Extension('_testimportmultiple', ['_testimportmultiple.c']))
        self.add(Extension('_testmultiphase', ['_testmultiphase.c']))
        self.add(Extension('_xxtestfuzz', ['_xxtestfuzz/_xxtestfuzz.c', '_xxtestfuzz/fuzzer.c']))
    ...
    def detect_readline_curses(self):
        ...
        if do_readline:
            if CROSS_COMPILING:
                ret = run_command(("%s -d %s | grep '(NEEDED)' > %s" % (sysconfig.get_config_var('READELF'), do_readline, tmpfile)))
            ...
            if (ret == 0):
                with open(tmpfile) as fp:
                    for ln in fp:
                        if ('curses' in ln):
                            readline_termcap_library = re.sub('.*lib(n?cursesw?)\\.so.*', '\\1', ln).rstrip()
                            break
                        if ('tinfo' in ln):
        ...
        if MACOS:
            ...
            if (dep_target and (tuple((int(n) for n in dep_target.split('.')[0:2])) < (10, 5))):
                os_release = 8
            if (os_release < 9):
        ...
        if do_readline:
            ...
            elif self.compiler.find_library_file((self.lib_dirs + ['/usr/lib/termcap']), 'termcap'):
                readline_libs.append('termcap')
            self.add(Extension('readline', ['readline.c'], library_dirs=['/usr/lib/termcap'], extra_link_args=readline_extra_link_args, libraries=readline_libs))
        else:
        ...
        if curses_library.startswith('ncurses'):
            ...
            self.add(Extension('_curses', ['_cursesmodule.c'], include_dirs=curses_includes, define_macros=curses_defines, libraries=curses_libs))
        ...
        if (curses_enabled and (not skip_curses_panel) and self.compiler.find_library_file(self.lib_dirs, panel_library)):
            self.add(Extension('_curses_panel', ['_curses_panel.c'], include_dirs=curses_includes, define_macros=curses_defines, libraries=[panel_library, *curses_libs]))
    ...
    def detect_socket(self):
        if (not VXWORKS):
            ...
            libs = ['net']
            self.add(Extension('_socket', ['socketmodule.c'], depends=['socketmodule.h'], libraries=libs))
    ...
    def detect_dbm_gdbm(self):
        ...
        db_inc_paths = ['/usr/include/db4', '/usr/local/include/db4', '/opt/sfw/include/db4', '/usr/include/db3', '/usr/local/include/db3', '/opt/sfw/include/db3', '/sw/include/db4', '/sw/include/db3']
        ...
        try:
            ...
            while db_found_vers:
                ...
                db_dirs_to_check = [db_incdir.replace('include', 'lib64'), db_incdir.replace('include', 'lib')]
                if (not MACOS):
                ...
                for dblib in (('db-%d.%d' % db_ver), ('db%d%d' % db_ver), ('db%d' % db_ver[0])):
                    dblib_file = self.compiler.find_library_file((db_dirs_to_check + self.lib_dirs), dblib)
                    if dblib_file:
        ...
        dbm_order = ['gdbm']
        if (not CYGWIN):
            config_args = [arg.strip("'") for arg in sysconfig.get_config_var('CONFIG_ARGS').split()]
            ...
            for cand in dbm_order:
                if (cand == 'ndbm'):
                    if (find_file('ndbm.h', self.inc_dirs, []) is not None):
                        ...
                        dbmext = Extension('_dbm', ['_dbmmodule.c'], define_macros=[('HAVE_NDBM_H', None)], libraries=ndbm_libs)
                        break
                elif (cand == 'gdbm'):
                    if self.compiler.find_library_file(self.lib_dirs, 'gdbm'):
                        ...
                        if (find_file('gdbm/ndbm.h', self.inc_dirs, []) is not None):
                            ...
                            dbmext = Extension('_dbm', ['_dbmmodule.c'], define_macros=[('HAVE_GDBM_NDBM_H', None)], libraries=gdbm_libs)
                            break
                        if (find_file('gdbm-ndbm.h', self.inc_dirs, []) is not None):
                            if dbm_setup_debug:
                                print('building dbm using gdbm')
                            dbmext = Extension('_dbm', ['_dbmmodule.c'], define_macros=[('HAVE_GDBM_DASH_NDBM_H', None)], libraries=gdbm_libs)
                            break
                elif (cand == 'bdb'):
                    if dblibs:
                        if dbm_setup_debug:
                            print('building dbm using bdb')
                        dbmext = Extension('_dbm', ['_dbmmodule.c'], library_dirs=dblib_dir, runtime_library_dirs=dblib_dir, include_dirs=db_incs, define_macros=[('HAVE_BERKDB_H', None), ('DB_DBM_HSEARCH', None)], libraries=dblibs)
                        break
    ...
    def detect_sqlite(self):
        ...
        for d_ in (self.inc_dirs + sqlite_inc_paths):
            ...
            if os.path.exists(f):
                ...
                with open(f) as file:
                    incf = file.read()
                m = re.search('\\s*.*#\\s*.*define\\s.*SQLITE_VERSION\\W*"([\\d\\.]*)"', incf)
                if m:
                    ...
                    if (sqlite_version_tuple >= MIN_SQLITE_VERSION_NUMBER):
                        ...
                        print(('%s: version %s is too old, need >= %s' % (d, sqlite_version, MIN_SQLITE_VERSION)))
                elif sqlite_setup_debug:
                    print(('sqlite: %s had no SQLITE_VERSION' % (f,)))
        if sqlite_incdir:
            sqlite_dirs_to_check = [os.path.join(sqlite_incdir, '..', 'lib64'), os.path.join(sqlite_incdir, '..', 'lib'), os.path.join(sqlite_incdir, '..', '..', 'lib64'), os.path.join(sqlite_incdir, '..', '..', 'lib')]
            sqlite_libfile = self.compiler.find_library_file((sqlite_dirs_to_check + self.lib_dirs), 'sqlite3')
            if sqlite_libfile:
                sqlite_libdir = [os.path.abspath(os.path.dirname(sqlite_libfile))]
        if (sqlite_incdir and sqlite_libdir):
            sqlite_srcs = ['_sqlite/cache.c', '_sqlite/connection.c', '_sqlite/cursor.c', '_sqlite/microprotocols.c', '_sqlite/module.c', '_sqlite/prepare_protocol.c', '_sqlite/row.c', '_sqlite/statement.c', '_sqlite/util.c']
            sqlite_defines = []
            if (not MS_WINDOWS):
            ...
            if (sqlite_libdir and (sqlite_libdir[0] in self.lib_dirs)):
                sqlite_libdir = None
            self.add(Extension('_sqlite3', sqlite_srcs, define_macros=sqlite_defines, include_dirs=include_dirs, library_dirs=sqlite_libdir, extra_link_args=sqlite_extra_link_args, libraries=['sqlite3']))
        else:
    ...
    def detect_platform_specific_exts(self):
        ...
        if MACOS:
            self.add(Extension('_scproxy', ['_scproxy.c'], extra_link_args=['-framework', 'SystemConfiguration', '-framework', 'CoreFoundation']))
    ...
    def detect_compress_exts(self):
        ...
        if (zlib_inc is not None):
            ...
            if (version >= version_req):
                if self.compiler.find_library_file(self.lib_dirs, 'z'):
                    ...
                    self.add(Extension('zlib', ['zlibmodule.c'], libraries=['z'], extra_link_args=zlib_extra_link_args))
                    have_zlib = True
                else:
        ...
        if have_zlib:
            ...
            libraries = []
            extra_link_args = []
        self.add(Extension('binascii', ['binascii.c'], extra_compile_args=extra_compile_args, libraries=libraries, extra_link_args=extra_link_args))
        if self.compiler.find_library_file(self.lib_dirs, 'bz2'):
            if MACOS:
                bz2_extra_link_args = ('-Wl,-search_paths_first',)
            else:
                bz2_extra_link_args = ()
            self.add(Extension('_bz2', ['_bz2module.c'], libraries=['bz2'], extra_link_args=bz2_extra_link_args))
        else:
    ...
    def detect_expat_elementtree(self):
        if ('--with-system-expat' in sysconfig.get_config_var('CONFIG_ARGS')):
            ...
            expat_depends = ['expat/ascii.h', 'expat/asciitab.h', 'expat/expat.h', 'expat/expat_config.h', 'expat/expat_external.h', 'expat/internal.h', 'expat/latin1tab.h', 'expat/utf8tab.h', 'expat/xmlrole.h', 'expat/xmltok.h', 'expat/xmltok_impl.h']
            cc = sysconfig.get_config_var('CC').split()[0]
            ret = run_command(('"%s" -Werror -Wno-unreachable-code -E -xc /dev/null >/dev/null 2>&1' % cc))
            if (ret == 0):
                extra_compile_args.append('-Wno-unreachable-code')
        self.add(Extension('pyexpat', define_macros=define_macros, extra_compile_args=extra_compile_args, include_dirs=expat_inc, libraries=expat_lib, sources=(['pyexpat.c'] + expat_sources), depends=expat_depends))
        if os.path.isfile(os.path.join(self.srcdir, 'Modules', '_elementtree.c')):
            define_macros.append(('USE_PYEXPAT_CAPI', None))
            self.add(Extension('_elementtree', define_macros=define_macros, include_dirs=expat_inc, libraries=expat_lib, sources=['_elementtree.c'], depends=['pyexpat.c', *expat_sources, *expat_depends]))
        else:
    ...
    def detect_multiprocessing(self):
        if MS_WINDOWS:
            multiprocessing_srcs = ['_multiprocessing/multiprocessing.c', '_multiprocessing/semaphore.c']
        else:
            multiprocessing_srcs = ['_multiprocessing/multiprocessing.c']
            if (sysconfig.get_config_var('HAVE_SEM_OPEN') and (not sysconfig.get_config_var('POSIX_SEMAPHORES_NOT_ENABLED'))):
                multiprocessing_srcs.append('_multiprocessing/semaphore.c')
            if (sysconfig.get_config_var('HAVE_SHM_OPEN') and sysconfig.get_config_var('HAVE_SHM_UNLINK')):
    ...
    def detect_uuid(self):
        ...
        if (uuid_incs is not None):
            if self.compiler.find_library_file(self.lib_dirs, 'uuid'):
                ...
                uuid_libs = []
            self.add(Extension('_uuid', ['_uuidmodule.c'], libraries=uuid_libs, include_dirs=uuid_incs))
        else:
    ...
    def detect_modules(self):
        ...
        if ('d' not in sysconfig.get_config_var('ABIFLAGS')):
            self.add(Extension('xxlimited', ['xxlimited.c'], define_macros=[('Py_LIMITED_API', '0x03050000')]))
    ...
    def detect_tkinter_explicitly(self):
        ...
        self.add(Extension('_tkinter', ['_tkinter.c', 'tkappinit.c'], define_macros=[('WITH_APPINIT', 1)], extra_compile_args=extra_compile_args, extra_link_args=extra_link_args))
        return True
    ...
    def detect_tkinter_darwin(self):
        ...
        if macosx_sdk_specified():
            framework_dirs = [join(sysroot, 'Library', 'Frameworks'), join(sysroot, 'System', 'Library', 'Frameworks')]
        else:
            framework_dirs = [join('/', 'Library', 'Frameworks'), join(sysroot, 'System', 'Library', 'Frameworks')]
        for F in framework_dirs:
            ...
            return False
        include_dirs = [join(F, (fw + '.framework'), H) for fw in ('Tcl', 'Tk') for H in ('Headers',)]
        compile_args = ['-F', F]
        ...
        if (not os.path.exists(self.build_temp)):
            os.makedirs(self.build_temp)
        run_command("file {}/Tk.framework/Tk | grep 'for architecture' > {}".format(F, tmpfile))
        with open(tmpfile) as fp:
            detected_archs = []
            for ln in fp:
                a = ln.split()[(- 1)]
        ...
        self.add(Extension('_tkinter', ['_tkinter.c', 'tkappinit.c'], define_macros=[('WITH_APPINIT', 1)], include_dirs=include_dirs, libraries=[], extra_compile_args=compile_args, extra_link_args=link_args))
        return True
    ...
    def detect_tkinter(self):
        ...
        for version in ['8.6', '86', '8.5', '85', '8.4', '84', '8.3', '83', '8.2', '82', '8.1', '81', '8.0', '80']:
        ...
        if ((tcllib is None) or (tklib is None) or (tcl_includes is None) or (tk_includes is None)):
        ...
        if (not CYGWIN):
            libs.append('X11')
        self.add(Extension('_tkinter', ['_tkinter.c', 'tkappinit.c'], define_macros=([('WITH_APPINIT', 1)] + defs), include_dirs=include_dirs, libraries=libs, library_dirs=added_lib_dirs))
        return True

    def configure_ctypes_darwin(self, ext):
        ffi_srcdir = os.path.abspath(os.path.join(self.srcdir, 'Modules', '_ctypes', 'libffi_osx'))
        sources = [os.path.join(ffi_srcdir, p) for p in ['ffi.c', 'x86/darwin64.S', 'x86/x86-darwin.S', 'x86/x86-ffi_darwin.c', 'x86/x86-ffi64.c', 'powerpc/ppc-darwin.S', 'powerpc/ppc-darwin_closure.S', 'powerpc/ppc-ffi_darwin.c', 'powerpc/ppc64-darwin_closure.S']]
    ...
    def detect_ctypes(self):
        ...
        extra_link_args = []
        sources = ['_ctypes/_ctypes.c', '_ctypes/callbacks.c', '_ctypes/callproc.c', '_ctypes/stgdict.c', '_ctypes/cfield.c']
        ...
        ext = Extension('_ctypes', include_dirs=include_dirs, extra_compile_args=extra_compile_args, extra_link_args=extra_link_args, libraries=[], sources=sources, depends=depends)
        self.add(ext)
        if TEST_EXTENSIONS:
            self.add(Extension('_ctypes_test', sources=['_ctypes/_ctypes_test.c'], libraries=['m']))
    ...
    def detect_decimal(self):
        ...
        if ('--with-system-libmpdec' in sysconfig.get_config_var('CONFIG_ARGS')):
            ...
            depends = ['_decimal/docstrings.h', '_decimal/libmpdec/basearith.h', '_decimal/libmpdec/bits.h', '_decimal/libmpdec/constants.h', '_decimal/libmpdec/convolute.h', '_decimal/libmpdec/crt.h', '_decimal/libmpdec/difradix2.h', '_decimal/libmpdec/fnt.h', '_decimal/libmpdec/fourstep.h', '_decimal/libmpdec/io.h', '_decimal/libmpdec/mpalloc.h', '_decimal/libmpdec/mpdecimal.h', '_decimal/libmpdec/numbertheory.h', '_decimal/libmpdec/sixstep.h', '_decimal/libmpdec/transpose.h', '_decimal/libmpdec/typearith.h', '_decimal/libmpdec/umodarith.h']
        config = {'x64': [('CONFIG_64', '1'), ('ASM', '1')], 'uint128': [('CONFIG_64', '1'), ('ANSI', '1'), ('HAVE_UINT128_T', '1')], 'ansi64': [('CONFIG_64', '1'), ('ANSI', '1')], 'ppro': [('CONFIG_32', '1'), ('PPRO', '1'), ('ASM', '1')], 'ansi32': [('CONFIG_32', '1'), ('ANSI', '1')], 'ansi-legacy': [('CONFIG_32', '1'), ('ANSI', '1'), ('LEGACY_COMPILER', '1')], 'universal': [('UNIVERSAL', '1')]}
        cc = sysconfig.get_config_var('CC')
        ...
        self.add(Extension('_decimal', include_dirs=include_dirs, libraries=libraries, define_macros=define_macros, undef_macros=undef_macros, extra_compile_args=extra_compile_args, sources=sources, depends=depends))
    ...
    def detect_openssl_hashlib(self):
        ...
        if config_vars.get('HAVE_X509_VERIFY_PARAM_SET1_HOST'):
            self.add(Extension('_ssl', ['_ssl.c'], include_dirs=openssl_includes, library_dirs=openssl_libdirs, libraries=openssl_libs, depends=['socketmodule.h', '_ssl/debughelpers.c']))
        else:
            self.missing.append('_ssl')
        self.add(Extension('_hashlib', ['_hashopenssl.c'], depends=['hashlib.h'], include_dirs=openssl_includes, library_dirs=openssl_libdirs, libraries=openssl_libs))
    ...
    def detect_hash_builtins(self):
        ...
        if ('sha256' in configured):
            self.add(Extension('_sha256', ['sha256module.c'], extra_compile_args=['-DPy_BUILD_CORE_MODULE'], depends=['hashlib.h']))
        if ('sha512' in configured):
            self.add(Extension('_sha512', ['sha512module.c'], extra_compile_args=['-DPy_BUILD_CORE_MODULE'], depends=['hashlib.h']))
        ...
        if ('blake2' in configured):
            ...
            self.add(Extension('_blake2', ['_blake2/blake2module.c', '_blake2/blake2b_impl.c', '_blake2/blake2s_impl.c'], depends=blake2_deps))
    ...
    def detect_nis(self):
        ...
        library_dirs = []
        includes_dirs = []
        rpcsvc_inc = find_file('rpcsvc/yp_prot.h', self.inc_dirs, [os.path.join(inc_dir, 'nsl') for inc_dir in self.inc_dirs])
        rpc_inc = find_file('rpc/rpc.h', self.inc_dirs, [os.path.join(inc_dir, 'tirpc') for inc_dir in self.inc_dirs])
        ...
        if self.compiler.find_library_file(self.lib_dirs, 'tirpc'):
            libs.append('tirpc')
        self.add(Extension('nis', ['nismodule.c'], libraries=libs, library_dirs=library_dirs, include_dirs=includes_dirs))
...
class PyBuildInstall(install):
    ...
    def initialize_options(self):
        ...
        self.warn_dir = 0
    sub_commands = [('install_lib', install.has_lib), ('install_headers', install.has_headers), ('install_scripts', install.has_scripts), ('install_data', install.has_data)]
...
def main():
    ...
    setup(name='Python', version=sys.version.split()[0], url=('http://www.python.org/%d.%d' % sys.version_info[:2]), maintainer='Guido van Rossum and the Python community', maintainer_email='python-dev@python.org', description='A high-level object-oriented programming language', long_description=SUMMARY.strip(), license='PSF license', classifiers=[x for x in CLASSIFIERS.split('\n') if x], platforms=['Many'], cmdclass={'build_ext': PyBuildExt, 'build_scripts': PyBuildScripts, 'install': PyBuildInstall, 'install_lib': PyBuildInstallLib}, ext_modules=[Extension('_struct', ['_struct.c'])], scripts=['Tools/scripts/pydoc3', 'Tools/scripts/idle3', 'Tools/scripts/2to3'])
