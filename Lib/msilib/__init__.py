...
def init_database(name, schema, ProductName, ProductCode, ProductVersion, Manufacturer):
    ...
    si.Persist()
    add_data(db, 'Property', [('ProductName', ProductName), ('ProductCode', ProductCode), ('ProductVersion', ProductVersion), ('Manufacturer', Manufacturer), ('ProductLanguage', '1033')])
    db.Commit()
    return db
...
class Directory():
    ...
    def remove_pyc(self):
        ...
        add_data(self.db, 'RemoveFile', [((self.component + 'c'), self.component, '*.pyc', self.logical, 2)])
...
class Feature():
    ...
    def __init__(self, db, id, title, desc, display, level=1, parent=None, directory=None, attributes=0):
        ...
        if parent:
            parent = parent.id
        add_data(db, 'Feature', [(id, parent, title, desc, display, level, directory, attributes)])
...
class Control():
    ...
    def event(self, event, argument, condition='1', ordering=None):
        add_data(self.dlg.db, 'ControlEvent', [(self.dlg.name, self.name, event, argument, condition, ordering)])
    ...
    def condition(self, action, condition):
        add_data(self.dlg.db, 'ControlCondition', [(self.dlg.name, self.name, action, condition)])
...
class RadioButtonGroup(Control):
    ...
    def add(self, name, x, y, w, h, text, value=None):
        if (value is None):
            value = name
        add_data(self.dlg.db, 'RadioButton', [(self.property, self.index, value, x, y, w, h, text, None)])
        self.index += 1
...
class Dialog():
    ...
    def control(self, name, type, x, y, w, h, attr, prop, text, next, help):
        add_data(self.db, 'Control', [(self.name, name, type, x, y, w, h, attr, prop, text, next, help)])
    ...
    def radiogroup(self, name, x, y, w, h, attr, prop, text, next):
        add_data(self.db, 'Control', [(self.name, name, 'RadioButtonGroup', x, y, w, h, attr, prop, text, next, None)])
