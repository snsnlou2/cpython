...
async def create_subprocess_shell(cmd, stdin=None, stdout=None, stderr=None, loop=None, limit=streams._DEFAULT_LIMIT, **kwds):
    if (loop is None):
    ...
    (transport, protocol) = (await loop.subprocess_shell(protocol_factory, cmd, stdin=stdin, stdout=stdout, stderr=stderr, **kwds))
    return Process(transport, protocol, loop)

async def create_subprocess_exec(program, *args, stdin=None, stdout=None, stderr=None, loop=None, limit=streams._DEFAULT_LIMIT, **kwds):
    if (loop is None):
        loop = events.get_event_loop()
    else:
        warnings.warn('The loop argument is deprecated since Python 3.8 and scheduled for removal in Python 3.10.', DeprecationWarning, stacklevel=2)
    protocol_factory = (lambda : SubprocessStreamProtocol(limit=limit, loop=loop))
    (transport, protocol) = (await loop.subprocess_exec(protocol_factory, program, *args, stdin=stdin, stdout=stdout, stderr=stderr, **kwds))
    return Process(transport, protocol, loop)
