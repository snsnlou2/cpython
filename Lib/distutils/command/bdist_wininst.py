...
class bdist_wininst(Command):
    ...
    user_options = [('bdist-dir=', None, 'temporary directory for creating the distribution'), ('plat-name=', 'p', ('platform name to embed in generated filenames (default: %s)' % get_platform())), ('keep-temp', 'k', ('keep the pseudo-installation tree around after ' + 'creating the distribution archive')), ('target-version=', None, ('require a specific python version' + ' on the target system')), ('no-target-compile', 'c', 'do not compile .py to .pyc on the target system'), ('no-target-optimize', 'o', 'do not compile .py to .pyo (optimized) on the target system'), ('dist-dir=', 'd', 'directory to put final built distributions in'), ('bitmap=', 'b', 'bitmap to use for the installer instead of python-powered logo'), ('title=', 't', 'title to display on the installer background instead of default'), ('skip-build', None, 'skip rebuilding everything (for testing/debugging)'), ('install-script=', None, 'basename of installation script to be run after installation or before deinstallation'), ('pre-install-script=', None, 'Fully qualified filename of a script to be run before any files are installed.  This script need not be in the distribution'), ('user-access-control=', None, "specify Vista's UAC handling - 'none'/default=no handling, 'auto'=use UAC if target Python installed for all users, 'force'=always use UAC")]
    boolean_options = ['keep-temp', 'no-target-compile', 'no-target-optimize', 'skip-build']
    _unsupported = (sys.platform != 'win32')

    def __init__(self, *args, **kw):
        super().__init__(*args, **kw)
        warnings.warn('bdist_wininst command is deprecated since Python 3.8, use bdist_wheel (wheel packages) instead', DeprecationWarning, 2)
