...
def getExampleEntries():
    return [entry[:(- 3)] for entry in os.listdir(demo_dir) if (entry.endswith('.py') and (entry[0] != '_'))]
help_entries = (('Turtledemo help', __doc__), ('About turtledemo', about_turtledemo), ('About turtle module', turtle.__doc__))
...
class DemoWindow(object):
    ...
    def __init__(self, filename=None):
        ...
        if darwin:
            import subprocess
            subprocess.run(['osascript', '-e', 'tell application "System Events"', '-e', 'set frontmost of the first process whose unix id is {} to true'.format(os.getpid()), '-e', 'end tell'], stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL)
        ...
        self.output_lbl = Label(root, height=1, text=' --- ', bg='#ddf', font=('Arial', 16, 'normal'), borderwidth=2, relief=RIDGE)
        self.start_btn = Button(root, text=' START ', font=btnfont, fg='white', disabledforeground='#fed', command=self.startDemo)
        self.stop_btn = Button(root, text=' STOP ', font=btnfont, fg='white', disabledforeground='#fed', command=self.stopIt)
        self.clear_btn = Button(root, text=' CLEAR ', font=btnfont, fg='white', disabledforeground='#fed', command=self.clearCanvas)
    ...
    def makeGraphFrame(self, root):
        ...
        self.canvwidth = 1000
        self.canvheight = 800
        turtle._Screen._canvas = self._canvas = canvas = turtle.ScrolledCanvas(root, 800, 600, self.canvwidth, self.canvheight)
