...
class BitmapImageTest(AbstractTkTest, unittest.TestCase):
    ...
    def test_create_from_file(self):
        image = tkinter.BitmapImage('::img::test', master=self.root, foreground='yellow', background='blue', file=self.testfile)
    ...
    def test_create_from_data(self):
        with open(self.testfile, 'rb') as f:
            data = f.read()
        image = tkinter.BitmapImage('::img::test', master=self.root, foreground='yellow', background='blue', data=data)
        self.assertEqual(str(image), '::img::test')
        self.assertEqual(image.type(), 'bitmap')
        self.assertEqual(image.width(), 16)
        self.assertEqual(image.height(), 16)
    ...
    def test_configure_data(self):
        ...
        self.assertEqualStrList(image['data'], ('-data', '', '', '', data.decode('ascii')))
        self.assertEqual(image.width(), 16)
        self.assertEqual(image.height(), 16)
        self.assertEqual(image['maskdata'], '-maskdata {} {} {} {}')
        image.configure(maskdata=data)
        self.assertEqualStrList(image['maskdata'], ('-maskdata', '', '', '', data.decode('ascii')))
    ...
    def test_configure_file(self):
        ...
        self.assertEqualStrList(image['maskfile'], ('-maskfile', '', '', '', self.testfile))
...
class PhotoImageTest(AbstractTkTest, unittest.TestCase):
    ...
    def test_put(self):
        ...
        self.assertEqual(image.get(5, 6), self.colorlist(0, (128 if (tkinter.TkVersion >= 8.6) else 255), 0))
    ...
    def test_write(self):
        ...
        image2 = tkinter.PhotoImage('::img::test2', master=self.root, format='ppm', file=os_helper.TESTFN)
        ...
        image3 = tkinter.PhotoImage('::img::test3', master=self.root, format='gif', file=os_helper.TESTFN)
