...
class Test_TestCase(unittest.TestCase, TestEquality, TestHashing):
    ...
    def test_run_call_order__failure_in_test_default_result(self):
        ...
        class Foo(Test.LoggingTestCase):
            ...
            def test(self):
                super(Foo, self).test()
                self.fail('raised by Foo.test')
        expected = ['startTestRun', 'startTest', 'setUp', 'test', 'tearDown', 'addFailure', 'stopTest', 'stopTestRun']
        events = []
        Foo(events).run()
        self.assertEqual(events, expected)
    ...
    def test_run_call_order__subtests(self):
        events = []
        result = LoggingResult(events)
        expected = ['startTest', 'setUp', 'test', 'tearDown', 'addSubTestFailure', 'addSubTestSuccess', 'addSubTestFailure', 'addSubTestFailure', 'addSubTestSuccess', 'addError', 'stopTest']
    ...
    def test_run_call_order__subtests_success(self):
        events = []
        result = LoggingResult(events)
        expected = ((['startTest', 'setUp', 'test', 'tearDown'] + (6 * ['addSubTestSuccess'])) + ['addSuccess', 'stopTest'])
        ...
        class Foo(Test.LoggingTestCase):
            ...
            def test(self):
                ...
                with self.subTest(i=2):
                    self.fail('failure')
                self.fail('failure')
        expected = ['startTest', 'setUp', 'test', 'tearDown', 'addSubTestFailure', 'stopTest']
        Foo(events).run(result)
        self.assertEqual(events, expected)
