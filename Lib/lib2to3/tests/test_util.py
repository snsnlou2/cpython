...
class Test_Call(MacroTestCase):
    ...
    def test(self):
        kids = [None, [Leaf(token.NUMBER, 1), Leaf(token.NUMBER, 2), Leaf(token.NUMBER, 3)], [Leaf(token.NUMBER, 1), Leaf(token.NUMBER, 3), Leaf(token.NUMBER, 2), Leaf(token.NUMBER, 4)], [Leaf(token.STRING, 'b'), Leaf(token.STRING, 'j', prefix=' ')]]
...
class Test_does_tree_import(support.TestCase):
    ...
    def try_with(self, string):
        failing_tests = (('a', 'a', 'from a import b'), ('a.d', 'a', 'from a.d import b'), ('d.a', 'a', 'from d.a import b'), (None, 'a', 'import b'), (None, 'a', 'import b, c, d'))
        for (package, name, import_) in failing_tests:
            n = self.does_tree_import(package, name, ((import_ + '\n') + string))
            self.assertFalse(n)
            n = self.does_tree_import(package, name, ((string + '\n') + import_))
            self.assertFalse(n)
        passing_tests = (('a', 'a', 'from a import a'), ('x', 'a', 'from x import a'), ('x', 'a', 'from x import b, c, a, d'), ('x.b', 'a', 'from x.b import a'), ('x.b', 'a', 'from x.b import b, c, a, d'), (None, 'a', 'import a'), (None, 'a', 'import b, c, a, d'))
