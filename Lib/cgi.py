...
class FieldStorage():
    ...
    def read_urlencoded(self):
        ...
        if self.qs_on_post:
            qs += ('&' + self.qs_on_post)
        query = urllib.parse.parse_qsl(qs, self.keep_blank_values, self.strict_parsing, encoding=self.encoding, errors=self.errors, max_num_fields=self.max_num_fields)
    ...
    def read_multi(self, environ, keep_blank_values, strict_parsing):
        ...
        self.list = []
        if self.qs_on_post:
            query = urllib.parse.parse_qsl(self.qs_on_post, self.keep_blank_values, self.strict_parsing, encoding=self.encoding, errors=self.errors, max_num_fields=self.max_num_fields)
        ...
        while True:
            ...
            part = klass(self.fp, headers, ib, environ, keep_blank_values, strict_parsing, limit, self.encoding, self.errors, max_num_fields)
            if (max_num_fields is not None):
                max_num_fields -= 1
                if part.list:
