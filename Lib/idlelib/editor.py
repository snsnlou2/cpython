...
class EditorWindow(object):
    ...
    def __init__(self, flist=None, filename=None, key=None, root=None):
        ...
        if (EditorWindow.help_url is None):
            ...
            if sys.platform.count('linux'):
                ...
                chmfile = os.path.join(sys.base_prefix, 'Doc', ('Python%s.chm' % _sphinx_version()))
                if os.path.isfile(chmfile):
                    dochome = chmfile
            elif (sys.platform == 'darwin'):
                dochome = os.path.join(sys.base_prefix, 'Resources/English.lproj/Documentation/index.html')
        ...
        self.recent_files_path = (idleConf.userdir and os.path.join(idleConf.userdir, 'recent-files.lst'))
        self.prompt_last_line = ''
        self.text_frame = text_frame = Frame(top)
        self.vbar = vbar = Scrollbar(text_frame, name='vbar')
        width = idleConf.GetOption('main', 'EditorWindow', 'width', type='int')
        text_options = {'name': 'text', 'padx': 5, 'wrap': 'none', 'highlightthickness': 0, 'width': width, 'tabstyle': 'wordprocessor', 'height': idleConf.GetOption('main', 'EditorWindow', 'height', type='int')}
        ...
        text.event_add('<<try-open-completions>>', '<KeyRelease-period>', '<KeyRelease-slash>', '<KeyRelease-backslash>')
    ...
    menu_specs = [('file', '_File'), ('edit', '_Edit'), ('format', 'F_ormat'), ('run', '_Run'), ('options', '_Options'), ('window', '_Window'), ('help', '_Help')]
    ...
    def right_menu_event(self, event):
        ...
        try:
            in_selection = (text.compare('sel.first', '<=', newdex) and text.compare(newdex, '<=', 'sel.last'))
        except TclError:
            in_selection = False
        if (not in_selection):
    ...
    def goto_line_event(self, event):
        text = self.text
        lineno = query.Goto(text, 'Go To Line', "Enter a positive integer\n('big' = end of file):").result
    ...
    def open_module(self):
        ...
        try:
            ...
            name = ''
        file_path = query.ModuleName(self.text, 'Open Module', 'Enter the name of a Python module\nto search on sys.path and open:', name).result
    ...
    def update_recent_files_list(self, new_file=None):
        ...
        if (file_path and os.path.exists(file_path)):
            with open(file_path, 'r', encoding='utf_8', errors='replace') as rf_list_file:
        ...
        if file_path:
            try:
                ...
                if (not getattr(self.root, 'recentfiles_message', False)):
                    ...
                    tkMessageBox.showwarning(title='IDLE Warning', message=f'''Cannot save Recent Files list to disk.
  {err}
Select OK to continue.''', parent=self.text)
    ...
    def toggle_line_numbers_event(self, event=None):
        ...
        if self.line_numbers.is_shown:
            ...
            menu_label = 'Hide'
        self.update_menu_label(menu='options', index='*Line Numbers', label=f'{menu_label} Line Numbers')
...
class IndentSearcher(object):
    ...
    def tokeneater(self, type, token, start, end, line, INDENT=tokenize.INDENT, NAME=tokenize.NAME, OPENERS=('class', 'def', 'for', 'if', 'try', 'while')):
        if self.finished:
            pass
...
def get_accelerator(keydefs, eventname):
    ...
    if ((not keylist) or (macosx.isCocoaTk() and (eventname in {'<<open-module>>', '<<goto-line>>', '<<change-indentwidth>>'}))):
        return ''
    s = keylist[0]
