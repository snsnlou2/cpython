...
def manage_socket(address):
    for i in range(3):
        ...
        try:
            ...
            print((('IDLE Subprocess: OSError: ' + err.args[1]) + ', retrying....'), file=sys.__stderr__)
            socket_error = err
    else:
        print('IDLE Subprocess: Connection to IDLE GUI failed, exiting.', file=sys.__stderr__)
...
def show_socket_error(err, address):
    ...
    fix_scaling(root)
    root.withdraw()
    showerror('Subprocess Connection Error', f'''IDLE's subprocess can't connect to {address[0]}:{address[1]}.
Fatal OSError #{err.errno}: {err.strerror}.
See the 'Startup failure' section of the IDLE doc, online at
https://docs.python.org/3/library/idle.html#startup-failure''', parent=root)
    root.destroy()
...
def install_recursionlimit_wrappers():
    ...
    fixdoc(setrecursionlimit, f'''            This IDLE wrapper adds {RECURSIONLIMIT_DELTA} to prevent possible
            uninterruptible loops.''')

    @functools.wraps(sys.getrecursionlimit)
    def getrecursionlimit():
        return (getrecursionlimit.__wrapped__() - RECURSIONLIMIT_DELTA)
    fixdoc(getrecursionlimit, f'''            This IDLE wrapper subtracts {RECURSIONLIMIT_DELTA} to compensate
...
def uninstall_recursionlimit_wrappers():
    ...
    if (getattr(sys.setrecursionlimit, '__wrapped__', None) and getattr(sys.getrecursionlimit, '__wrapped__', None)):
