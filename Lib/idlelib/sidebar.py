...
def get_widget_padding(widget):
    ...
    padx = sum(map(widget.tk.getint, [info['padx'], widget.cget('padx'), widget.cget('border')]))
    pady = sum(map(widget.tk.getint, [info['pady'], widget.cget('pady'), widget.cget('border')]))
    return (padx, pady)
...
class BaseSideBar():
    ...
    def __init__(self, editwin):
        ...
        self.sidebar_text = tk.Text(self.parent, width=1, wrap=tk.NONE, padx=2, pady=pady, borderwidth=0, highlightthickness=0)
    ...
    def update_colors(self):
        ...
        self._update_colors(foreground=colors['foreground'], background=colors['background'])

    def _update_colors(self, foreground, background):
        self.sidebar_text.config(fg=foreground, bg=background, selectforeground=foreground, selectbackground=background, inactiveselectbackground=background)
...
class LineNumbers(BaseSideBar):
    ...
    def bind_events(self):
        ...
        def bind_mouse_event(event_name, target_event_name):
            handler = functools.partial(self.redirect_mousebutton_event, event_name=target_event_name)
            self.sidebar_text.bind(event_name, handler)
        for button in [2, 3, 4, 5]:
            for event_name in (f'<Button-{button}>', f'<ButtonRelease-{button}>', f'<B{button}-Motion>'):
                bind_mouse_event(event_name, target_event_name=event_name)
            for event_name in (f'<Double-Button-{button}>', f'<Triple-Button-{button}>'):
    ...
    def update_sidebar_text(self, end):
        ...
        self.sidebar_text.config(state=tk.NORMAL)
        if (end > self.prev_end):
            new_text = '\n'.join(itertools.chain([''], map(str, range((self.prev_end + 1), (end + 1)))))
