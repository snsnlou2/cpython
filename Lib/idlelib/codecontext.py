...
class CodeContext():
    ...
    def toggle_code_context_event(self, event=None):
        ...
        if (self.context is None):
            ...
            context = self.context = tkinter.Text(self.editwin.text_frame, height=1, width=1, highlightthickness=0, padx=padx, border=border, relief=SUNKEN, state='disabled')
            self.update_font()
            ...
            self.cell00 = tkinter.Frame(self.editwin.text_frame, bg=line_number_colors['background'])
            ...
            self._reset()
            menu_status = 'Show'
        self.editwin.update_menu_label(menu='options', index='* Code Context', label=f'{menu_status} Code Context')
        return 'break'
