...
class HelpsourceBrowsefileTest(unittest.TestCase):
    ...
    def test_file_replaces_path(self):
        ...
        for (path, func, result) in (('', (lambda a, b, c: ''), ''), ('', (lambda a, b, c: __file__), __file__), ('htest', (lambda a, b, c: ''), 'htest'), ('htest', (lambda a, b, c: __file__), __file__)):
            with self.subTest():
...
class HelpsourceEntryokTest(unittest.TestCase):
    ...
    def test_entry_ok_helpsource(self):
        dialog = self.Dummy_HelpSource()
        for (name, path, result) in ((None, None, None), (None, 'doc.txt', None), ('doc', None, None), ('doc', 'doc.txt', ('doc', 'doc.txt'))):
            with self.subTest():
