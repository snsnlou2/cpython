...
class LineNumbersTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        GetHighlight_patcher = unittest.mock.patch.object(idlelib.sidebar.idleConf, 'GetHighlight', mock_idleconf_GetHighlight)
        ...
        GetFont_patcher = unittest.mock.patch.object(idlelib.sidebar.idleConf, 'GetFont', mock_idleconf_GetFont)
    ...
    def simulate_drag(self, start_line, end_line):
        ...
        for (x, y) in zip(map(int, lerp(start_x, end_x, steps=11)), map(int, lerp(start_y, end_y, steps=11))):
