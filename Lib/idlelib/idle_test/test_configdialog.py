...
class ButtonTest(unittest.TestCase):
    ...
    def test_click_help(self):
        ...
        self.assertTrue((contents.startswith('When you click') and contents.endswith('a different name.\n')))
...
class FontPageTest(unittest.TestCase):
    ...
    def test_font_set(self):
        ...
        expected = {'EditorWindow': {'font': 'Test Font', 'font-size': default_size, 'font-bold': str(default_bold)}}
        self.assertEqual(mainpage, expected)
        self.assertEqual(d.set_samples.called, 1)
        changes.clear()
        d.font_size.set('20')
        expected = {'EditorWindow': {'font': 'Test Font', 'font-size': '20', 'font-bold': str(default_bold)}}
        self.assertEqual(mainpage, expected)
        self.assertEqual(d.set_samples.called, 2)
        changes.clear()
        d.font_bold.set((not default_bold))
        expected = {'EditorWindow': {'font': 'Test Font', 'font-size': '20', 'font-bold': str((not default_bold))}}
        self.assertEqual(mainpage, expected)
...
class HighPageTest(unittest.TestCase):
    ...
    def test_create_new_and_save_new(self):
        ...
        eq(idleConf.GetThemeDict('default', 'IDLE Classic'), idleConf.GetThemeDict('user', first_new))
        ...
        self.assertNotEqual(idleConf.GetThemeDict('user', first_new), idleConf.GetThemeDict('user', second_new))
        idleConf.SetOption('highlight', first_new, 'hit-background', 'yellow')
        eq(idleConf.GetThemeDict('user', first_new), idleConf.GetThemeDict('user', second_new))
    ...
    def test_set_color_sample(self):
        ...
        d.fg_bg_toggle.set(True)
        d.set_color_sample()
        self.assertEqual(d.style.lookup(d.frame_color_set['style'], 'background'), d.highlight_sample.tag_cget('hilite', 'foreground'))
...
class KeysPageTest(unittest.TestCase):
    ...
    def test_builtin_name(self):
        ...
        eq(mainpage, {'Keys': {'name': 'IDLE Classic Windows', 'name2': 'IDLE Modern UNIX'}})
    ...
    def test_load_keys_list(self):
        ...
        b.insert(0, '<<find>>')
        b.insert(1, '<<help>>')
        gks.result = {'<<copy>>': ['<Control-Key-c>', '<Control-Key-C>'], '<<force-open-completions>>': ['<Control-Key-space>'], '<<spam>>': ['<Key-F11>']}
        changes.add_option('keys', 'my keys', 'spam', '<Shift-Key-a>')
        expected = ('copy - <Control-Key-c> <Control-Key-C>', 'force-open-completions - <Control-Key-space>', 'spam - <Shift-Key-a>')
...
class VarTraceTest(unittest.TestCase):
    ...
    def test_add(self):
        ...
        expected = [(iv, self.var_changed_increment), (bv, self.var_changed_boolean), (sv, func)]
        self.assertEqual(tr.traced, [])
