...
class ExpandingButton(tk.Button):
    ...
    def expand(self, event=None):
        ...
        if self.is_dangerous:
            confirm = tkMessageBox.askokcancel(title='Expand huge output?', message=('\n\n'.join(['The squeezed output is very long: %d lines, %d chars.', 'Expanding it could make IDLE slow or unresponsive.', 'It is recommended to view or copy the output instead.', 'Really expand?']) % (self.numoflines, len(self.s))), default=tkMessageBox.CANCEL, parent=self.text)
            if (not confirm):
                return 'break'
