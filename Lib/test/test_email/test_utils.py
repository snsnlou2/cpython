...
class DateTimeTests(unittest.TestCase):
    ...
    def test_naive_datetime(self):
        self.assertEqual(utils.format_datetime(self.naive_dt), (self.datestring + ' -0000'))

    def test_aware_datetime(self):
        self.assertEqual(utils.format_datetime(self.aware_dt), (self.datestring + self.offsetstring))

    def test_usegmt(self):
        utc_dt = datetime.datetime(*self.dateargs, tzinfo=datetime.timezone.utc)
        self.assertEqual(utils.format_datetime(utc_dt, usegmt=True), (self.datestring + ' GMT'))
    ...
    def test_parsedate_to_datetime(self):
        self.assertEqual(utils.parsedate_to_datetime((self.datestring + self.offsetstring)), self.aware_dt)

    def test_parsedate_to_datetime_naive(self):
        self.assertEqual(utils.parsedate_to_datetime((self.datestring + ' -0000')), self.naive_dt)
...
class LocaltimeTests(unittest.TestCase):
    ...
    @unittest.skipIf(sys.platform.startswith('win'), "Windows does not use Olson's TZ database")
    @unittest.skipUnless((os.path.exists('/usr/share/zoneinfo') or os.path.exists('/usr/lib/zoneinfo')), "Can't find the Olson's TZ database")
