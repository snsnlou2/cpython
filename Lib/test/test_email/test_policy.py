...
class PolicyAPITests(unittest.TestCase):
    longMessage = True
    compat32_defaults = {'max_line_length': 78, 'linesep': '\n', 'cte_type': '8bit', 'raise_on_defect': False, 'mangle_from_': True, 'message_factory': None}
    policy_defaults = compat32_defaults.copy()
    policy_defaults.update({'utf8': False, 'raise_on_defect': False, 'header_factory': email.policy.EmailPolicy.header_factory, 'refold_source': 'long', 'content_manager': email.policy.EmailPolicy.content_manager, 'mangle_from_': False, 'message_factory': email.message.EmailMessage})
    new_policy = email.policy.EmailPolicy()
    policies = {email.policy.compat32: make_defaults(compat32_defaults, {}), email.policy.default: make_defaults(policy_defaults, {}), email.policy.SMTP: make_defaults(policy_defaults, {'linesep': '\r\n'}), email.policy.SMTPUTF8: make_defaults(policy_defaults, {'linesep': '\r\n', 'utf8': True}), email.policy.HTTP: make_defaults(policy_defaults, {'linesep': '\r\n', 'max_line_length': None}), email.policy.strict: make_defaults(policy_defaults, {'raise_on_defect': True}), new_policy: make_defaults(policy_defaults, {})}
        ...
        for (policy, expected) in self.policies.items():
            for (attr, value) in expected.items():
                with self.subTest(policy=policy, attr=attr):
                    self.assertEqual(getattr(policy, attr), value, 'change {} docs/docstrings if defaults have changed'.format(policy))

    def test_all_attributes_covered(self):
        for (policy, expected) in self.policies.items():
            for attr in dir(policy):
                with self.subTest(policy=policy, attr=attr):
                    if (attr.startswith('_') or isinstance(getattr(email.policy.EmailPolicy, attr), types.FunctionType)):
                        continue
                    else:
                        self.assertIn(attr, expected, '{} is not fully tested'.format(attr))

    def test_abc(self):
        with self.assertRaises(TypeError) as cm:
            email.policy.Policy()
        msg = str(cm.exception)
        abstract_methods = ('fold', 'fold_binary', 'header_fetch_parse', 'header_source_parse', 'header_store_parse')
    ...
    def test_adding_default_policies_preserves_default_factory(self):
        ...
        self.assertEqual(newpolicy.header_factory, email.policy.EmailPolicy.header_factory)
...
class TestPolicyPropagation(unittest.TestCase):
    ...
    def test_parser_propagates_policy_to_sub_messages(self):
        msg = self._make_msg(textwrap.dedent('            Subject: mime test\n            MIME-Version: 1.0\n            Content-Type: multipart/mixed, boundary="XXX"\n\n            --XXX\n            Content-Type: text/plain\n\n            test\n            --XXX\n            Content-Type: text/plain\n\n            test2\n            --XXX--\n            '))
...
class TestConcretePolicies(unittest.TestCase):
    ...
    def test_header_store_parse_rejects_newlines(self):
        ...
        self.assertRaises(ValueError, instance.header_store_parse, 'From', 'spam\negg@foo.py')
