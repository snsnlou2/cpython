...
class FindmatchTest(unittest.TestCase):
    ...
    def test_findmatch(self):
        ...
        cases = [([{}, 'video/mpeg'], {}, (None, None)), ([c, 'foo/bar'], {}, (None, None)), ([c, 'video/mpeg'], {}, ('animate /dev/null', video_entry)), ([c, 'audio/basic', 'edit'], {}, ('audiocompose /dev/null', audio_basic_entry)), ([c, 'audio/basic', 'compose'], {}, ('audiocompose /dev/null', audio_basic_entry)), ([c, 'audio/basic', 'description'], {}, ('"An audio fragment"', audio_basic_entry)), ([c, 'audio/basic', 'foobar'], {}, (None, None)), ([c, 'video/*'], {'filename': fname}, (('animate %s' % fname), video_entry)), ([c, 'audio/basic', 'compose'], {'filename': fname}, (('audiocompose %s' % fname), audio_basic_entry)), ([c, 'audio/basic'], {'key': 'description', 'filename': fname}, ('"An audio fragment"', audio_basic_entry)), ([c, 'audio/*'], {'filename': fname}, ('/usr/local/bin/showaudio audio/*', audio_entry)), ([c, 'message/external-body'], {'plist': plist}, ('showexternal /dev/null default john python.org     /tmp foo bar', message_entry))]
