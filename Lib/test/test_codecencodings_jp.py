...
shiftjis_commonenctests = ((b'abc\x80\x80\x82\x84', 'strict', None), (b'abc\xf8', 'strict', None), (b'abc\x80\x80\x82\x84def', 'ignore', 'abcｄdef'))

class Test_SJIS_COMPAT(multibytecodec_support.TestBase, unittest.TestCase):
    encoding = 'shift_jis'
    tstring = multibytecodec_support.load_teststring('shift_jis')
    codectests = (shiftjis_commonenctests + ((b'abc\x80\x80\x82\x84', 'replace', 'abc��ｄ'), (b'abc\x80\x80\x82\x84\x88', 'replace', 'abc��ｄ�'), (b'\\~', 'strict', '\\~'), (b'\x81_\x81a\x81|', 'strict', '＼‖−'), (b'abc\x819', 'replace', 'abc�9'), (b'abc\xea\xfc', 'replace', 'abc��'), (b'abc\xffX', 'replace', 'abc�X')))

class Test_SJIS_2004(multibytecodec_support.TestBase, unittest.TestCase):
    encoding = 'shift_jis_2004'
    tstring = multibytecodec_support.load_teststring('shift_jis')
    codectests = (shiftjis_commonenctests + ((b'\\~', 'strict', '¥‾'), (b'\x81_\x81a\x81|', 'strict', '\\‖−'), (b'abc\xea\xfc', 'strict', 'abc撿'), (b'\x819xy', 'replace', '�9xy'), (b'\xffXxy', 'replace', '�Xxy'), (b'\x80\x80\x82\x84xy', 'replace', '��ｄxy'), (b'\x80\x80\x82\x84\x88xy', 'replace', '��ｄ塤y'), (b'\xfc\xfbxy', 'replace', '�閴y')))
...
class Test_SJISX0213(multibytecodec_support.TestBase, unittest.TestCase):
    ...
    codectests = (shiftjis_commonenctests + ((b'abc\x80\x80\x82\x84', 'replace', 'abc��ｄ'), (b'abc\x80\x80\x82\x84\x88', 'replace', 'abc��ｄ�'), (b'\\~', 'replace', '¥‾'), (b'\x81_\x81a\x81|', 'replace', '\\‖−')))
    xmlcharnametest = ('«ℜ» = 〈ሴ〉', b'\x85G&real;\x85Q = &lang;&#4660;&rang;')
