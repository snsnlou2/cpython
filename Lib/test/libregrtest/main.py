...
from test.libregrtest.runtest import findtests, runtest, get_abs_module, STDTESTS, NOTTESTS, PASSED, FAILED, ENV_CHANGED, SKIPPED, RESOURCE_DENIED, INTERRUPTED, CHILD_ERROR, TEST_DID_NOT_RUN, TIMEOUT, PROGRESS_MIN_TIME, format_test_result, is_failed
...
class Regrtest():
    ...
    def get_executed(self):
        return (((((set(self.good) | set(self.bad)) | set(self.skipped)) | set(self.resource_denieds)) | set(self.environment_changed)) | set(self.run_no_tests))
    ...
    def find_tests(self, tests):
        ...
        if self.ns.start:
            try:
                ...
                print(("Couldn't find starting test (%s), using all tests" % self.ns.start), file=sys.stderr)
    ...
    def display_result(self):
        ...
        if self.environment_changed:
            print()
            print('{} altered the execution environment:'.format(count(len(self.environment_changed), 'test')))
    ...
    def display_header(self):
        ...
        print(('== encodings: locale=%s, FS=%s' % (locale.getpreferredencoding(False), sys.getfilesystemencoding())))
    ...
    def get_tests_result(self):
        ...
        elif (not any((self.good, self.bad, self.skipped, self.interrupted, self.environment_changed))):
    ...
    def run_tests(self):
        if (self.ns.header or (not (self.ns.pgo or self.ns.quiet or self.ns.single or self.tests or self.ns.args))):
            self.display_header()
