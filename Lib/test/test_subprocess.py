...
class ProcessTestCase(BaseTestCase):
    ...
    def test_check_output_input_arg(self):
        output = subprocess.check_output([sys.executable, '-c', 'import sys; sys.stdout.write(sys.stdin.read().upper())'], input=b'pear')
        self.assertIn(b'PEAR', output)
    ...
    def test_check_output_stdin_with_input_arg(self):
        ...
        tf.seek(0)
        with self.assertRaises(ValueError) as c:
            output = subprocess.check_output([sys.executable, '-c', "print('will not be run')"], stdin=tf, input=b'hare')
    ...
    def test_cwd_with_relative_executable(self):
        ...
        with os_helper.temp_cwd() as wrong_dir:
            self.assertRaises(FileNotFoundError, subprocess.Popen, [doesntexist], executable=rel_python)
            self.assertRaises(FileNotFoundError, subprocess.Popen, [doesntexist], executable=rel_python, cwd=wrong_dir)
            python_dir = self._normalize_cwd(python_dir)
    ...
    @unittest.skipIf((sysconfig.get_config_var('Py_ENABLE_SHARED') == 1), 'The Python shared library cannot be loaded with an empty environment.')
    ...
    def test_repr(self):
        ...
        with subprocess.Popen(cmd, stdin=subprocess.PIPE, universal_newlines=True) as proc:
            ...
            self.assertTrue((repr(proc).startswith(result.format(proc.returncode)) and repr(proc).endswith('>')))
...
class RunFuncTestCase(BaseTestCase):
    ...
    def test_stderr_with_capture_output_arg(self):
        tf = tempfile.TemporaryFile()
        self.addCleanup(tf.close)
        with self.assertRaises(ValueError, msg='Expected ValueError when stderr and capture_output args supplied.') as c:
...
class POSIXProcessTestCase(BaseTestCase):
            ...
            try:
                ...
                try:
                    for fd in devzero_fds:
                        self._testcase.assertNotIn(fd, (self.stdin.fileno(), self.stdout.fileno(), self.stderr.fileno()), msg='At least one fd was closed early.')
                finally:
                    for fd in devzero_fds:
                        os.close(fd)
    ...
    def test_preexec_gc_module_failure(self):
        ...
        try:
            ...
            self.assertRaises(RuntimeError, subprocess.Popen, [sys.executable, '-c', ''], preexec_fn=(lambda : None))
            del gc.isenabled
            self.assertRaises(AttributeError, subprocess.Popen, [sys.executable, '-c', ''], preexec_fn=(lambda : None))
        finally:
    ...
    @unittest.skipIf(sys.platform.startswith(('netbsd', 'openbsd')), 'Due to known OS bug (issue #16762)')
    ...
    @unittest.skipUnless(((_testcapi is not None) and hasattr(_testcapi, 'W_STOPCODE')), 'need _testcapi.W_STOPCODE')
        ...
        try:
            with mock.patch.object(subprocess.Popen, '_wait') as mock__wait:
                ...
                for call in mock__wait.call_args_list[1:]:
                    self.assertNotEqual(call, mock.call(timeout=None), f'no open-ended wait() after the first allowed: {mock__wait.call_args_list}')
                sigint_calls = []
...
class CommandsWithSpaces(BaseTestCase):
    ...
    def setUp(self):
        ...
        os.write(f, b"import sys;sys.stdout.write('%d %s' % (len(sys.argv), [a.lower () for a in sys.argv]))")
        os.close(f)
