...
class UnicodeTest(string_tests.CommonTest, string_tests.MixinStrUnicodeUserStringTest, string_tests.MixinStrUnicodeTest, unittest.TestCase):
    ...
    def test_ascii(self):
        if (not sys.platform.startswith('java')):
            ...
            self.assertEqual(ascii((('𐀀' * 39) + ('\uffff' * 4096))), ascii((('𐀀' * 39) + ('\uffff' * 4096))))
    ...
    def test_rsplit(self):
        ...
        for (left, right) in ('ba', 'āĀ', '𐌁𐌀'):
            ...
            for delim in ('c', 'Ă', '𐌂'):
                ...
                self.checkequal([left, right], ((left + delim) + right), 'rsplit', delim)
                self.checkequal([(left + right)], (left + right), 'rsplit', (delim * 2))
                self.checkequal([left, right], ((left + (delim * 2)) + right), 'rsplit', (delim * 2))
    ...
    def assertCorrectUTF8Decoding(self, seq, res, err):
        ...
        self.assertEqual(seq.decode('utf-8', 'replace'), res)
        self.assertEqual(((b'aaaa' + seq) + b'bbbb').decode('utf-8', 'replace'), (('aaaa' + res) + 'bbbb'))
        res = res.replace('�', '')
        self.assertEqual(seq.decode('utf-8', 'ignore'), res)
        self.assertEqual(((b'aaaa' + seq) + b'bbbb').decode('utf-8', 'ignore'), (('aaaa' + res) + 'bbbb'))
...
class CAPITest(unittest.TestCase):
    ...
    def test_from_format(self):
        ...
        def PyUnicode_FromFormat(format, *args):
            cargs = tuple(((py_object(arg) if isinstance(arg, str) else arg) for arg in args))
        ...
        self.assertRaisesRegex(ValueError, '^PyUnicode_FromFormatV\\(\\) expects an ASCII-encoded format string, got a non-ASCII byte: 0xe9$', PyUnicode_FromFormat, b'unicode\xe9=%s', 'ascii')
    ...
    def test_copycharacters(self):
        ...
        for (idx, from_) in enumerate(strings):
            for to in strings[:idx]:
                self.assertRaises(SystemError, unicode_copycharacters, to, 0, from_, 0, 5)
            for from_start in range(5):
                self.assertEqual(unicode_copycharacters(from_, 0, from_, from_start, 5), (from_[from_start:(from_start + 5)].ljust(5, '\x00'), (5 - from_start)))
            for to_start in range(5):
                self.assertEqual(unicode_copycharacters(from_, to_start, from_, to_start, 5), (from_[to_start:(to_start + 5)].rjust(5, '\x00'), (5 - to_start)))
        s = strings[0]
    ...
    def test_encode_decimal(self):
        ...
        self.assertRaises(UnicodeEncodeError, unicode_encodedecimal, '123€', 'strict')
        self.assertRaisesRegex(ValueError, "^'decimal' codec can't encode character", unicode_encodedecimal, '123€', 'replace')
