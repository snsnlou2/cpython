...
class IOTest(unittest.TestCase):
    ...
    def test_closefd(self):
        self.assertRaises(ValueError, self.open, os_helper.TESTFN, 'w', closefd=False)
...
class CBufferedReaderTest(BufferedReaderTest, SizeofTest):
    ...
    @unittest.skipIf(MEMORY_SANITIZER, 'MSan defaults to crashing instead of returning NULL for malloc failure.')
    def test_constructor(self):
...
class BufferedWriterTest(unittest.TestCase, CommonBufferedTests):
    ...
    def test_uninitialized(self):
        ...
        del bufio
        bufio = self.tp.__new__(self.tp)
        self.assertRaisesRegex((ValueError, AttributeError), 'uninitialized|has no attribute', bufio.write, b'')
        bufio.__init__(self.MockRawIO())
...
class StatefulIncrementalDecoder(codecs.IncrementalDecoder):
    ...
    def lookupTestDecoder(cls, name):
        if (cls.codecEnabled and (name == 'test_decoder')):
            ...
            return codecs.CodecInfo(name='test_decoder', encode=latin1.encode, decode=None, incrementalencoder=None, streamreader=None, streamwriter=None, incrementaldecoder=cls)
...
class MiscIOTest(unittest.TestCase):
    ...
    def test_io_after_close(self):
        for kwargs in [{'mode': 'w'}, {'mode': 'wb'}, {'mode': 'w', 'buffering': 1}, {'mode': 'w', 'buffering': 2}, {'mode': 'wb', 'buffering': 0}, {'mode': 'r'}, {'mode': 'rb'}, {'mode': 'r', 'buffering': 1}, {'mode': 'r', 'buffering': 2}, {'mode': 'rb', 'buffering': 0}, {'mode': 'w+'}, {'mode': 'w+b'}, {'mode': 'w+', 'buffering': 1}, {'mode': 'w+', 'buffering': 2}, {'mode': 'w+b', 'buffering': 0}]:
...
class CMiscIOTest(MiscIOTest):
    ...
    def check_daemon_threads_shutdown_deadlock(self, stream_name):
        ...
        err = res.err.decode()
        if (res.rc != 0):
            pattern = "Fatal Python error: _enter_buffered_busy: could not acquire lock for <(_io\\.)?BufferedWriter name='<{stream_name}>'> at interpreter shutdown, possibly due to daemon threads".format_map(locals())
...
def load_tests(*args):
    ...
    mocks = (MockRawIO, MisbehavedRawIO, MockFileIO, CloseFailureIO, MockNonBlockWriterIO, MockUnseekableIO, MockRawIOWithoutRead, SlowFlushRawIO)
