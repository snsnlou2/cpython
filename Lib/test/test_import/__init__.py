...
from test.support.import_helper import forget, make_legacy_pyc, unlink, unload, DirsOnSysPath
from test.support.os_helper import TESTFN, rmtree, temp_umask, TESTFN_UNENCODABLE, temp_dir
...
class ImportTests(unittest.TestCase):
    ...
    def test_from_import_missing_attr_has_name_and_so_path(self):
        ...
        self.assertRegex(str(cm.exception), "cannot import name 'i_dont_exist' from '_testcapi' \\(.*\\.(so|pyd)\\)")
    ...
    def test_from_import_star_invalid_type(self):
        ...
        with _ready_to_import() as (name, path):
            ...
            globals = {}
            with self.assertRaisesRegex(TypeError, f'{re.escape(name)}\.__all__ must be str'):
        ...
        with _ready_to_import() as (name, path):
            ...
            globals = {}
            with self.assertRaisesRegex(TypeError, f'{re.escape(name)}\.__dict__ must be str'):
                exec(f'from {name} import *', globals)
            self.assertNotIn(b'invalid_type', globals)
    ...
    def test_timestamp_overflow(self):
        ...
        try:
            ...
            try:
                ...
                if (e.errno not in (getattr(errno, 'EOVERFLOW', None), getattr(errno, 'EINVAL', None))):
                    raise
    ...
    def test_from_import_AttributeError(self):
        ...
        self.assertEqual(str(cm.exception), "cannot import name 'does_not_exist' from '<unknown module name>' (unknown location)")
    ...
    def test_dll_dependency_import(self):
        ...
        depname = os.path.join(os.path.dirname(pydname), 'sqlite3{}.dll'.format(('_d' if ('_d' in pydname) else '')))
        with os_helper.temp_dir() as tmp:
            ...
            subprocess.check_call([pyexe, '-Sc', ';'.join(['import os', 'p = os.add_dll_directory({!r})'.format(os.path.dirname(depname)), 'import _sqlite3', 'p.close'])], stderr=subprocess.STDOUT, env=env, cwd=os.path.dirname(pyexe))
            shutil.copy(depname, tmp2)
            subprocess.check_call([pyexe, '-Sc', 'import _sqlite3'], stderr=subprocess.STDOUT, env=env, cwd=os.path.dirname(pyexe))
...
class RelativeImportTests(unittest.TestCase):
    ...
    def test_parentless_import_shadowed_by_global(self):
        script_helper.assert_python_failure('-W', 'ignore', '-c', 'foo = 1; from . import foo')
    ...
    def test_import_from_unloaded_package(self):
        with uncache('package2', 'package2.submodule1', 'package2.submodule2'), DirsOnSysPath(os.path.join(os.path.dirname(__file__), 'data')):
...
class PycacheTests(unittest.TestCase):
    ...
    def test_import_pyc_path(self):
        ...
        self.assertTrue(os.path.exists(pyc_path), 'bytecode file {!r} for {!r} does not exist'.format(pyc_path, TESTFN))

    @unittest.skipUnless((os.name == 'posix'), 'test meaningful only on posix systems')
    @unittest.skipIf((hasattr(os, 'geteuid') and (os.geteuid() == 0)), 'due to varying filesystem permission semantics (issue #11956)')
    @skip_if_dont_write_bytecode
    ...
    def test_missing_source_legacy(self):
        ...
        m = __import__(TESTFN)
        try:
            self.assertEqual(m.__file__, os.path.join(os.curdir, os.path.relpath(pyc_file)))
        finally:
            os.remove(pyc_file)
    ...
    def test___cached___legacy_pyc(self):
        ...
        importlib.invalidate_caches()
        m = __import__(TESTFN)
        self.assertEqual(m.__cached__, os.path.join(os.curdir, os.path.relpath(pyc_file)))
...
class TestSymbolicallyLinkedPackage(unittest.TestCase):
    ...
    @unittest.skipUnless(((not hasattr(sys, 'getwindowsversion')) or (sys.getwindowsversion() >= (6, 0))), 'Windows Vista or later required')
...
class CircularImportTests(unittest.TestCase):
    ...
    def test_indirect(self):
        try:
            ...
            self.fail('relative import in module contributing to circular import failed')
    ...
    def test_circular_from_import(self):
        ...
        self.assertIn("cannot import name 'b' from partially initialized module 'test.test_import.data.circular_imports.from_cycle1' (most likely due to a circular import)", str(cm.exception))
