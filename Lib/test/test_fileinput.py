...
class Test_fileinput_input(BaseFileInputGlobalMethodsTest):
    ...
    def do_test_call_input(self):
        ...
        mode = object()
        openhook = object()
        result = fileinput.input(files=files, inplace=inplace, backup=backup, mode=mode, openhook=openhook)
...
class Test_hook_compressed(unittest.TestCase):
    ...
    def do_test_use_builtin_open(self, filename, mode):
        ...
        self.assertEqual(self.fake_open.invocation_count, 1)
        self.assertEqual(self.fake_open.last_invocation, ((filename, mode), {}))
