...
def _is_gui_available():
    ...
    if (sys.platform.startswith('win') and platform.win32_is_iot()):
        ...
        class USEROBJECTFLAGS(ctypes.Structure):
            _fields_ = [('fInherit', ctypes.wintypes.BOOL), ('fReserved', ctypes.wintypes.BOOL), ('dwFlags', ctypes.wintypes.DWORD)]
...
def _requires_unix_version(sysname, min_version):
    ...
    if (platform.system() == sysname):
        ...
        skip = False
    return unittest.skipIf(skip, f'{sysname} version {min_version_txt} or higher required, not {version_txt}')
...
MISSING_C_DOCSTRINGS = (check_impl_detail() and (sys.platform != 'win32') and (not sysconfig.get_config_var('WITH_DOC_STRINGS')))
...
def check__all__(test_case, module, name_of_module=None, extra=(), not_exported=()):
    ...
    for name in dir(module):
        ...
        if ((getattr(obj, '__module__', None) in name_of_module) or ((not hasattr(obj, '__module__')) and (not isinstance(obj, types.ModuleType)))):
            expected.add(name)
...
def suppress_msvcrt_asserts(verbose=False):
    try:
        ...
        return
    msvcrt.SetErrorMode((((msvcrt.SEM_FAILCRITICALERRORS | msvcrt.SEM_NOALIGNMENTFAULTEXCEPT) | msvcrt.SEM_NOGPFAULTERRORBOX) | msvcrt.SEM_NOOPENFILEERRORBOX))
...
def wait_process(pid, *, exitcode, timeout=None):
        ...
        while True:
            ...
            if (dt > SHORT_TIMEOUT):
                try:
                    ...
                    pass
                raise AssertionError(f'process {pid} is still running after {dt:.1f} seconds')
    ...
    if (exitcode2 != exitcode):
        raise AssertionError(f'process {pid} exited with code {exitcode2}, but exit code {exitcode} is expected')
    if (pid2 != pid):
