...
requires_setformat = unittest.skipUnless(hasattr(float, '__setformat__'), 'requires __setformat__')
...
class FormatFunctionsTestCase(unittest.TestCase):
    ...
    def setUp(self):
        self.save_formats = {'double': float.__getformat__('double'), 'float': float.__getformat__('float')}
...
class ReprTestCase(unittest.TestCase):
    ...
    def test_repr(self):
        with open(os.path.join(os.path.split(__file__)[0], 'floating_points.txt')) as floats_file:
