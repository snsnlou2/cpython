...
class ReferencesTestCase(TestBase):
    ...
    def check_proxy(self, o, proxy):
        ...
        del o.foo
        self.assertFalse(hasattr(proxy, 'foo'), 'proxy does not reflect attribute removal')
        proxy.foo = 1
        self.assertEqual(o.foo, 1, 'object does not reflect attribute addition via proxy')
        proxy.foo = 2
        self.assertEqual(o.foo, 2, 'object does not reflect attribute modification via proxy')
        del proxy.foo
        self.assertFalse(hasattr(o, 'foo'), 'object does not reflect attribute removal via proxy')
