...
class TestOptionalsSingleDashCombined(ParserTestCase):
    'Test an Optional with a single-dash option string'
    argument_signatures = [Sig('-x', action='store_true'), Sig('-yyy', action='store_const', const=42), Sig('-z')]
...
class TestOptionalsDoubleDashPrefixMatch(ParserTestCase):
    ...
    successes = [('', NS(badger=False, ba=None)), ('--ba X', NS(badger=False, ba='X')), ('--ba=X', NS(badger=False, ba='X')), ('--bad', NS(badger=True, ba=None)), ('--badg', NS(badger=True, ba=None)), ('--badge', NS(badger=True, ba=None)), ('--badger', NS(badger=True, ba=None))]

class TestOptionalsSingleDoubleDash(ParserTestCase):
    'Test an Optional with single- and double-dash option strings'
    argument_signatures = [Sig('-f', action='store_true'), Sig('--bar'), Sig('-baz', action='store_const', const=42)]
    failures = ['--bar', '-fbar', '-fbaz', '-bazf', '-b B', 'B']
    successes = [('', NS(f=False, bar=None, baz=None)), ('-f', NS(f=True, bar=None, baz=None)), ('--ba B', NS(f=False, bar='B', baz=None)), ('-f --bar B', NS(f=True, bar='B', baz=None)), ('-f -b', NS(f=True, bar=None, baz=42)), ('-ba -f', NS(f=True, bar=None, baz=42))]
...
class TestOptionalsAlternatePrefixCharsMultipleShortArgs(ParserTestCase):
    ...
    parser_signature = Sig(prefix_chars='+-', add_help=False)
    argument_signatures = [Sig('-x', action='store_true'), Sig('+y', action='store_true'), Sig('+z', action='store_true')]
...
class TestOptionalsNargsOptional(ParserTestCase):
    ...
    argument_signatures = [Sig('-w', nargs='?'), Sig('-x', nargs='?', const=42), Sig('-y', nargs='?', default='spam'), Sig('-z', nargs='?', type=int, const='42', default='84')]
    failures = ['2']
...
class TestOptionalsChoices(ParserTestCase):
    ...
    argument_signatures = [Sig('-f', choices='abc'), Sig('-g', type=int, choices=range(5))]
...
class TestBooleanOptionalAction(ParserTestCase):
    ...
    successes = [('', NS(foo=None)), ('--foo', NS(foo=True)), ('--no-foo', NS(foo=False)), ('--foo --no-foo', NS(foo=False)), ('--no-foo --foo', NS(foo=True))]
...
class TestBooleanOptionalActionRequired(ParserTestCase):
    ...
    argument_signatures = [Sig('--foo', required=True, action=argparse.BooleanOptionalAction)]
    failures = ['']
...
class TestOptionalsActionAppendConst(ParserTestCase):
    ...
    argument_signatures = [Sig('-b', action='append_const', const=Exception), Sig('-c', action='append', dest='b')]
    failures = ['a', '-c', 'a -c', '-bx', '-b x']
    successes = [('', NS(b=None)), ('-b', NS(b=[Exception])), ('-b -cx -b -cyz', NS(b=[Exception, 'x', Exception, 'yz']))]

class TestOptionalsActionAppendConstWithDefault(ParserTestCase):
    'Tests the append_const action for an Optional'
    argument_signatures = [Sig('-b', action='append_const', const=Exception, default=['X']), Sig('-c', action='append', dest='b')]
    failures = ['a', '-c', 'a -c', '-bx', '-b x']
    successes = [('', NS(b=['X'])), ('-b', NS(b=['X', Exception])), ('-b -cx -b -cyz', NS(b=['X', Exception, 'x', Exception, 'yz']))]
...
class TestOptionalsAllowLongAbbreviation(ParserTestCase):
    ...
    argument_signatures = [Sig('--foo'), Sig('--foobaz'), Sig('--fooble', action='store_true')]
    failures = ['--foob 5', '--foob']
    successes = [('', NS(foo=None, foobaz=None, fooble=False)), ('--foo 7', NS(foo='7', foobaz=None, fooble=False)), ('--fooba a', NS(foo=None, foobaz='a', fooble=False)), ('--foobl --foo g', NS(foo='g', foobaz=None, fooble=True))]

class TestOptionalsDisallowLongAbbreviation(ParserTestCase):
    'Do not allow abbreviations of long options at all'
    parser_signature = Sig(allow_abbrev=False)
    argument_signatures = [Sig('--foo'), Sig('--foodle', action='store_true'), Sig('--foonly')]
    failures = ['-foon 3', '--foon 3', '--food', '--food --foo 2']
    successes = [('', NS(foo=None, foodle=False, foonly=None)), ('--foo 3', NS(foo='3', foodle=False, foonly=None)), ('--foonly 7 --foodle --foo 2', NS(foo='2', foodle=True, foonly='7'))]
...
class TestDisallowLongAbbreviationAllowsShortGrouping(ParserTestCase):
    ...
    failures = ['-r', '-c -r']
    successes = [('', NS(r=None, c=None)), ('-ra', NS(r='a', c=None)), ('-rcc', NS(r='cc', c=None)), ('-cc', NS(r=None, c=2)), ('-cc -ra', NS(r='a', c=2)), ('-ccrcc', NS(r='cc', c=2))]
...
class TestPositionalsNargs2ZeroOrMore(ParserTestCase):
    ...
    failures = ['', '--foo', 'a']
    successes = [('a b', NS(foo=['a', 'b'], bar=[])), ('a b c', NS(foo=['a', 'b'], bar=['c']))]
...
class TestPositionalsNargs2Optional(ParserTestCase):
    ...
    successes = [('a b', NS(foo=['a', 'b'], bar=None)), ('a b c', NS(foo=['a', 'b'], bar='c'))]
...
class TestPositionalsNargsNoneZeroOrMore1(ParserTestCase):
    ...
    failures = ['', '--foo', 'a']
    successes = [('a b', NS(foo='a', bar=[], baz=['b'])), ('a b c', NS(foo='a', bar=['b'], baz=['c']))]

class TestPositionalsNargsNoneOneOrMore1(ParserTestCase):
    'Test three Positionals: no nargs, one or more nargs and 1 nargs'
    argument_signatures = [Sig('foo'), Sig('bar', nargs='+'), Sig('baz', nargs=1)]
    failures = ['', '--foo', 'a', 'b']
    successes = [('a b c', NS(foo='a', bar=['b'], baz=['c'])), ('a b c d', NS(foo='a', bar=['b', 'c'], baz=['d']))]

class TestPositionalsNargsNoneOptional1(ParserTestCase):
    'Test three Positionals: no nargs, optional narg and 1 nargs'
    argument_signatures = [Sig('foo'), Sig('bar', nargs='?', default=0.625), Sig('baz', nargs=1)]
    failures = ['', '--foo', 'a']
    successes = [('a b', NS(foo='a', bar=0.625, baz=['b'])), ('a b c', NS(foo='a', bar='b', baz=['c']))]
...
class TestPositionalsNargsOptionalOptional(ParserTestCase):
    ...
    failures = ['--foo', 'a b c']
    successes = [('', NS(foo=None, bar=42)), ('a', NS(foo='a', bar=42)), ('a b', NS(foo='a', bar='b'))]
...
class TestPositionalsNargsOptionalZeroOrMore(ParserTestCase):
    ...
    failures = ['--foo']
    successes = [('', NS(foo=None, bar=[])), ('a', NS(foo='a', bar=[])), ('a b', NS(foo='a', bar=['b'])), ('a b c', NS(foo='a', bar=['b', 'c']))]

class TestPositionalsNargsOptionalOneOrMore(ParserTestCase):
    'Test an Optional narg followed by one or more nargs'
    argument_signatures = [Sig('foo', nargs='?'), Sig('bar', nargs='+')]
    failures = ['', '--foo']
    successes = [('a', NS(foo=None, bar=['a'])), ('a b', NS(foo='a', bar=['b'])), ('a b c', NS(foo='a', bar=['b', 'c']))]
...
class TestPositionalsActionAppend(ParserTestCase):
    ...
    argument_signatures = [Sig('spam', action='append'), Sig('spam', action='append', nargs=2)]
    failures = ['', '--foo', 'a', 'a b', 'a b c d']
...
class TestOptionalsNumericAndPositionals(ParserTestCase):
    ...
    argument_signatures = [Sig('x', nargs='?'), Sig('-4', dest='y', action='store_true')]
    failures = ['-2', '-315']
...
class TestOptionalsAlmostNumericAndPositionals(ParserTestCase):
    ...
    argument_signatures = [Sig('x', nargs='?'), Sig('-k4', dest='y', action='store_true')]
    failures = ['-k3']
...
class TestPrefixCharacterOnlyArguments(ParserTestCase):
    ...
    argument_signatures = [Sig('-', dest='x', nargs='?', const='badger'), Sig('+', dest='y', type=int, default=42), Sig('-+-', dest='z', action='store_true')]
    failures = ['-y', '+ -']
...
class TestOptionLike(ParserTestCase):
    ...
    argument_signatures = [Sig('-x', type=float), Sig('-3', type=float, dest='y'), Sig('z', nargs='*')]
...
class TestDefaultSuppress(ParserTestCase):
    ...
    argument_signatures = [Sig('foo', nargs='?', default=argparse.SUPPRESS), Sig('bar', nargs='*', default=argparse.SUPPRESS), Sig('--baz', action='store_true', default=argparse.SUPPRESS)]
    failures = ['-x']
    successes = [('', NS()), ('a', NS(foo='a')), ('a b', NS(foo='a', bar=['b'])), ('--baz', NS(baz=True)), ('a --baz', NS(foo='a', baz=True)), ('--baz a b', NS(foo='a', bar=['b'], baz=True))]
...
class TestParserDefaultSuppress(ParserTestCase):
    ...
    argument_signatures = [Sig('foo', nargs='?'), Sig('bar', nargs='*'), Sig('--baz', action='store_true')]
    failures = ['-x']
    successes = [('', NS()), ('a', NS(foo='a')), ('a b', NS(foo='a', bar=['b'])), ('--baz', NS(baz=True)), ('a --baz', NS(foo='a', baz=True)), ('--baz a b', NS(foo='a', bar=['b'], baz=True))]
...
class TestParserDefault42(ParserTestCase):
    ...
    argument_signatures = [Sig('--version', action='version', version='1.0'), Sig('foo', nargs='?'), Sig('bar', nargs='*'), Sig('--baz', action='store_true')]
    failures = ['-x']
    successes = [('', NS(foo=42, bar=42, baz=42, version=42)), ('a', NS(foo='a', bar=42, baz=42, version=42)), ('a b', NS(foo='a', bar=['b'], baz=42, version=42)), ('--baz', NS(foo=42, bar=42, baz=True, version=42)), ('a --baz', NS(foo='a', bar=42, baz=True, version=42)), ('--baz a b', NS(foo='a', bar=['b'], baz=True, version=42))]
...
class TestFileTypeR(TempDirMixin, ParserTestCase):
    ...
    argument_signatures = [Sig('-x', type=argparse.FileType()), Sig('spam', type=argparse.FileType('r'))]
    failures = ['-x', '', 'non-existent-file.txt']
    successes = [('foo', NS(x=None, spam=RFile('foo'))), ('-x foo bar', NS(x=RFile('foo'), spam=RFile('bar'))), ('bar -x foo', NS(x=RFile('foo'), spam=RFile('bar'))), ('-x - -', NS(x=eq_stdin, spam=eq_stdin)), ('readonly', NS(x=None, spam=RFile('readonly')))]
...
class TestFileTypeDefaults(TempDirMixin, ParserTestCase):
    ...
    def setUp(self):
        ...
        file.write('good')
        file.close()
    argument_signatures = [Sig('-c', type=argparse.FileType('r'), default='no-file.txt')]
    failures = ['']
...
class TestFileTypeRB(TempDirMixin, ParserTestCase):
    ...
    def setUp(self):
        ...
        for file_name in ['foo', 'bar']:
            with open(os.path.join(self.temp_dir, file_name), 'w') as file:
                file.write(file_name)
    argument_signatures = [Sig('-x', type=argparse.FileType('rb')), Sig('spam', type=argparse.FileType('rb'))]
    failures = ['-x', '']
    successes = [('foo', NS(x=None, spam=RFile('foo'))), ('-x foo bar', NS(x=RFile('foo'), spam=RFile('bar'))), ('bar -x foo', NS(x=RFile('foo'), spam=RFile('bar'))), ('-x - -', NS(x=eq_stdin, spam=eq_stdin))]
...
class TestFileTypeW(TempDirMixin, ParserTestCase):
    ...
    def setUp(self):
        ...
        self.create_readonly_file('readonly')
    argument_signatures = [Sig('-x', type=argparse.FileType('w')), Sig('spam', type=argparse.FileType('w'))]
    failures = ['-x', '', 'readonly']
    successes = [('foo', NS(x=None, spam=WFile('foo'))), ('-x foo bar', NS(x=WFile('foo'), spam=WFile('bar'))), ('bar -x foo', NS(x=WFile('foo'), spam=WFile('bar'))), ('-x - -', NS(x=eq_stdout, spam=eq_stdout))]

class TestFileTypeWB(TempDirMixin, ParserTestCase):
    argument_signatures = [Sig('-x', type=argparse.FileType('wb')), Sig('spam', type=argparse.FileType('wb'))]
    failures = ['-x', '']
    successes = [('foo', NS(x=None, spam=WFile('foo'))), ('-x foo bar', NS(x=WFile('foo'), spam=WFile('bar'))), ('bar -x foo', NS(x=WFile('foo'), spam=WFile('bar'))), ('-x - -', NS(x=eq_stdout, spam=eq_stdout))]
...
class TestFileTypeMissingInitialization(TestCase):
    ...
    def test(self):
        ...
        self.assertEqual(('%r is a FileType class object, instance of it must be passed' % (argparse.FileType,)), str(cm.exception))
...
class TestTypeUserDefined(ParserTestCase):
    ...
    failures = []
    successes = [('a -x b', NS(x=MyType('b'), spam=MyType('a'))), ('-xf g', NS(x=MyType('f'), spam=MyType('g')))]
...
class TestTypeClassicClass(ParserTestCase):
    ...
    failures = []
    successes = [('a -x b', NS(x=C('b'), spam=C('a'))), ('-xf g', NS(x=C('f'), spam=C('g')))]
...
class TestActionUserDefined(ParserTestCase):
    ...
    argument_signatures = [Sig('-s', dest='spam', action=OptionalAction, type=float, default=0.25), Sig('badger', action=PositionalAction, type=int, nargs='?', default=2)]
    failures = []
    successes = [('-s0.125', NS(spam=0.125, badger=2)), ('42', NS(spam=0.25, badger=42)), ('-s 0.625 1', NS(spam=0.625, badger=1)), ('84 -s2', NS(spam=2.0, badger=84))]
...
class TestAddSubparsers(TestCase):
    ...
    def test_parser_command_help(self):
        self.assertEqual(self.command_help_parser.format_usage(), 'usage: PROG [-h] [--foo] bar {1,2,3} ...\n')
        self.assertEqual(self.command_help_parser.format_help(), textwrap.dedent('            usage: PROG [-h] [--foo] bar {1,2,3} ...\n\n            main description\n\n            positional arguments:\n              bar         bar help\n              {1,2,3}     command help\n                1         1 help\n                2         2 help\n                3         3 help\n\n            optional arguments:\n              -h, --help  show this help message and exit\n              --foo       foo help\n            '))
...
class TestMutuallyExclusiveManySuppressed(MEMixin, TestCase):
    ...
    successes = [('--spam', NS(spam=True, badger=True, bladder=None)), ('--badger', NS(spam=False, badger=False, bladder=None)), ('--bladder B', NS(spam=False, badger=True, bladder='B')), ('--spam --spam', NS(spam=True, badger=True, bladder=None))]
...
class TestMutuallyExclusiveOptionalAndPositional(MEMixin, TestCase):
    ...
    successes = [('--foo', NS(foo=True, spam=None, badger='X')), ('--spam S', NS(foo=False, spam='S', badger='X')), ('X', NS(foo=False, spam=None, badger=['X'])), ('X Y Z', NS(foo=False, spam=None, badger=['X', 'Y', 'Z']))]
    successes_when_not_required = [('', NS(foo=False, spam=None, badger='X'))]
    usage_when_not_required = '        usage: PROG [-h] [--foo | --spam SPAM | badger ...]\n        '
...
class TestMutuallyExclusiveOptionalsMixed(MEMixin, TestCase):
    ...
    usage_when_required = usage_when_not_required = '        usage: PROG [-h] [-x] [-a] [-b] [-y] [-c]\n        '
    help = '\n        optional arguments:\n          -h, --help  show this help message and exit\n          -x          x help\n          -a          a help\n          -b          b help\n          -y          y help\n          -c          c help\n        '
...
class TestMutuallyExclusiveInGroup(MEMixin, TestCase):
    ...
    def get_parser(self, required=None):
        parser = ErrorRaisingArgumentParser(prog='PROG')
        titled_group = parser.add_argument_group(title='Titled group', description='Group description')
...
class TestMutuallyExclusiveOptionalsAndPositionalsMixed(MEMixin, TestCase):
    ...
    usage_when_required = usage_when_not_required = '        usage: PROG [-h] [-y] [-b] [-c] x [a]\n        '
    help = '\n        positional arguments:\n          x           x help\n          a           a help\n\n        optional arguments:\n          -h, --help  show this help message and exit\n          -y          y help\n          -b          b help\n          -c          c help\n        '
...
class TestMutuallyExclusiveFirstSuppressedParent(MEPBase, TestMutuallyExclusiveFirstSuppressed):
    pass

class TestMutuallyExclusiveManySuppressedParent(MEPBase, TestMutuallyExclusiveManySuppressed):
    pass

class TestMutuallyExclusiveOptionalAndPositionalParent(MEPBase, TestMutuallyExclusiveOptionalAndPositional):
    pass

class TestMutuallyExclusiveOptionalsMixedParent(MEPBase, TestMutuallyExclusiveOptionalsMixed):
    pass

class TestMutuallyExclusiveOptionalsAndPositionalsMixedParent(MEPBase, TestMutuallyExclusiveOptionalsAndPositionalsMixed):
    pass
            ...
            def _get_parser(self, tester):
                ...
                if subparsers_sigs:
                    ...
                    for subparser_sig in subparsers_sigs:
                        subparsers.add_parser(*subparser_sig.args, **subparser_sig.kwargs)
                return parser
...
class TestHelpBiggerOptionals(HelpTestCase):
    ...
    argument_signatures = [Sig('-v', '--version', action='version', version='0.1'), Sig('-x', action='store_true', help='X HELP'), Sig('--y', help='Y HELP'), Sig('foo', help='FOO HELP'), Sig('bar', help='BAR HELP')]
...
class TestHelpBiggerOptionalGroups(HelpTestCase):
    ...
    parser_signature = Sig(prog='PROG', description='DESCRIPTION', epilog='EPILOG')
    argument_signatures = [Sig('-v', '--version', action='version', version='0.1'), Sig('-x', action='store_true', help='X HELP'), Sig('--y', help='Y HELP'), Sig('foo', help='FOO HELP'), Sig('bar', help='BAR HELP')]
    argument_group_signatures = [(Sig('GROUP TITLE', description='GROUP DESCRIPTION'), [Sig('baz', help='BAZ HELP'), Sig('-z', nargs='+', help='Z HELP')])]
...
class TestHelpBiggerPositionals(HelpTestCase):
    ...
    argument_signatures = [Sig('-x', action='store_true', help='X HELP'), Sig('--y', help='Y HELP'), Sig('ekiekiekifekang', help='EKI HELP'), Sig('bar', help='BAR HELP')]
    argument_group_signatures = []
    usage = '        usage: USAGE\n        '
...
class TestHelpReformatting(HelpTestCase):
    ...
    parser_signature = Sig(prog='PROG', description='   oddly    formatted\ndescription\n\nthat is so long that it should go onto multiple lines when wrapped')
    argument_signatures = [Sig('-x', metavar='XX', help='oddly\n    formatted -x help'), Sig('y', metavar='yyy', help='normal y help')]
    argument_group_signatures = [(Sig('title', description='\n    oddly formatted group\n\ndescription'), [Sig('-a', action='store_true', help=' oddly \nformatted    -a  help  \n    again, so long that it should be wrapped over multiple lines')])]
...
class TestHelpWrappingShortNames(HelpTestCase):
    ...
    argument_signatures = [Sig('-x', metavar='XX', help=('XHH HX' * 20)), Sig('y', metavar='yyy', help=('YH YH' * 20))]
    argument_group_signatures = [(Sig('ALPHAS'), [Sig('-a', action='store_true', help=('AHHH HHA' * 10))])]
    usage = '        usage: PROG [-h] [-x XX] [-a] yyy\n        '
...
class TestHelpWrappingLongNames(HelpTestCase):
    ...
    argument_signatures = [Sig('-v', '--version', action='version', version=('V V' * 30)), Sig('-x', metavar=('X' * 25), help=('XH XH' * 20)), Sig('y', metavar=('y' * 25), help=('YH YH' * 20))]
    argument_group_signatures = [(Sig('ALPHAS'), [Sig('-a', metavar=('A' * 25), help=('AH AH' * 20)), Sig('z', metavar=('z' * 25), help=('ZH ZH' * 20))])]
    usage = '        usage: USAGE\n        '
...
class TestHelpUsage(HelpTestCase):
    ...
    argument_signatures = [Sig('-w', nargs='+', help='w'), Sig('-x', nargs='*', help='x'), Sig('a', help='a'), Sig('b', help='b', nargs=2), Sig('c', help='c', nargs='?'), Sig('--foo', help='Whether to foo', action=argparse.BooleanOptionalAction), Sig('--bar', help='Whether to bar', default=True, action=argparse.BooleanOptionalAction), Sig('-f', '--foobar', '--barfoo', action=argparse.BooleanOptionalAction)]
    argument_group_signatures = [(Sig('group'), [Sig('-y', nargs='?', help='y'), Sig('-z', nargs=3, help='z'), Sig('d', help='d', nargs='*'), Sig('e', help='e', nargs='+')])]
...
class TestHelpOnlyUserGroups(HelpTestCase):
    ...
    argument_signatures = []
    argument_group_signatures = [(Sig('xxxx'), [Sig('-x', help='x'), Sig('a', help='a')]), (Sig('yyyy'), [Sig('b', help='b'), Sig('-y', help='y')])]
...
class TestHelpUsageLongProg(HelpTestCase):
    ...
    argument_signatures = [Sig('-w', metavar='W'), Sig('-x', metavar='X'), Sig('a'), Sig('b')]
    argument_group_signatures = []
...
class TestHelpUsageLongProgOptionsWrap(HelpTestCase):
    ...
    parser_signature = Sig(prog=('P' * 60))
    argument_signatures = [Sig('-w', metavar=('W' * 25)), Sig('-x', metavar=('X' * 25)), Sig('-y', metavar=('Y' * 25)), Sig('-z', metavar=('Z' * 25)), Sig('a'), Sig('b')]
    argument_group_signatures = []
...
class TestHelpUsageOptionalsWrap(HelpTestCase):
    ...
    parser_signature = Sig(prog='PROG')
    argument_signatures = [Sig('-w', metavar=('W' * 25)), Sig('-x', metavar=('X' * 25)), Sig('-y', metavar=('Y' * 25)), Sig('-z', metavar=('Z' * 25)), Sig('a'), Sig('b'), Sig('c')]
    argument_group_signatures = []
...
class TestHelpUsagePositionalsWrap(HelpTestCase):
    ...
    parser_signature = Sig(prog='PROG')
    argument_signatures = [Sig('-x'), Sig('-y'), Sig('-z'), Sig(('a' * 25)), Sig(('b' * 25)), Sig(('c' * 25))]
    argument_group_signatures = []
...
class TestHelpUsageOptionalsPositionalsWrap(HelpTestCase):
    ...
    parser_signature = Sig(prog='PROG')
    argument_signatures = [Sig('-x', metavar=('X' * 25)), Sig('-y', metavar=('Y' * 25)), Sig('-z', metavar=('Z' * 25)), Sig(('a' * 25)), Sig(('b' * 25)), Sig(('c' * 25))]
    argument_group_signatures = []
...
class TestHelpUsageOptionalsOnlyWrap(HelpTestCase):
    ...
    parser_signature = Sig(prog='PROG')
    argument_signatures = [Sig('-x', metavar=('X' * 25)), Sig('-y', metavar=('Y' * 25)), Sig('-z', metavar=('Z' * 25))]
    argument_group_signatures = []
    usage = '        usage: PROG [-h] [-x XXXXXXXXXXXXXXXXXXXXXXXXX] [-y YYYYYYYYYYYYYYYYYYYYYYYYY]\n                    [-z ZZZZZZZZZZZZZZZZZZZZZZZZZ]\n        '
    help = (usage + '\n        optional arguments:\n          -h, --help            show this help message and exit\n          -x XXXXXXXXXXXXXXXXXXXXXXXXX\n          -y YYYYYYYYYYYYYYYYYYYYYYYYY\n          -z ZZZZZZZZZZZZZZZZZZZZZZZZZ\n        ')
    version = ''
...
class TestHelpVariableExpansion(HelpTestCase):
    ...
    parser_signature = Sig(prog='PROG')
    argument_signatures = [Sig('-x', type=int, help='x %(prog)s %(default)s %(type)s %%'), Sig('-y', action='store_const', default=42, const='XXX', help='y %(prog)s %(default)s %(const)s'), Sig('--foo', choices='abc', help='foo %(prog)s %(default)s %(choices)s'), Sig('--bar', default='baz', choices=[1, 2], metavar='BBB', help='bar %(prog)s %(default)s %(dest)s'), Sig('spam', help='spam %(prog)s %(default)s'), Sig('badger', default=0.5, help='badger %(prog)s %(default)s')]
    argument_group_signatures = [(Sig('group'), [Sig('-a', help='a %(prog)s %(default)s'), Sig('-b', default=(- 1), help='b %(prog)s %(default)s')])]
...
class TestHelpSuppressOptional(HelpTestCase):
    ...
    parser_signature = Sig(prog='PROG', add_help=False)
    argument_signatures = [Sig('--foo', help=argparse.SUPPRESS), Sig('spam', help='spam help')]
    argument_group_signatures = []
...
class TestHelpSuppressPositional(HelpTestCase):
    ...
    parser_signature = Sig(prog='PROG')
    argument_signatures = [Sig('--foo', help='foo help'), Sig('spam', help=argparse.SUPPRESS)]
    argument_group_signatures = []
...
class TestHelpAlternatePrefixChars(HelpTestCase):
    ...
    argument_signatures = [Sig('^^foo', action='store_true', help='foo help'), Sig(';b', ';;bar', help='bar help')]
    argument_group_signatures = []
...
class TestHelpTupleMetavar(HelpTestCase):
    ...
    parser_signature = Sig(prog='PROG')
    argument_signatures = [Sig('-w', help='w', nargs='+', metavar=('W1', 'W2')), Sig('-x', help='x', nargs='*', metavar=('X1', 'X2')), Sig('-y', help='y', nargs=3, metavar=('Y1', 'Y2', 'Y3')), Sig('-z', help='z', nargs='?', metavar=('Z1',))]
    argument_group_signatures = []
...
class TestHelpRawText(HelpTestCase):
    ...
    parser_signature = Sig(prog='PROG', formatter_class=argparse.RawTextHelpFormatter, description='Keep the formatting\n    exactly as it is written\n\nhere\n')
    argument_signatures = [Sig('--foo', help='    foo help should also\nappear as given here'), Sig('spam', help='spam help')]
    argument_group_signatures = [(Sig('title', description='    This text\n  should be indented\n    exactly like it is here\n'), [Sig('--bar', help='bar help')])]
...
class TestHelpRawDescription(HelpTestCase):
    'Test the RawTextHelpFormatter'
    parser_signature = Sig(prog='PROG', formatter_class=argparse.RawDescriptionHelpFormatter, description='Keep the formatting\n    exactly as it is written\n\nhere\n')
    argument_signatures = [Sig('--foo', help='  foo help should not\n    retain this odd formatting'), Sig('spam', help='spam help')]
    argument_group_signatures = [(Sig('title', description='    This text\n  should be indented\n    exactly like it is here\n'), [Sig('--bar', help='bar help')])]
    usage = '        usage: PROG [-h] [--foo FOO] [--bar BAR] spam\n        '
...
class TestHelpArgumentDefaults(HelpTestCase):
    ...
    parser_signature = Sig(prog='PROG', formatter_class=argparse.ArgumentDefaultsHelpFormatter, description='description')
    argument_signatures = [Sig('--foo', help='foo help - oh and by the way, %(default)s'), Sig('--bar', action='store_true', help='bar help'), Sig('spam', help='spam help'), Sig('badger', nargs='?', default='wooden', help='badger help')]
    argument_group_signatures = [(Sig('title', description='description'), [Sig('--baz', type=int, default=42, help='baz help')])]
...
class TestHelpVersionActionSuppress(HelpTestCase):
    ...
    parser_signature = Sig(prog='PROG')
    argument_signatures = [Sig('-v', '--version', action='version', version='1.0', help=argparse.SUPPRESS), Sig('--foo', help='foo help'), Sig('spam', help='spam help')]
    argument_group_signatures = []
    usage = '        usage: PROG [-h] [--foo FOO] spam\n        '
    help = (usage + '\n        positional arguments:\n          spam        spam help\n\n        optional arguments:\n          -h, --help  show this help message and exit\n          --foo FOO   foo help\n        ')
...
class TestHelpMetavarTypeFormatter(HelpTestCase):
    ...
    def custom_type(string):
        return string
    parser_signature = Sig(prog='PROG', description='description', formatter_class=argparse.MetavarTypeHelpFormatter)
    argument_signatures = [Sig('a', type=int), Sig('-b', type=custom_type), Sig('-c', type=float, metavar='SOME FLOAT')]
    argument_group_signatures = []
...
class TestWrappingMetavar(TestCase):
    ...
    def setUp(self):
        super().setUp()
        self.parser = ErrorRaisingArgumentParser('this_is_spammy_prog_with_a_long_name_sorry_about_the_name')
