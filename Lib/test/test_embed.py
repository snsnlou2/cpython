...
class InitConfigTests(EmbeddingTestsMixin, unittest.TestCase):
    ...
    PRE_CONFIG_COMPAT = {'_config_init': API_COMPAT, 'allocator': PYMEM_ALLOCATOR_NOT_SET, 'parse_argv': 0, 'configure_locale': 1, 'coerce_c_locale': 0, 'coerce_c_locale_warn': 0, 'utf8_mode': 0}
    if MS_WINDOWS:
        PRE_CONFIG_COMPAT.update({'legacy_windows_fs_encoding': 0})
    PRE_CONFIG_PYTHON = dict(PRE_CONFIG_COMPAT, _config_init=API_PYTHON, parse_argv=1, coerce_c_locale=GET_DEFAULT_CONFIG, utf8_mode=GET_DEFAULT_CONFIG)
    PRE_CONFIG_ISOLATED = dict(PRE_CONFIG_COMPAT, _config_init=API_ISOLATED, configure_locale=0, isolated=1, use_environment=0, utf8_mode=0, dev_mode=0, coerce_c_locale=0)
    COPY_PRE_CONFIG = ['dev_mode', 'isolated', 'use_environment']
    CONFIG_COMPAT = {'_config_init': API_COMPAT, 'isolated': 0, 'use_environment': 1, 'dev_mode': 0, 'install_signal_handlers': 1, 'use_hash_seed': 0, 'hash_seed': 0, 'faulthandler': 0, 'tracemalloc': 0, 'import_time': 0, 'show_ref_count': 0, 'dump_refs': 0, 'malloc_stats': 0, 'filesystem_encoding': GET_DEFAULT_CONFIG, 'filesystem_errors': GET_DEFAULT_CONFIG, 'pycache_prefix': None, 'program_name': GET_DEFAULT_CONFIG, 'parse_argv': 0, 'argv': [''], 'orig_argv': [], 'xoptions': [], 'warnoptions': [], 'pythonpath_env': None, 'home': None, 'executable': GET_DEFAULT_CONFIG, 'base_executable': GET_DEFAULT_CONFIG, 'prefix': GET_DEFAULT_CONFIG, 'base_prefix': GET_DEFAULT_CONFIG, 'exec_prefix': GET_DEFAULT_CONFIG, 'base_exec_prefix': GET_DEFAULT_CONFIG, 'module_search_paths': GET_DEFAULT_CONFIG, 'platlibdir': sys.platlibdir, 'site_import': 1, 'bytes_warning': 0, 'inspect': 0, 'interactive': 0, 'optimization_level': 0, 'parser_debug': 0, 'write_bytecode': 1, 'verbose': 0, 'quiet': 0, 'user_site_directory': 1, 'configure_c_stdio': 0, 'buffered_stdio': 1, 'stdio_encoding': GET_DEFAULT_CONFIG, 'stdio_errors': GET_DEFAULT_CONFIG, 'skip_source_first_line': 0, 'run_command': None, 'run_module': None, 'run_filename': None, '_install_importlib': 1, 'check_hash_pycs_mode': 'default', 'pathconfig_warnings': 1, '_init_main': 1, '_isolated_interpreter': 0}
    if MS_WINDOWS:
        CONFIG_COMPAT.update({'legacy_windows_stdio': 0})
    CONFIG_PYTHON = dict(CONFIG_COMPAT, _config_init=API_PYTHON, configure_c_stdio=1, parse_argv=1)
    CONFIG_ISOLATED = dict(CONFIG_COMPAT, _config_init=API_ISOLATED, isolated=1, use_environment=0, user_site_directory=0, dev_mode=0, install_signal_handlers=0, use_hash_seed=0, faulthandler=0, tracemalloc=0, pathconfig_warnings=0)
    if MS_WINDOWS:
        CONFIG_ISOLATED['legacy_windows_stdio'] = 0
    DEFAULT_GLOBAL_CONFIG = {'Py_HasFileSystemDefaultEncoding': 0, 'Py_HashRandomizationFlag': 1, '_Py_HasFileSystemDefaultEncodeErrors': 0}
    COPY_GLOBAL_PRE_CONFIG = [('Py_UTF8Mode', 'utf8_mode')]
    COPY_GLOBAL_CONFIG = [('Py_BytesWarningFlag', 'bytes_warning'), ('Py_DebugFlag', 'parser_debug'), ('Py_DontWriteBytecodeFlag', 'write_bytecode', True), ('Py_FileSystemDefaultEncodeErrors', 'filesystem_errors'), ('Py_FileSystemDefaultEncoding', 'filesystem_encoding'), ('Py_FrozenFlag', 'pathconfig_warnings', True), ('Py_IgnoreEnvironmentFlag', 'use_environment', True), ('Py_InspectFlag', 'inspect'), ('Py_InteractiveFlag', 'interactive'), ('Py_IsolatedFlag', 'isolated'), ('Py_NoSiteFlag', 'site_import', True), ('Py_NoUserSiteDirectory', 'user_site_directory', True), ('Py_OptimizeFlag', 'optimization_level'), ('Py_QuietFlag', 'quiet'), ('Py_UnbufferedStdioFlag', 'buffered_stdio', True), ('Py_VerboseFlag', 'verbose')]
    if MS_WINDOWS:
        COPY_GLOBAL_PRE_CONFIG.extend((('Py_LegacyWindowsFSEncodingFlag', 'legacy_windows_fs_encoding'),))
        COPY_GLOBAL_CONFIG.extend((('Py_LegacyWindowsStdioFlag', 'legacy_windows_stdio'),))
    if MS_WINDOWS:
        PATH_CONFIG = {'isolated': (- 1), 'site_import': (- 1), 'python3_dll': GET_DEFAULT_CONFIG}
    else:
        PATH_CONFIG = {}
    ...
    def _get_expected_config_impl(self):
        ...
        proc = subprocess.run(args, env=env, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        if proc.returncode:
            raise Exception(f'failed to get the default config: stdout={proc.stdout!r} stderr={proc.stderr!r}')
    ...
    def get_expected_config(self, expected_preconfig, expected, expected_pathconfig, env, api, modify_path_cb=None):
        cls = self.__class__
        ...
        if ((not expected_preconfig['configure_locale']) or (api == API_COMPAT)):
            for key in ('filesystem_encoding', 'filesystem_errors', 'stdio_encoding', 'stdio_errors'):
    ...
    def check_all_configs(self, testname, expected_config=None, expected_preconfig=None, expected_pathconfig=None, modify_path_cb=None, stderr=None, *, api, preconfig_api=None, env=None, ignore_stderr=False, cwd=None):
        ...
        self.get_expected_config(expected_preconfig, expected_config, expected_pathconfig, env, api, modify_path_cb)
    ...
    def test_init_compat_env(self):
        ...
        self.check_all_configs('test_init_compat_env', config, preconfig, api=API_COMPAT)
    ...
    def test_init_env_dev_mode_alloc(self):
        ...
        config = dict(dev_mode=1, faulthandler=1, warnoptions=['default'])
        self.check_all_configs('test_init_env_dev_mode_alloc', config, preconfig, api=API_COMPAT)
    ...
    def test_init_main(self):
        code = 'import _testinternalcapi, json; print(json.dumps(_testinternalcapi.get_configs()))'
        config = {'argv': ['-c', 'arg2'], 'orig_argv': ['python3', '-c', code, 'arg2'], 'program_name': './python3', 'run_command': (code + '\n'), 'parse_argv': 1, '_init_main': 0}
        self.check_all_configs('test_init_main', config, api=API_PYTHON, stderr='Run Python code before _Py_InitializeMain')

    def test_init_parse_argv(self):
        config = {'parse_argv': 1, 'argv': ['-c', 'arg1', '-v', 'arg3'], 'orig_argv': ['./argv0', '-E', '-c', 'pass', 'arg1', '-v', 'arg3'], 'program_name': './argv0', 'run_command': 'pass\n', 'use_environment': 0}
    ...
    def default_program_name(self, config):
        ...
        config.update({'program_name': program_name, 'base_executable': executable, 'executable': executable})
    ...
    def module_search_paths(self, prefix=None, exec_prefix=None):
        ...
        if MS_WINDOWS:
            ...
            return [os.path.join(prefix, sys.platlibdir, f'python{ver.major}{ver.minor}.zip'), os.path.join(prefix, sys.platlibdir, f'python{ver.major}.{ver.minor}'), os.path.join(exec_prefix, sys.platlibdir, f'python{ver.major}.{ver.minor}', 'lib-dynload')]
...
class AuditingTests(EmbeddingTestsMixin, unittest.TestCase):
    ...
    def test_audit_run_command(self):
        self.run_embedded_interpreter('test_audit_run_command', timeout=support.SHORT_TIMEOUT, returncode=1)
    ...
    def test_audit_run_stdin(self):
        self.run_embedded_interpreter('test_audit_run_stdin', timeout=support.SHORT_TIMEOUT, returncode=1)
