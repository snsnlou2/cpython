...
class PublicAPITests(BaseTest):
    ...
    def test_module_all_attribute(self):
        ...
        target_api = ['warn', 'warn_explicit', 'showwarning', 'formatwarning', 'filterwarnings', 'simplefilter', 'resetwarnings', 'catch_warnings']
...
class FilterTests(BaseTest):
    ...
    def test_module_globals(self):
        with original_warnings.catch_warnings(record=True, module=self.module) as w:
            ...
            self.module.warn_explicit('msg', UserWarning, 'filename', 42, module_globals=None)
            self.assertEqual(len(w), 1)
            with self.assertRaises(TypeError):
                self.module.warn_explicit('msg', UserWarning, 'filename', 42, module_globals=True)
            self.assertEqual(len(w), 1)
            self.module.warn_explicit('msg', UserWarning, 'filename', 42, module_globals={})
            self.assertEqual(len(w), 2)
    ...
    def test_filterwarnings_duplicate_filters(self):
        with original_warnings.catch_warnings(module=self.module):
            ...
            self.module.filterwarnings('ignore', category=UserWarning)
            self.module.filterwarnings('error', category=UserWarning)
            self.assertEqual(len(self.module.filters), 2, 'filterwarnings inserted duplicate filter')
            self.assertEqual(self.module.filters[0][0], 'error', 'filterwarnings did not promote filter to the beginning of list')
    ...
    def test_simplefilter_duplicate_filters(self):
        with original_warnings.catch_warnings(module=self.module):
            ...
            self.module.simplefilter('error', category=UserWarning)
            self.assertEqual(len(self.module.filters), 2, 'simplefilter inserted duplicate filter')
            self.assertEqual(self.module.filters[0][0], 'error', 'simplefilter did not promote filter to the beginning of list')
...
class _WarningsTests(BaseTest, unittest.TestCase):
    ...
    def test_stderr_none(self):
        (rc, stdout, stderr) = assert_python_ok('-c', "import sys; sys.stderr = None; import warnings; warnings.simplefilter('always'); warnings.warn('Warning!')")
    ...
    def test_issue31285(self):
        ...
        with original_warnings.catch_warnings(module=wmod):
            ...
            with support.captured_stderr() as stderr:
                wmod.warn_explicit('foo', UserWarning, 'bar', 1, module_globals={'__loader__': get_bad_loader(42), '__name__': 'foobar'})
            self.assertIn('UserWarning: foo', stderr.getvalue())
            show = wmod._showwarnmsg
            try:
                del wmod._showwarnmsg
                with support.captured_stderr() as stderr:
                    wmod.warn_explicit('eggs', UserWarning, 'bar', 1, module_globals={'__loader__': get_bad_loader([42]), '__name__': 'foobar'})
    ...
    def test_issue31416(self):
        ...
        with original_warnings.catch_warnings(module=wmod):
            ...
            wmod.filters = []
            with support.swap_attr(wmod, 'defaultaction', None), self.assertRaises(TypeError):
                wmod.warn_explicit('foo', Warning, 'bar', 1)
    ...
    def test_issue31566(self):
        with original_warnings.catch_warnings(module=self.module):
            self.module.filterwarnings('error', category=UserWarning)
            with support.swap_item(globals(), '__name__', b'foo'), support.swap_item(globals(), '__file__', None):
...
class WarningsDisplayTests(BaseTest):
    ...
    def test_formatwarning_override(self):
        ...
        category = Warning
        file_object = StringIO()
        expected = (f'm={message}:c={category}:f={file_name}:l={line_num}' + f':t={file_line}')
        with support.swap_attr(self.module, 'formatwarning', myformatwarning):
            self.module.showwarning(message, category, file_name, line_num, file_object, file_line)
...
class EnvironmentVariableTests(BaseTest):
    ...
    def test_single_warning(self):
        (rc, stdout, stderr) = assert_python_ok('-c', 'import sys; sys.stdout.write(str(sys.warnoptions))', PYTHONWARNINGS='ignore::DeprecationWarning', PYTHONDEVMODE='')
        self.assertEqual(stdout, b"['ignore::DeprecationWarning']")

    def test_comma_separated_warnings(self):
        (rc, stdout, stderr) = assert_python_ok('-c', 'import sys; sys.stdout.write(str(sys.warnoptions))', PYTHONWARNINGS='ignore::DeprecationWarning,ignore::UnicodeWarning', PYTHONDEVMODE='')
        self.assertEqual(stdout, b"['ignore::DeprecationWarning', 'ignore::UnicodeWarning']")

    def test_envvar_and_command_line(self):
        (rc, stdout, stderr) = assert_python_ok('-Wignore::UnicodeWarning', '-c', 'import sys; sys.stdout.write(str(sys.warnoptions))', PYTHONWARNINGS='ignore::DeprecationWarning', PYTHONDEVMODE='')
        self.assertEqual(stdout, b"['ignore::DeprecationWarning', 'ignore::UnicodeWarning']")

    def test_conflicting_envvar_and_command_line(self):
        (rc, stdout, stderr) = assert_python_failure('-Werror::DeprecationWarning', '-c', "import sys, warnings; sys.stdout.write(str(sys.warnoptions)); warnings.warn('Message', DeprecationWarning)", PYTHONWARNINGS='default::DeprecationWarning', PYTHONDEVMODE='')
        self.assertEqual(stdout, b"['default::DeprecationWarning', 'error::DeprecationWarning']")
        self.assertEqual(stderr.splitlines(), [b'Traceback (most recent call last):', b'  File "<string>", line 1, in <module>', b'DeprecationWarning: Message'])
    ...
    def test_default_filter_configuration(self):
        ...
        if Py_DEBUG:
            ...
            expected_default_filters = [('default', None, DeprecationWarning, main_module_filter, 0), ('ignore', None, DeprecationWarning, None, 0), ('ignore', None, PendingDeprecationWarning, None, 0), ('ignore', None, ImportWarning, None, 0), ('ignore', None, ResourceWarning, None, 0)]
    ...
    @unittest.skipUnless((sys.getfilesystemencoding() != 'ascii'), 'requires non-ascii filesystemencoding')
    def test_nonascii(self):
        PYTHONWARNINGS = ('ignore:DeprecationWarning' + os_helper.FS_NONASCII)
        (rc, stdout, stderr) = assert_python_ok('-c', 'import sys; sys.stdout.write(str(sys.warnoptions))', PYTHONIOENCODING='utf-8', PYTHONWARNINGS=PYTHONWARNINGS, PYTHONDEVMODE='')
