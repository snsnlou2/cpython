...
def to_tuple(t):
    ...
    return tuple(result)
exec_tests = ['None', "'module docstring'", 'def f(): pass', "def f(): 'function docstring'", 'def f(a): pass', 'def f(a=0): pass', 'def f(*args): pass', 'def f(**kwargs): pass', "def f(a, b=1, c=None, d=[], e={}, *args, f=42, **kwargs): 'doc for f()'", 'class C:pass', "class C: 'docstring for class C'", 'class C(object): pass', 'def f():return 1', 'del v', 'v = 1', 'a,b = c', '(a,b) = c', '[a,b] = c', 'v += 1', 'for v in v:pass', 'while v:pass', 'if v:pass', 'if a:\n  pass\nelif b:\n  pass', 'if a:\n  pass\nelif b:\n  pass\nelse:\n  pass', 'with x as y: pass', 'with x as y, z as q: pass', "raise Exception('string')", 'try:\n  pass\nexcept Exception:\n  pass', 'try:\n  pass\nfinally:\n  pass', 'assert v', 'import sys', 'from sys import v', 'global v', '1', 'pass', 'for v in v:break', 'for v in v:continue', 'for a,b in c: pass', 'for (a,b) in c: pass', 'for [a,b] in c: pass', '(\n    (\n    Aa\n    ,\n       Bb\n    )\n    for\n    Aa\n    ,\n    Bb in Cc\n    )', '{a : b for w in x for m in p if g}', '{a : b for v,w in x}', '{r for l in x if g}', '{r for l,m in x}', "async def f():\n 'async function'\n await something()", 'async def f():\n async for e in i: 1\n else: 2', 'async def f():\n async with a as b: 1', '{**{1:2}, 2:3}', '{*{1, 2}, 3}', 'async def f():\n [i async for b in c]', '@deco1\n@deco2()\n@deco3(1)\ndef f(): pass', '@deco1\n@deco2()\n@deco3(1)\nasync def f(): pass', '@deco1\n@deco2()\n@deco3(1)\nclass C: pass', '@deco(a for a in b)\ndef f(): pass', '@a.b.c\ndef f(): pass', '(a := 1)', 'def f(a, /,): pass', 'def f(a, /, c, d, e): pass', 'def f(a, /, c, *, d, e): pass', 'def f(a, /, c, *, d, e, **kwargs): pass', 'def f(a=1, /,): pass', 'def f(a=1, /, b=2, c=4): pass', 'def f(a=1, /, b=2, *, c=4): pass', 'def f(a=1, /, b=2, *, c): pass', 'def f(a=1, /, b=2, *, c=4, **kwargs): pass', 'def f(a=1, /, b=2, *, c, **kwargs): pass']
single_tests = ['1+2']
eval_tests = ['None', 'a and b', 'a + b', 'not v', 'lambda:None', '{ 1:2 }', '{}', '{None,}', '{\n      1\n        :\n          2\n     }', '[a for b in c if d]', '(a for b in c if d)', '[(a,b) for a,b in c]', '[(a,b) for (a,b) in c]', '[(a,b) for [a,b] in c]', '{(a,b) for a,b in c}', '{(a,b) for (a,b) in c}', '{(a,b) for [a,b] in c}', '((a,b) for a,b in c)', '((a,b) for (a,b) in c)', '((a,b) for [a,b] in c)', '1 < 2 < 3', 'f(1,2,c=3,*d,**e)', 'f(*[0, 1])', 'f(a for a in b)', '10', "'string'", 'a.b', 'a[b:c]', 'v', '[1,2,3]', '[]', '1,2,3', '(1,2,3)', '()', 'a.b.c.d(a.b[1:2])']
...
class ASTHelpers_Test(unittest.TestCase):
    ...
    def test_level_as_none(self):
        body = [ast.ImportFrom(module='time', names=[ast.alias(name='sleep')], level=None, lineno=0, col_offset=0)]
        mod = ast.Module(body, [])
...
class ASTValidatorTests(unittest.TestCase):
    ...
    def _check_arguments(self, fac, check):
        ...
        def arguments(args=None, posonlyargs=None, vararg=None, kwonlyargs=None, kwarg=None, defaults=None, kw_defaults=None):
            ...
            if (kw_defaults is None):
                kw_defaults = []
            args = ast.arguments(args, posonlyargs, vararg, kwonlyargs, kw_defaults, kwarg, defaults)
            return fac(args)
        ...
        check(arguments(kw_defaults=[ast.Num(4)]), 'length of kwonlyargs is not the same as kw_defaults')
        args = [ast.arg('x', ast.Name('x', ast.Load()))]
        check(arguments(args=args, defaults=[ast.Name('x', ast.Store())]), 'must have Load context')
        args = [ast.arg('a', ast.Name('x', ast.Load())), ast.arg('b', ast.Name('y', ast.Load()))]
        check(arguments(kwonlyargs=args, kw_defaults=[None, ast.Name('x', ast.Store())]), 'must have Load context')
    ...
    def _check_comprehension(self, fac):
        ...
        g = ast.comprehension(ast.Name('x', ast.Load()), ast.Name('x', ast.Load()), [], 0)
        self.expr(fac([g]), 'must have Store context')
        g = ast.comprehension(ast.Name('x', ast.Store()), ast.Name('x', ast.Store()), [], 0)
    ...
    def _simple_comp(self, fac):
        g = ast.comprehension(ast.Name('x', ast.Store()), ast.Name('x', ast.Load()), [], 0)
    ...
    def _sequence(self, fac):
        ...
        self.expr(fac([ast.Name('x', ast.Store())], ast.Load()), 'must have Load context')
