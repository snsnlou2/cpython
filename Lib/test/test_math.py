...
class MathTests(unittest.TestCase):
    ...
    @unittest.skipIf(((sys.platform == 'win32') and (platform.machine() in ('ARM', 'ARM64'))), 'Windows UCRT is off by 2 ULP this test requires accuracy within 1 ULP')
    def testCosh(self):
    ...
    def testFactorialNonIntegers(self):
        ...
        self.assertRaises(TypeError, math.factorial, (- 1e+100))
    ...
    def testHypot(self):
        ...
        for i in range((len(args) + 1)):
            self.assertAlmostEqual(hypot(*args[:i]), math.sqrt(sum(((s ** 2) for s in args[:i]))))
        ...
        for n in range(32):
            self.assertTrue(math.isclose(hypot(*([fourthmax] * n)), (fourthmax * math.sqrt(n))))
        for exp in range(32):
    ...
    def testDist(self):
        ...
        for i in range(9):
            for j in range(5):
                ...
                self.assertAlmostEqual(dist(p, q), sqrt(sum((((px - qx) ** 2.0) for (px, qx) in zip(p, q)))))
        self.assertEqual(dist([1.0, 2.0, 3.0], [4.0, 2.0, (- 1.0)]), 5.0)
        ...
        self.assertEqual(dist((True, True, False, True, False), (True, False, True, True, False)), sqrt(2.0))
        self.assertEqual(dist((13.25, 12.5, (- 3.25)), (13.25, 12.5, (- 3.25))), 0.0)
        self.assertEqual(dist((), ()), 0.0)
        self.assertEqual(1.0, math.copysign(1.0, dist(((- 0.0),), (0.0,))))
        self.assertEqual(1.0, math.copysign(1.0, dist((0.0,), ((- 0.0),))))
        self.assertEqual(dist((1.5, 1.5, 0.5), (0, 0, 0)), dist((1.5, 0.5, 1.5), (0, 0, 0)))
    ...
    def testTanh(self):
        ...
        self.ftest('tanh(1)+tanh(-1)', (math.tanh(1) + math.tanh((- 1))), 0, abs_tol=math.ulp(1))
