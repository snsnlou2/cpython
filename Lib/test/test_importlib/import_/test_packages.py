...
class ParentModuleTests():
    ...
    def test_raising_parent_after_importing_child(self):
        ...
        def __init__():
            import pkg.module
            (1 / 0)
        mock = util.mock_spec('pkg.__init__', 'pkg.module', module_code={'pkg': __init__})
        with mock:
            with util.import_state(meta_path=[mock]):
    ...
    def test_raising_parent_after_double_relative_importing_child(self):
        ...
        def __init__():
            ...
            (1 / 0)
        mock = util.mock_spec('pkg.__init__', 'pkg.subpkg.__init__', 'pkg.subpkg.module', module_code={'pkg.subpkg': __init__})
        with mock:
            with util.import_state(meta_path=[mock]):
                with self.assertRaises((ZeroDivisionError, ImportError)):
...
(Frozen_ParentTests, Source_ParentTests) = util.test_both(ParentModuleTests, __import__=util.__import__)
