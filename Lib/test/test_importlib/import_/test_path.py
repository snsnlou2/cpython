...
class FinderTests():
    ...
    def test_empty_path_hooks(self):
        ...
        with util.import_state(path_importer_cache={}, path_hooks=[], path=[path_entry]):
    ...
    def test_None_on_sys_path(self):
        ...
        new_path_hooks = [zipimport.zipimporter, self.machinery.FileFinder.path_hook(*self.importlib._bootstrap_external._get_supported_file_loaders())]
        missing = object()
...
(Frozen_FindModuleTests, Source_FindModuleTests) = util.test_both(FindModuleTests, importlib=importlib, machinery=machinery)
...
(Frozen_FindSpecTests, Source_FindSpecTests) = util.test_both(FindSpecTests, importlib=importlib, machinery=machinery)
    ...
    def test_finder_with_failing_find_spec(self):
        ...
        class Finder():
            ...
            def find_module(fullname):
                return None
        with util.import_state(path=([Finder.path_location] + sys.path[:]), path_hooks=[Finder]):
...
(Frozen_PEFTests, Source_PEFTests) = util.test_both(PathEntryFinderTests, machinery=machinery)
