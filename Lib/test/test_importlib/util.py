...
def import_importlib(module_name):
    ...
    source = import_helper.import_fresh_module(module_name, fresh=fresh, blocked=('_frozen_importlib', '_frozen_importlib_external'))
...
__import__ = {'Frozen': staticmethod(builtins.__import__), 'Source': staticmethod(source_importlib.__import__)}
...
class mock_spec(_ImporterMock):
    ...
    def find_spec(self, fullname, path=None, parent=None):
        try:
            ...
            return None
        spec = util.spec_from_file_location(fullname, module.__file__, loader=self, submodule_search_locations=getattr(module, '__path__', None))
        return spec
...
class CASEOKTestBase():
    ...
    def caseok_env_changed(self, *, should_exist):
        ...
        if (any(((x in self.importlib._bootstrap_external._os.environ) for x in possibilities)) != should_exist):
...
def create_package(file, path, is_package=True, contents=()):
    ...
    loader = Reader()
    spec = machinery.ModuleSpec(name, loader, origin='does-not-exist', is_package=is_package)
