...
(Frozen_ImportModuleTests, Source_ImportModuleTests) = test_util.test_both(ImportModuleTests, init=init)
...
(Frozen_FindLoaderPEP451Tests, Source_FindLoaderPEP451Tests) = test_util.test_both(FindLoaderPEP451Tests, init=init)
...
class FindLoaderPEP302Tests(FindLoaderTests):
    ...
    class FakeMetaFinder():
        ...
        def find_module(name, path=None):
            return (name, path)
(Frozen_FindLoaderPEP302Tests, Source_FindLoaderPEP302Tests) = test_util.test_both(FindLoaderPEP302Tests, init=init)
        ...
        with os_helper.temp_cwd(None) as cwd:
            with test_util.uncache('spam'):
                with import_helper.DirsOnSysPath(cwd):
                    ...
                    expected = {'__name__': name, '__package__': '', '__file__': path, '__cached__': cached, '__doc__': None}
                    ...
                    expected = {'__name__': name, '__package__': name, '__file__': init_path, '__cached__': cached, '__path__': [os.path.dirname(init_path)], '__doc__': None}
                    os.mkdir(name)
        ...
        with os_helper.temp_cwd(None) as cwd:
            with test_util.uncache('spam'):
                with import_helper.DirsOnSysPath(cwd):
                    ...
                    expected = {'__name__': name, '__package__': name, '__doc__': None, '__file__': None}
                    os.mkdir(name)
                    ...
                    init_path = os.path.join(cwd, name, '__init__.py')
                    cached = self.util.cache_from_source(init_path)
                    expected = {'__name__': name, '__package__': name, '__file__': init_path, '__cached__': cached, '__path__': [os.path.dirname(init_path)], '__doc__': None, 'eggs': None}
    ...
    def test_module_missing_spec(self):
        ...
        with test_util.uncache(name):
            ...
            with self.assertRaises(ModuleNotFoundError):
                self.init.reload(module)
(Frozen_ReloadTests, Source_ReloadTests) = test_util.test_both(ReloadTests, init=init, util=util)
...
class InvalidateCacheTests():
    ...
    def test_method_lacking(self):
        ...
        self.init.invalidate_caches()
(Frozen_InvalidateCacheTests, Source_InvalidateCacheTests) = test_util.test_both(InvalidateCacheTests, init=init)
        ...
        for (name, module) in sys.modules.items():
            if isinstance(module, types.ModuleType):
                with self.subTest(name=name):
                    self.assertTrue(hasattr(module, '__loader__'), '{!r} lacks a __loader__ attribute'.format(name))
            ...
            if isinstance(module, types.ModuleType):
                with self.subTest(name=name):
                    ...
                    if self.machinery.BuiltinImporter.find_module(name):
                        ...
                        self.assertIsNot(module.__spec__, None)
(Frozen_StartupTests, Source_StartupTests) = test_util.test_both(StartupTests, machinery=machinery)
