...
class TestMkstempInner(TestBadTempdir, BaseTestCase):
    ...
    def make_temp(self):
        return tempfile._mkstemp_inner(tempfile.gettempdir(), tempfile.gettempprefix(), '', tempfile._bin_openflags, str)
...
class TestSpooledTemporaryFile(BaseTestCase):
    ...
    def test_del_on_close(self):
        ...
        try:
            ...
            filename = f.name
            f.close()
            self.assertFalse((isinstance(filename, str) and os.path.exists(filename)), ('SpooledTemporaryFile %s exists after close' % filename))
        finally:
            os.rmdir(dir)
