...
class PyUnpicklerTests(AbstractUnpickleTests):
    ...
    truncated_errors = (pickle.UnpicklingError, EOFError, AttributeError, ValueError, struct.error, IndexError, ImportError)
...
class InMemoryPickleTests(AbstractPickleTests, AbstractUnpickleTests, BigmemPickleTests):
...
class PyPersPicklerTests(AbstractPersistentPicklerTests, PersistentPicklerUnpicklerMixin):
    pickler = pickle._Pickler
    unpickler = pickle._Unpickler

class PyIdPersPicklerTests(AbstractIdentityPersistentPicklerTests, PersistentPicklerUnpicklerMixin):
    pickler = pickle._Pickler
    unpickler = pickle._Unpickler
...
ALT_IMPORT_MAPPING = {('_elementtree', 'xml.etree.ElementTree'), ('cPickle', 'pickle'), ('StringIO', 'io'), ('cStringIO', 'io')}
ALT_NAME_MAPPING = {('__builtin__', 'basestring', 'builtins', 'str'), ('exceptions', 'StandardError', 'builtins', 'Exception'), ('UserDict', 'UserDict', 'collections', 'UserDict'), ('socket', '_socketobject', 'socket', 'SocketType')}
        ...
        for (module2, module3) in IMPORT_MAPPING.items():
            with self.subTest((module2, module3)):
                try:
                    ...
                    if support.verbose:
                        print(exc)
                if (((module2, module3) not in ALT_IMPORT_MAPPING) and (REVERSE_IMPORT_MAPPING.get(module3, None) != module2)):
                    for ((m3, n3), (m2, n2)) in REVERSE_NAME_MAPPING.items():
                        if ((module3, module2) == (m3, m2)):
                            break
                    else:
                        self.fail(('No reverse mapping from %r to %r' % (module3, module2)))
    ...
    def test_exceptions(self):
        ...
        for (name, exc) in get_exceptions(builtins):
            with self.subTest(name):
                if (exc in (BlockingIOError, ResourceWarning, StopAsyncIteration, RecursionError)):
                    continue
...
def test_main():
    tests = [PyPickleTests, PyUnpicklerTests, PyPicklerTests, PyPersPicklerTests, PyIdPersPicklerTests, PyDispatchTableTests, PyChainDispatchTableTests, CompatPickleTests, PyPicklerHookTests]
    if has_c_implementation:
        tests.extend([CPickleTests, CUnpicklerTests, CPicklerTests, CPersPicklerTests, CIdPersPicklerTests, CDumpPickle_LoadPickle, DumpPickle_CLoadPickle, PyPicklerUnpicklerObjectTests, CPicklerUnpicklerObjectTests, CDispatchTableTests, CChainDispatchTableTests, CPicklerHookTests, InMemoryPickleTests, SizeofTests])
