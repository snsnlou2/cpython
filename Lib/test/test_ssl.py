...
for (proto, ver) in (('PROTOCOL_SSLv23', 'SSLv3'), ('PROTOCOL_TLSv1', 'TLSv1'), ('PROTOCOL_TLSv1_1', 'TLSv1_1')):
    try:
...
def requires_tls_version(version):
    ...
    def decorator(func):
        ...
        return wrapper
    return decorator
requires_minimum_version = unittest.skipUnless(hasattr(ssl.SSLContext, 'minimum_version'), 'required OpenSSL >= 1.1.0g')
...
def test_wrap_socket(sock, ssl_version=ssl.PROTOCOL_TLS, *, cert_reqs=ssl.CERT_NONE, ca_certs=None, ciphers=None, certfile=None, keyfile=None, **kwargs):
...
class BasicSocketTests(unittest.TestCase):
    ...
    def test_openssl_version(self):
        ...
        if IS_LIBRESSL:
            ...
            self.assertTrue(s.startswith('OpenSSL {:d}.{:d}.{:d}'.format(major, minor, fix)), (s, t, hex(n)))
    ...
    def test_server_side(self):
        ctx = ssl.SSLContext(ssl.PROTOCOL_TLS_SERVER)
        with socket.socket() as sock:
            self.assertRaises(ValueError, ctx.wrap_socket, sock, True, server_hostname='some.hostname')
    ...
    @unittest.skipUnless(('tls-unique' in ssl.CHANNEL_BINDING_TYPES), "'tls-unique' channel binding not available")
...
class ContextTests(unittest.TestCase):
    ...
    @unittest.skipUnless((PY_SSL_DEFAULT_CIPHERS == 1), 'Test applies only to Python default ciphers')
    ...
    @unittest.skipUnless(hasattr(ssl.SSLContext, 'security_level'), 'requires OpenSSL >= 1.1.0')
    def test_security_level(self):
        ctx = ssl.SSLContext()
        security_level_range = {0, 1, 2, 3, 4, 5}
        self.assertIn(ctx.security_level, security_level_range)

    @unittest.skipUnless(have_verify_flags(), 'verify_flags need OpenSSL > 0.9.8')
    ...
    def _assert_context_options(self, ctx):
        ...
        if (OP_CIPHER_SERVER_PREFERENCE != 0):
            self.assertEqual((ctx.options & OP_CIPHER_SERVER_PREFERENCE), OP_CIPHER_SERVER_PREFERENCE)
    ...
    def test__create_stdlib_context(self):
        ...
        self.assertEqual(ctx.verify_mode, ssl.CERT_NONE)
        self._assert_context_options(ctx)
        ctx = ssl._create_stdlib_context(ssl.PROTOCOL_TLSv1, cert_reqs=ssl.CERT_REQUIRED, check_hostname=True)
        self.assertEqual(ctx.protocol, ssl.PROTOCOL_TLSv1)
        self.assertEqual(ctx.verify_mode, ssl.CERT_REQUIRED)
        self.assertTrue(ctx.check_hostname)
        self._assert_context_options(ctx)
...
class ThreadedEchoServer(threading.Thread):
            ...
            while self.running:
                try:
                    ...
                    if (self.server.chatty and support.verbose):
                        sys.stdout.write(' Connection reset by peer: {}\n'.format(self.addr))
                    self.close()
                    self.running = False
    ...
    def __init__(self, certificate=None, ssl_version=None, certreqs=None, cacerts=None, chatty=True, connectionchatty=False, starttls_server=False, npn_protocols=None, alpn_protocols=None, ciphers=None, context=None):
        if context:
...
def server_params_test(client_context, server_context, indata=b'FOO\n', chatty=True, connectionchatty=False, sni_name=None, session=None):
    ...
    with server:
        with client_context.wrap_socket(socket.socket(), server_hostname=sni_name, session=session) as s:
            ...
            stats.update({'compression': s.compression(), 'cipher': s.cipher(), 'peercert': s.getpeercert(), 'client_alpn_protocol': s.selected_alpn_protocol(), 'client_npn_protocol': s.selected_npn_protocol(), 'version': s.version(), 'session_reused': s.session_reused, 'session': s.session})
            s.close()
...
class ThreadedTests(unittest.TestCase):
    ...
    def test_echo(self):
        ...
        for protocol in PROTOCOLS:
            ...
            with self.subTest(protocol=ssl._PROTOCOL_NAMES[protocol]):
                ...
                server_params_test(context, context, chatty=True, connectionchatty=True)
        (client_context, server_context, hostname) = testing_context()
    ...
    def test_protocol_sslv2(self):
        ...
        if no_sslv2_implies_sslv3_hello():
            try_protocol_combo(ssl.PROTOCOL_SSLv2, ssl.PROTOCOL_TLS, False, client_options=ssl.OP_NO_SSLv2)
    ...
    def test_protocol_sslv3(self):
        ...
        if no_sslv2_implies_sslv3_hello():
            try_protocol_combo(ssl.PROTOCOL_SSLv3, ssl.PROTOCOL_TLS, False, client_options=ssl.OP_NO_SSLv2)
    ...
    def test_protocol_tlsv1_1(self):
        ...
        try_protocol_combo(ssl.PROTOCOL_TLSv1_1, ssl.PROTOCOL_TLS, False, client_options=ssl.OP_NO_TLSv1_1)
    ...
    def test_starttls(self):
        ...
        server = ThreadedEchoServer(CERTFILE, starttls_server=True, chatty=True, connectionchatty=True)
        wrapped = False
        with server:
            s = socket.socket()
            s.setblocking(True)
            ...
            for indata in msgs:
                ...
                if ((indata == b'STARTTLS') and msg.startswith(b'ok')):
                    if support.verbose:
                        sys.stdout.write((' client:  read %r from server, starting TLS...\n' % msg))
                    conn = test_wrap_socket(s)
                    wrapped = True
                elif ((indata == b'ENDTLS') and msg.startswith(b'ok')):
                    if support.verbose:
                        sys.stdout.write((' client:  read %r from server, ending TLS...\n' % msg))
                    s = conn.unwrap()
                    wrapped = False
    ...
    def test_recv_send(self):
        ...
        with server:
            s = test_wrap_socket(socket.socket(), server_side=False, certfile=CERTFILE, ca_certs=CERTFILE, cert_reqs=ssl.CERT_NONE, ssl_version=ssl.PROTOCOL_TLS_CLIENT)
            s.connect((HOST, server.port))
            ...
            def _recvfrom_into():
                ...
                return b[:count]
            send_methods = [('send', s.send, True, [], len), ('sendto', s.sendto, False, ['some.address'], len), ('sendall', s.sendall, True, [], (lambda x: None))]
            recv_methods = [('recv', s.recv, True, []), ('recvfrom', s.recvfrom, False, ['some.address']), ('recv_into', _recv_into, True, []), ('recvfrom_into', _recvfrom_into, False, [])]
            data_prefix = 'PREFIX_'
            for (meth_name, send_meth, expect_success, args, ret_val_meth) in send_methods:
                ...
                try:
                    ...
                    outdata = s.read()
                    if (outdata != indata.lower()):
                        self.fail('While sending with <<{name:s}>> bad data <<{outdata:r}>> ({nout:d}) received; expected <<{indata:r}>> ({nin:d})\n'.format(name=meth_name, outdata=outdata[:20], nout=len(outdata), indata=indata[:20], nin=len(indata)))
                except ValueError as e:
                    if expect_success:
                        self.fail('Failed to send with method <<{name:s}>>; expected to succeed.\n'.format(name=meth_name))
                    if (not str(e).startswith(meth_name)):
                        self.fail('Method <<{name:s}>> failed with unexpected exception message: {exp:s}\n'.format(name=meth_name, exp=e))
            for (meth_name, recv_meth, expect_success, args) in recv_methods:
                ...
                try:
                    ...
                    if (outdata != indata.lower()):
                        self.fail('While receiving with <<{name:s}>> bad data <<{outdata:r}>> ({nout:d}) received; expected <<{indata:r}>> ({nin:d})\n'.format(name=meth_name, outdata=outdata[:20], nout=len(outdata), indata=indata[:20], nin=len(indata)))
                except ValueError as e:
                    if expect_success:
                        self.fail('Failed to receive with method <<{name:s}>>; expected to succeed.\n'.format(name=meth_name))
                    if (not str(e).startswith(meth_name)):
                        self.fail('Method <<{name:s}>> failed with unexpected exception message: {exp:s}\n'.format(name=meth_name, exp=e))
                    s.read()
            data = b'data'
    ...
    def test_handshake_timeout(self):
        ...
        try:
            try:
                ...
                c.settimeout(0.2)
                c.connect((host, port))
                self.assertRaisesRegex(socket.timeout, 'timed out', test_wrap_socket, c)
            finally:
                c.close()
            try:
                c = socket.socket(socket.AF_INET)
                ...
                c.settimeout(0.2)
                self.assertRaisesRegex(socket.timeout, 'timed out', c.connect, (host, port))
            finally:
                c.close()
        finally:
            finish = True
    ...
    @unittest.skipUnless(hasattr(ssl, 'OP_NO_COMPRESSION'), 'ssl.OP_NO_COMPRESSION needed for this test')
    ...
    def test_alpn_protocols(self):
        ...
        for (client_protocols, expected) in protocol_tests:
            ...
            try:
                ...
                stats = e
            if ((expected is None) and IS_OPENSSL_1_1_0 and (ssl.OPENSSL_VERSION_INFO < (1, 1, 0, 6))):
    ...
    def test_npn_protocols(self):
        ...
        protocol_tests = [(['http/1.1', 'spdy/2'], 'http/1.1'), (['spdy/2', 'http/1.1'], 'http/1.1'), (['spdy/2', 'test'], 'spdy/2'), (['abc', 'def'], 'abc')]
        for (client_protocols, expected) in protocol_tests:
            (client_context, server_context, hostname) = testing_context()
            ...
            self.assertEqual(client_result, expected, (msg % (client_result, 'client')))
            server_result = (stats['server_npn_protocols'][(- 1)] if len(stats['server_npn_protocols']) else 'nothing')
            self.assertEqual(server_result, expected, (msg % (server_result, 'server')))
...
class TestPostHandshakeAuth(unittest.TestCase):
    ...
    def test_pha_setter(self):
        protocols = [ssl.PROTOCOL_TLS, ssl.PROTOCOL_TLS_SERVER, ssl.PROTOCOL_TLS_CLIENT]
        for protocol in protocols:
        ...
        with threading_helper.catch_threading_exception() as cm:
            ...
            with server:
                with client_context.wrap_socket(socket.socket(), server_hostname=hostname) as s:
                    ...
                    self.assertEqual(s.recv(1024), b'OK\n')
                    s.write(b'HASCERT')
                    with self.assertRaisesRegex(ssl.SSLError, 'tlsv13 alert certificate required'):
                        s.recv(1024)
...
class TestSSLDebug(unittest.TestCase):
    ...
    @requires_keylog
    @unittest.skipIf(sys.flags.ignore_environment, 'test is not compatible with ignore_environment')
    @unittest.skipIf(Py_DEBUG_WIN32, 'Avoid mixing debug/release CRT on Windows')
...
def test_main(verbose=False):
    ...
    tests = [ContextTests, BasicSocketTests, SSLErrorTests, MemoryBIOTests, SSLObjectTests, SimpleBackgroundTests, ThreadedTests, TestPostHandshakeAuth, TestSSLDebug]
