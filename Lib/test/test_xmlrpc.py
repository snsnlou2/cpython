...
class HeadersServerTestCase(BaseServerTestCase):
    ...
    requestHandler = RequestHandler
    standard_headers = ['Host', 'Accept-Encoding', 'Content-Type', 'User-Agent', 'Content-Length']
    ...
    def test_header_many(self):
        p = xmlrpclib.ServerProxy(URL, headers=[('X-Test', 'foo'), ('X-Test-Second', 'bar')])
        self.assertEqual(p.pow(6, 8), (6 ** 8))
        headers = self.RequestHandler.test_headers
        self.assertContainsAdditionalHeaders(headers, {'X-Test': 'foo', 'X-Test-Second': 'bar'})
