...
class PosixTester(unittest.TestCase):
    ...
    def testNoArgFunctions(self):
        NO_ARG_FUNCTIONS = ['ctermid', 'getcwd', 'getcwdb', 'uname', 'times', 'getloadavg', 'getegid', 'geteuid', 'getgid', 'getgroups', 'getpid', 'getpgrp', 'getppid', 'getuid', 'sync']
    ...
    @unittest.skipUnless(hasattr(posix, 'posix_fallocate'), 'test needs posix.posix_fallocate()')
    ...
    @unittest.skipUnless(hasattr(posix, 'posix_fadvise'), 'test needs posix.posix_fadvise()')
    ...
    @unittest.skipUnless((hasattr(posix, 'mknod') and hasattr(stat, 'S_IFIFO')), "don't have mknod()/S_IFIFO")
    def test_mknod(self):
        os_helper.unlink(os_helper.TESTFN)
    ...
    @unittest.skipUnless((posix.listdir in os.supports_fd), 'test needs fd support for posix.listdir()')
    ...
    def test_stat_dir_fd(self):
        ...
        try:
            ...
            self.assertEqual(s1, s2)
            self.assertRaisesRegex(TypeError, 'should be integer or None, not', posix.stat, os_helper.TESTFN, dir_fd=posix.getcwd())
            self.assertRaisesRegex(TypeError, 'should be integer or None, not', posix.stat, os_helper.TESTFN, dir_fd=float(f))
            self.assertRaises(OverflowError, posix.stat, os_helper.TESTFN, dir_fd=(10 ** 20))
        finally:
            posix.close(f)
    ...
    def test_utime_dir_fd(self):
        ...
        try:
            ...
            posix.utime(os_helper.TESTFN, (int(now), int(((now - int(now)) * 1000000000.0))), dir_fd=f)
            posix.utime(os_helper.TESTFN, dir_fd=f, times=(int(now), int(((now - int(now)) * 1000000000.0))))
    ...
    def test_link_dir_fd(self):
        ...
        try:
            ...
            self.assertEqual(posix.stat(os_helper.TESTFN)[1], posix.stat((os_helper.TESTFN + 'link'))[1])
        finally:
            posix.close(f)
    ...
    @unittest.skipUnless(((os.mknod in os.supports_dir_fd) and hasattr(stat, 'S_IFIFO')), 'test requires both stat.S_IFIFO and dir_fd support for os.mknod()')
    ...
    def test_readlink_dir_fd(self):
        ...
        f = posix.open(posix.getcwd(), posix.O_RDONLY)
        try:
            self.assertEqual(posix.readlink((os_helper.TESTFN + 'link')), posix.readlink((os_helper.TESTFN + 'link'), dir_fd=f))
        finally:
            os_helper.unlink((os_helper.TESTFN + 'link'))
            posix.close(f)
    ...
    def test_mkfifo_dir_fd(self):
        ...
        try:
            ...
            posix.close(f)
    requires_sched_h = unittest.skipUnless(hasattr(posix, 'sched_yield'), "don't have scheduling support")
    requires_sched_affinity = unittest.skipUnless(hasattr(posix, 'sched_setaffinity'), "don't have sched affinity support")
    ...
    @requires_sched_h
    @unittest.skipUnless(hasattr(posix, 'sched_get_priority_max'), 'requires sched_get_priority_max()')
    ...
    @unittest.skipUnless(hasattr(os, 'SEEK_HOLE'), 'test needs an OS that reports file holes')
...
class _PosixSpawnMixin():
    ...
    @requires_sched
    @unittest.skipIf(sys.platform.startswith(('freebsd', 'netbsd')), 'bpo-34685: test can fail on BSD')
...
def test_main():
    try:
        support.run_unittest(PosixTester, PosixGroupsTester, TestPosixSpawn, TestPosixSpawnP)
    finally:
