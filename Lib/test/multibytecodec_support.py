    ...
    def _test_mapping_file_plain(self):
        ...
        with self.open_mapping_file() as f:
            for line in f:
                ...
                csetval = eval(data[0])
                if (csetval <= 127):
                    csetch = bytes([(csetval & 255)])
                elif (csetval >= 16777216):
                    csetch = bytes([(csetval >> 24), ((csetval >> 16) & 255), ((csetval >> 8) & 255), (csetval & 255)])
                elif (csetval >= 65536):
                    csetch = bytes([(csetval >> 16), ((csetval >> 8) & 255), (csetval & 255)])
                elif (csetval >= 256):
                    csetch = bytes([(csetval >> 8), (csetval & 255)])
                else:
                    continue
