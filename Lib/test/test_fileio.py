...
class AutoFileTests():
    ...
    def testRepr(self):
        self.assertEqual(repr(self.f), ('<%s.FileIO name=%r mode=%r closefd=True>' % (self.modulename, self.f.name, self.f.mode)))
        del self.f.name
        self.assertEqual(repr(self.f), ('<%s.FileIO fd=%r mode=%r closefd=True>' % (self.modulename, self.f.fileno(), self.f.mode)))
        self.f.close()
    ...
    def testReprNoCloseFD(self):
        ...
        try:
            with self.FileIO(fd, 'r', closefd=False) as f:
                self.assertEqual(repr(f), ('<%s.FileIO name=%r mode=%r closefd=False>' % (self.modulename, f.name, f.mode)))
        finally:
            os.close(fd)
