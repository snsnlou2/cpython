            ...
            for obj in ('_testcapi', '_testcapi.MethClass', '_testcapi.MethClass()', '_testcapi.MethStatic()'):
                with self.subTest(f'{obj}.{func_name}'):
                    cmd = textwrap.dedent(f'''
                        ...
                        bar()
                    ''')
                    gdb_output = self.get_stack_trace(cmd, breakpoint=func_name, cmds_after_breakpoint=['bt', 'py-bt'])
                    self.assertIn(f'<built-in method {func_name}', gdb_output)
                    gdb_output = self.get_stack_trace(cmd, breakpoint=func_name, cmds_after_breakpoint=['py-bt-full'])
