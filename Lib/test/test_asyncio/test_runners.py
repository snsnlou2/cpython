...
class RunTests(BaseTest):
    ...
    def test_asyncio_run_only_coro(self):
        for o in {1, (lambda : None)}:
            with self.subTest(obj=o), self.assertRaisesRegex(ValueError, 'a coroutine was expected'):
                asyncio.run(o)
    ...
    def test_asyncio_run_reports_hanging_tasks_errors(self):
        ...
        self.assertEqual(asyncio.run(main()), 123)
        self.assertTrue(lo_task.done())
        call_exc_handler_mock.assert_called_with({'message': test_utils.MockPattern('asyncio.run.*shutdown'), 'task': lo_task, 'exception': test_utils.MockInstanceOf(ZeroDivisionError)})
