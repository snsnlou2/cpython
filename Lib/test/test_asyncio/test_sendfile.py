...
class SendfileMixin(SendfileBase):
    ...
    def prepare_sendfile(self, *, is_ssl=False, close_after=0):
        ...
        srv_sock.bind((socket_helper.HOST, port))
        server = self.run_loop(self.loop.create_server((lambda : srv_proto), sock=srv_sock, ssl=srv_ctx))
        self.reduce_receive_buffer_size(srv_sock)
        if is_ssl:
        ...
        (tr, pr) = self.run_loop(self.loop.create_connection((lambda : cli_proto), sock=cli_sock, ssl=cli_ctx, server_hostname=server_hostname))
    ...
    def test_sendfile_not_supported(self):
        (tr, pr) = self.run_loop(self.loop.create_datagram_endpoint(asyncio.DatagramProtocol, family=socket.AF_INET))
        try:
    ...
    def test_sendfile_force_fallback(self):
        ...
        def sendfile_native(transp, file, offset, count):
            return base_events.BaseEventLoop._sendfile_native(self.loop, transp, file, offset, count)
    ...
    def test_sendfile_force_unsupported_native(self):
        ...
        with self.assertRaisesRegex(asyncio.SendfileNotAvailableError, 'not supported'):
            self.run_loop(self.loop.sendfile(cli_proto.transport, self.file, fallback=False))
        cli_proto.transport.close()
        self.run_loop(srv_proto.done)
    ...
    def test_sendfile_partial(self):
        (srv_proto, cli_proto) = self.prepare_sendfile()
        ret = self.run_loop(self.loop.sendfile(cli_proto.transport, self.file, 1000, 100))
        cli_proto.transport.close()
        self.run_loop(srv_proto.done)
    ...
    def test_sendfile_ssl_close_peer_after_receiving(self):
        (srv_proto, cli_proto) = self.prepare_sendfile(is_ssl=True, close_after=len(self.DATA))
        ret = self.run_loop(self.loop.sendfile(cli_proto.transport, self.file))
        self.run_loop(srv_proto.done)
        self.assertEqual(ret, len(self.DATA))
        self.assertEqual(srv_proto.nbytes, len(self.DATA))
    ...
    def test_sendfile_no_fallback_for_fallback_transport(self):
        ...
        with self.assertRaisesRegex(RuntimeError, 'fallback is disabled'):
            self.loop.run_until_complete(self.loop.sendfile(transport, None, fallback=False))
