...
class StreamTests(test_utils.TestCase):
    ...
    def test_open_connection_no_loop_ssl(self):
        with test_utils.run_test_server(use_ssl=True) as httpd:
            conn_fut = asyncio.open_connection(*httpd.address, ssl=test_utils.dummy_ssl_context(), loop=self.loop)
    ...
    def test_open_unix_connection_no_loop_ssl(self):
        with test_utils.run_test_unix_server(use_ssl=True) as httpd:
            conn_fut = asyncio.open_unix_connection(httpd.address, ssl=test_utils.dummy_ssl_context(), server_hostname='', loop=self.loop)
            self._basetest_open_connection_no_loop_ssl(conn_fut)
    ...
    def test_start_server(self):
        ...
        class MyServer():
            ...
            def start(self):
                ...
                self.server = self.loop.run_until_complete(asyncio.start_server(self.handle_client, sock=sock, loop=self.loop))
            ...
            def start_callback(self):
                ...
                sock.close()
                self.server = self.loop.run_until_complete(asyncio.start_server(self.handle_client_callback, host=addr[0], port=addr[1], loop=self.loop))
                return addr
        ...
        async def client(addr):
            with self.assertWarns(DeprecationWarning):
                (reader, writer) = (await asyncio.open_connection(*addr, loop=self.loop))
    ...
    def test_start_unix_server(self):
        ...
        class MyServer():
            ...
            def start(self):
                self.server = self.loop.run_until_complete(asyncio.start_unix_server(self.handle_client, path=self.path, loop=self.loop))
        ...
        async def client(path):
            with self.assertWarns(DeprecationWarning):
                (reader, writer) = (await asyncio.open_unix_connection(path, loop=self.loop))
            writer.write(b'hello world!\n')
            msgback = (await reader.readline())
            writer.close()
            (await writer.wait_closed())
    ...
    def test_drain_raises(self):
        ...
        thread.start()
        addr = q.get()
        with self.assertRaises((ConnectionResetError, ConnectionAbortedError, BrokenPipeError)):
    ...
    def test_wait_closed_on_close(self):
        with test_utils.run_test_server() as httpd:
            with self.assertWarns(DeprecationWarning):
                (rd, wr) = self.loop.run_until_complete(asyncio.open_connection(*httpd.address, loop=self.loop))
