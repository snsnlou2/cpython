...
class EventLoopTestsMixin():
    ...
    def test_create_unix_connection(self):
        ...
        with test_utils.run_test_unix_server() as httpd:
            conn_fut = self.loop.create_unix_connection((lambda : MyProto(loop=self.loop)), httpd.address)
    ...
    def _test_create_ssl_connection(self, httpd, create_connection, check_sockname=True, peername=None):
        conn_fut = create_connection(ssl=test_utils.dummy_ssl_context())
        self._basetest_create_ssl_connection(conn_fut, check_sockname, peername)
        if hasattr(ssl, 'Purpose'):

            def _dummy_ssl_create_context(purpose=ssl.Purpose.SERVER_AUTH, *, cafile=None, capath=None, cadata=None):
                "\n                A ssl.create_default_context() replacement that doesn't enable\n                cert validation.\n                "
                self.assertEqual(purpose, ssl.Purpose.SERVER_AUTH)
                return test_utils.dummy_ssl_context()
            with mock.patch('ssl.create_default_context', side_effect=_dummy_ssl_create_context) as m:
    ...
    def test_create_ssl_connection(self):
        with test_utils.run_test_server(use_ssl=True) as httpd:
            create_connection = functools.partial(self.loop.create_connection, (lambda : MyProto(loop=self.loop)), *httpd.address)
    ...
    def test_create_ssl_unix_connection(self):
        ...
        with test_utils.run_test_unix_server(use_ssl=True) as httpd:
            create_connection = functools.partial(self.loop.create_unix_connection, (lambda : MyProto(loop=self.loop)), httpd.address, server_hostname='127.0.0.1')
            self._test_create_ssl_connection(httpd, create_connection, check_sockname, peername=httpd.address)

    def test_create_connection_local_addr(self):
        with test_utils.run_test_server() as httpd:
            port = socket_helper.find_unused_port()
            f = self.loop.create_connection((lambda : MyProto(loop=self.loop)), *httpd.address, local_addr=(httpd.address[0], port))
    ...
    def test_create_connection_local_addr_in_use(self):
        with test_utils.run_test_server() as httpd:
            f = self.loop.create_connection((lambda : MyProto(loop=self.loop)), *httpd.address, local_addr=httpd.address)
    ...
    def test_connect_accepted_socket(self, server_ssl=None, client_ssl=None):
        ...
        proto.loop = loop
        loop.run_until_complete(loop.connect_accepted_socket((lambda : proto), conn, ssl=server_ssl))
        loop.run_forever()
    ...
    def test_ssl_connect_accepted_socket(self):
        if ((sys.platform == 'win32') and (sys.version_info < (3, 5)) and isinstance(self.loop, proactor_events.BaseProactorEventLoop)):
            raise unittest.SkipTest('SSL not supported with proactor event loops before Python 3.5')
    ...
    def test_connect_accepted_socket_ssl_timeout_for_plain_socket(self):
        ...
        coro = self.loop.connect_accepted_socket(MyProto, sock, ssl_handshake_timeout=support.LOOPBACK_TIMEOUT)
    ...
    def create_server_multiple_hosts(self, family, hosts, mock_sock):
        ...
        if (family == socket.AF_INET):
            ...
            mock_sock.socket().getsockbyname.side_effect = [(host, 80, 0, 0) for host in unique_hosts]
    ...
    def test_create_unix_server_path_socket_error(self):
        ...
        with sock:
            ...
            with self.assertRaisesRegex(ValueError, 'path and sock can not be specified at the same time'):
                self.loop.run_until_complete(f)
    ...
    def test_create_server_ssl(self):
        proto = MyProto(loop=self.loop)
        (server, host, port) = self._make_ssl_server((lambda : proto), test_utils.ONLYCERT, test_utils.ONLYKEY)
        f_c = self.loop.create_connection(MyBaseProto, host, port, ssl=test_utils.dummy_ssl_context())
    ...
    def test_create_unix_server_ssl(self):
        proto = MyProto(loop=self.loop)
        (server, path) = self._make_ssl_unix_server((lambda : proto), test_utils.ONLYCERT, test_utils.ONLYKEY)
        f_c = self.loop.create_unix_connection(MyBaseProto, path, ssl=test_utils.dummy_ssl_context(), server_hostname='')
        (client, pr) = self.loop.run_until_complete(f_c)
        client.write(b'xxx')
        self.loop.run_until_complete(proto.connected)
        self.assertEqual('CONNECTED', proto.state)
    ...
    def test_create_server_ssl_verify_failed(self):
        proto = MyProto(loop=self.loop)
        (server, host, port) = self._make_ssl_server((lambda : proto), test_utils.SIGNED_CERTFILE)
    ...
    def test_create_unix_server_ssl_verify_failed(self):
        proto = MyProto(loop=self.loop)
        (server, path) = self._make_ssl_unix_server((lambda : proto), test_utils.SIGNED_CERTFILE)
        sslcontext_client = ssl.SSLContext(ssl.PROTOCOL_TLS_CLIENT)
        sslcontext_client.options |= ssl.OP_NO_SSLv2
        sslcontext_client.verify_mode = ssl.CERT_REQUIRED
        if hasattr(sslcontext_client, 'check_hostname'):
            sslcontext_client.check_hostname = True
        f_c = self.loop.create_unix_connection(MyProto, path, ssl=sslcontext_client, server_hostname='invalid')
        with mock.patch.object(self.loop, 'call_exception_handler'):
            with test_utils.disable_logger():
                with self.assertRaisesRegex(ssl.SSLError, '(?i)certificate.verify.failed'):
                    self.loop.run_until_complete(f_c)
    ...
    def test_create_unix_server_ssl_verified(self):
        ...
        if hasattr(sslcontext_client, 'check_hostname'):
            sslcontext_client.check_hostname = True
        f_c = self.loop.create_unix_connection(MyProto, path, ssl=sslcontext_client, server_hostname='localhost')
        (client, pr) = self.loop.run_until_complete(f_c)
        proto.transport.close()
        client.close()
        server.close()
    ...
    def test_create_server_ssl_verified(self):
        ...
        if hasattr(sslcontext_client, 'check_hostname'):
            sslcontext_client.check_hostname = True
        f_c = self.loop.create_connection(MyProto, host, port, ssl=sslcontext_client, server_hostname='localhost')
        (client, pr) = self.loop.run_until_complete(f_c)
        self.check_ssl_extra_info(client, peername=(host, port), peercert=test_utils.PEERCERT)
        proto.transport.close()
        client.close()
        server.close()
        self.loop.run_until_complete(proto.done)
    ...
    def _test_create_datagram_endpoint(self, local_addr, family):
        ...
        coro = self.loop.create_datagram_endpoint(TestMyDatagramProto, local_addr=local_addr, family=family)
        (s_transport, server) = self.loop.run_until_complete(coro)
        sockname = s_transport.get_extra_info('sockname')
        (host, port) = socket.getnameinfo(sockname, (socket.NI_NUMERICHOST | socket.NI_NUMERICSERV))
        self.assertIsInstance(s_transport, asyncio.Transport)
        self.assertIsInstance(server, TestMyDatagramProto)
        self.assertEqual('INITIALIZED', server.state)
        self.assertIs(server.transport, s_transport)
        coro = self.loop.create_datagram_endpoint((lambda : MyDatagramProto(loop=self.loop)), remote_addr=(host, port))
    ...
    def test_unclosed_pipe_transport(self):
        ...
        async def connect():
            (read_transport, _) = (await loop.connect_read_pipe((lambda : read_proto), rpipeobj))
            (write_transport, _) = (await loop.connect_write_pipe((lambda : write_proto), wpipeobj))
    ...
    def test_write_pty(self):
        ...
        def reader(data):
            ...
            data += chunk
            return len(data)
        test_utils.run_until(self.loop, (lambda : (reader(data) >= 1)), timeout=support.SHORT_TIMEOUT)
        self.assertEqual(b'1', data)
        transport.write(b'2345')
        test_utils.run_until(self.loop, (lambda : (reader(data) >= 5)), timeout=support.SHORT_TIMEOUT)
        self.assertEqual(b'12345', data)
        self.assertEqual('CONNECTED', proto.state)
        os.close(master)
        self.assertIsNotNone(proto.transport.get_extra_info('pipe'))
    ...
    def test_bidirectional_pty(self):
        ...
        self.assertEqual('CONNECTED', write_proto.state)
        os.write(master, b'a')
        test_utils.run_until(self.loop, (lambda : (read_proto.nbytes >= 1)), timeout=support.SHORT_TIMEOUT)
        self.assertEqual(['INITIAL', 'CONNECTED'], read_proto.state)
        ...
        test_utils.run_until(self.loop, (lambda : (read_proto.nbytes >= 5)), timeout=support.SHORT_TIMEOUT)
        self.assertEqual(['INITIAL', 'CONNECTED'], read_proto.state)
    ...
    def test_timeout_rounding(self):
        ...
        self.assertLessEqual(self.loop._run_once_counter, 20, {'clock_resolution': self.loop._clock_resolution, 'selector': self.loop._selector.__class__.__name__})
...
class SubprocessTestsMixin():
    ...
    def test_subprocess_exec(self):
        ...
        connect = self.loop.subprocess_exec(functools.partial(MySubprocessProtocol, self.loop), sys.executable, prog)
    ...
    def test_subprocess_shell(self):
        with self.assertWarns(DeprecationWarning):
            connect = self.loop.subprocess_shell(functools.partial(MySubprocessProtocol, self.loop), 'echo Python')
            (transp, proto) = self.loop.run_until_complete(connect)
            self.assertIsInstance(proto, MySubprocessProtocol)
            self.loop.run_until_complete(proto.connected)
    ...
    def test_subprocess_exitcode(self):
        connect = self.loop.subprocess_shell(functools.partial(MySubprocessProtocol, self.loop), 'exit 7', stdin=None, stdout=None, stderr=None)
        with self.assertWarns(DeprecationWarning):
            (transp, proto) = self.loop.run_until_complete(connect)
        self.assertIsInstance(proto, MySubprocessProtocol)
        self.loop.run_until_complete(proto.completed)
    ...
    def test_subprocess_stderr_redirect_to_stdout(self):
        prog = os.path.join(os.path.dirname(__file__), 'echo2.py')
        connect = self.loop.subprocess_exec(functools.partial(MySubprocessProtocol, self.loop), sys.executable, prog, stderr=subprocess.STDOUT)
        with self.assertWarns(DeprecationWarning):
            (transp, proto) = self.loop.run_until_complete(connect)
            self.assertIsInstance(proto, MySubprocessProtocol)
            self.loop.run_until_complete(proto.connected)
    ...
    def test_subprocess_wait_no_same_group(self):
        connect = self.loop.subprocess_shell(functools.partial(MySubprocessProtocol, self.loop), 'exit 7', stdin=None, stdout=None, stderr=None, start_new_session=True)
...
if (sys.platform == 'win32'):
    ...
    if hasattr(selectors, 'KqueueSelector'):
        ...
        class KqueueEventLoopTests(UnixEventLoopTestsMixin, SubprocessTestsMixin, test_utils.TestCase):
    ...
    if hasattr(selectors, 'EpollSelector'):
        ...
        class EPollEventLoopTests(UnixEventLoopTestsMixin, SubprocessTestsMixin, test_utils.TestCase):

            def create_event_loop(self):
                return asyncio.SelectorEventLoop(selectors.EpollSelector())
    if hasattr(selectors, 'PollSelector'):

        class PollEventLoopTests(UnixEventLoopTestsMixin, SubprocessTestsMixin, test_utils.TestCase):

            def create_event_loop(self):
                return asyncio.SelectorEventLoop(selectors.PollSelector())

    class SelectEventLoopTests(UnixEventLoopTestsMixin, SubprocessTestsMixin, test_utils.TestCase):
...
class HandleTests(test_utils.TestCase):
    ...
    def test_callback_with_exception(self):
        ...
        h._run()
        self.loop.call_exception_handler.assert_called_with({'message': test_utils.MockPattern('Exception in callback.*'), 'exception': mock.ANY, 'handle': h, 'source_traceback': h._source_traceback})
    ...
    def test_handle_repr(self):
        ...
        regex = ('^<Handle noop\\(1, 2\\)\\(3\\) at %s:%s>$' % (re.escape(filename), lineno))
        self.assertRegex(repr(h), regex)
        cb = functools.partial(noop, x=1)
        h = asyncio.Handle(cb, (2, 3), self.loop)
        regex = ('^<Handle noop\\(x=1\\)\\(2, 3\\) at %s:%s>$' % (re.escape(filename), lineno))
        self.assertRegex(repr(h), regex)
    ...
    def test_handle_repr_debug(self):
        ...
        h = asyncio.Handle(noop, (1, 2), self.loop)
        (filename, lineno) = test_utils.get_function_source(noop)
        self.assertEqual(repr(h), ('<Handle noop(1, 2) at %s:%s created at %s:%s>' % (filename, lineno, create_filename, create_lineno)))
        h.cancel()
        self.assertEqual(repr(h), ('<Handle cancelled noop(1, 2) at %s:%s created at %s:%s>' % (filename, lineno, create_filename, create_lineno)))
        h.cancel()
        self.assertEqual(repr(h), ('<Handle cancelled noop(1, 2) at %s:%s created at %s:%s>' % (filename, lineno, create_filename, create_lineno)))
    ...
    def test_handle_source_traceback(self):
        ...
        def check_source_traceback(h):
            ...
            self.assertEqual(h._source_traceback[(- 1)][:3], (__file__, lineno, 'test_handle_source_traceback'))
    ...
    def test_coroutine_like_object_debug_formatting(self):
        ...
        self.assertEqual(coroutines._format_coroutine(coro), '<CoroLike without __name__>() running')
        coro = CoroLike()
...
class TimerTests(unittest.TestCase):
    ...
    def test_timer_repr_debug(self):
        ...
        h = asyncio.TimerHandle(123, noop, (), self.loop)
        (filename, lineno) = test_utils.get_function_source(noop)
        self.assertEqual(repr(h), ('<TimerHandle when=123 noop() at %s:%s created at %s:%s>' % (filename, lineno, create_filename, create_lineno)))
        h.cancel()
        self.assertEqual(repr(h), ('<TimerHandle cancelled when=123 noop() at %s:%s created at %s:%s>' % (filename, lineno, create_filename, create_lineno)))
...
class PolicyTests(unittest.TestCase):
    ...
    def test_get_event_loop_calls_set_event_loop(self):
        policy = asyncio.DefaultEventLoopPolicy()
        with mock.patch.object(policy, 'set_event_loop', wraps=policy.set_event_loop) as m_set_event_loop:
            loop = policy.get_event_loop()
    ...
    if (sys.platform != 'win32'):
        ...
        def test_get_event_loop_new_process(self):
            ...
            async def main():
                ...
                result = (await self.loop.run_in_executor(pool, _test_get_event_loop_new_process__sub_proc))
                pool.shutdown()
                return result
