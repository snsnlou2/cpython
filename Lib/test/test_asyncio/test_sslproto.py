...
class BaseStartTLS(func_tests.FunctionalTestCaseMixin):
    ...
    def test_create_connection_ssl_failed_certificate(self):
        ...
        async def client(addr):
            with self.assertWarns(DeprecationWarning):
                (reader, writer) = (await asyncio.open_connection(*addr, ssl=client_sslctx, server_hostname='', loop=self.loop, ssl_handshake_timeout=support.LOOPBACK_TIMEOUT))
        with self.tcp_server(server, max_clients=1, backlog=1) as srv:
    ...
    def test_start_tls_client_corrupted_ssl(self):
        ...
        async def client(addr):
            with self.assertWarns(DeprecationWarning):
                (reader, writer) = (await asyncio.open_connection(*addr, ssl=client_sslctx, server_hostname='', loop=self.loop))
