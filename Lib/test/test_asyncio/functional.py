...
class FunctionalTestCaseMixin():
    ...
    def tcp_server(self, server_prog, *, family=socket.AF_INET, addr=None, timeout=support.LOOPBACK_TIMEOUT, backlog=1, max_clients=10):
        if (addr is None):
    ...
    def tcp_client(self, client_prog, family=socket.AF_INET, timeout=support.LOOPBACK_TIMEOUT):
...
class TestSocketWrapper():
    ...
    def start_tls(self, ssl_context, *, server_side=False, server_hostname=None):
        ssl_sock = ssl_context.wrap_socket(self.__sock, server_side=server_side, server_hostname=server_hostname, do_handshake_on_connect=False)
        try:
