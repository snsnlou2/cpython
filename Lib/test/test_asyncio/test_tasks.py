...
class BaseTaskTests():
    ...
    def test_task_del_collect(self):
        ...
        async def run():
            return Evil()
        self.loop.run_until_complete(asyncio.gather(*[self.new_task(self.loop, run()) for _ in range(100)], loop=self.loop))
...
>>>{text}<<<''')
    ...
    def test_as_completed_invalid_args(self):
        fut = self.new_future(self.loop)
        self.assertRaises(TypeError, self.loop.run_until_complete, asyncio.as_completed(fut, loop=self.loop))
        coro = coroutine_function()
        self.assertRaises(TypeError, self.loop.run_until_complete, asyncio.as_completed(coro, loop=self.loop))
        coro.close()
        ...
        for (cancel_args, expected_args) in cases:
            with self.subTest(cancel_args=cancel_args):
                ...
                async def test():
                    ...
                    while True:
                        time += 0.05
                        (await asyncio.gather(asyncio.sleep(0.05), return_exceptions=True, loop=loop))
                        if (time > 1):
                            return
...
class WaitTests(test_utils.TestCase):
    ...
    def test_loop_argument_is_deprecated_in_wait(self):
        with self.assertWarns(DeprecationWarning):
            self.loop.run_until_complete(asyncio.wait([coroutine_function()], loop=self.loop))

    def test_loop_argument_is_deprecated_in_wait_for(self):
        with self.assertWarns(DeprecationWarning):
            self.loop.run_until_complete(asyncio.wait_for(coroutine_function(), 0.01, loop=self.loop))
