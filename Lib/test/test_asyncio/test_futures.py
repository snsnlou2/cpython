...
class BaseFutureTests():
    ...
    def test_exception(self):
        ...
        self.assertRaisesRegex(TypeError, 'StopIteration .* cannot be raised', f.set_exception, StopIteration)
        f.set_exception(exc)
        self.assertFalse(f.cancelled())
        self.assertTrue(f.done())
    ...
    def test_future_repr(self):
        ...
        self.assertEqual(repr(f_pending_debug), f'<{self.cls.__name__} pending created at {frame[0]}:{frame[1]}>')
        ...
        self.assertEqual(repr(f_exception), f'<{self.cls.__name__} finished exception=RuntimeError()>')
        ...
        self.assertRegex(repr(f_one_callbacks), (('<' + self.cls.__name__) + (' pending cb=\\[%s\\]>' % fake_repr)))
        ...
        self.assertRegex(repr(f_two_callbacks), (('<' + self.cls.__name__) + (' pending cb=\\[%s, %s\\]>' % (first_repr, last_repr))))
        ...
        self.assertRegex(repr(f_many_callbacks), (('<' + self.cls.__name__) + (' pending cb=\\[%s\\]>' % cb_regex)))
    ...
    def test_future_source_traceback(self):
        ...
        self.assertEqual(future._source_traceback[(- 2)][:3], (__file__, lineno, 'test_future_source_traceback'))
