...
@unittest.skipIf(((not term) or (term == 'unknown')), ('$TERM=%r, calling initscr() may cause exit' % term))
...
class TestCurses(unittest.TestCase):
    ...
    def test_embedded_null_chars(self):
        ...
        for arg in ['a', b'a']:
            with self.subTest(arg=arg):
                self.assertRaises(ValueError, stdscr.addstr, 'a\x00')
                self.assertRaises(ValueError, stdscr.addnstr, 'a\x00', 1)
                self.assertRaises(ValueError, stdscr.insstr, 'a\x00')
                self.assertRaises(ValueError, stdscr.insnstr, 'a\x00', 1)

    def test_module_funcs(self):
        'Test module-level functions'
        for func in [curses.baudrate, curses.beep, curses.can_change_color, curses.cbreak, curses.def_prog_mode, curses.doupdate, curses.flash, curses.flushinp, curses.has_colors, curses.has_ic, curses.has_il, curses.isendwin, curses.killchar, curses.longname, curses.nocbreak, curses.noecho, curses.nonl, curses.noqiflush, curses.noraw, curses.reset_prog_mode, curses.termattrs, curses.termname, curses.erasechar, curses.has_extended_color_support]:
    ...
    @unittest.skipIf((getattr(curses, 'ncurses_version', (99,)) < (5, 8)), 'unget_wch is broken in ncurses 5.7 and earlier')
