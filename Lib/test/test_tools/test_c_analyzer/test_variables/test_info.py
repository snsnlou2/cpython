...
with tool_imports_for_tests():
    ...
    from c_analyzer.variables.info import normalize_vartype, Variable

class NormalizeVartypeTests(unittest.TestCase):

    def test_basic(self):
        tests = [(None, None), ('', ''), ('int', 'int'), (PseudoStr('int'), 'int'), (StrProxy('int'), 'int')]
...
class VariableTests(unittest.TestCase):
    VALID_ARGS = (('x/y/z/spam.c', 'func', 'eggs'), 'static', 'int')
    ...
    def test_init_typical_global(self):
        for storage in ('static', 'extern', 'implicit'):
            with self.subTest(storage):
                static = Variable(id=ID(filename='x/y/z/spam.c', funcname=None, name='eggs'), storage=storage, vartype='int')
                self.assertEqual(static, (('x/y/z/spam.c', None, 'eggs'), storage, 'int'))

    def test_init_typical_local(self):
        for storage in ('static', 'local'):
            with self.subTest(storage):
                static = Variable(id=ID(filename='x/y/z/spam.c', funcname='func', name='eggs'), storage=storage, vartype='int')
        self.assertEqual(static, (('x/y/z/spam.c', 'func', 'eggs'), storage, 'int'))

    def test_init_all_missing(self):
        for value in ('', None):
            with self.subTest(repr(value)):
                static = Variable(id=value, storage=value, vartype=value)
                self.assertEqual(static, (None, None, None))

    def test_init_all_coerced(self):
        id = ID('x/y/z/spam.c', 'func', 'spam')
        tests = [('str subclass', dict(id=(PseudoStr('x/y/z/spam.c'), PseudoStr('func'), PseudoStr('spam')), storage=PseudoStr('static'), vartype=PseudoStr('int')), (id, 'static', 'int')), ('non-str 1', dict(id=id, storage=Object(), vartype=Object()), (id, '<object>', '<object>')), ('non-str 2', dict(id=id, storage=StrProxy('static'), vartype=StrProxy('variable')), (id, 'static', 'variable')), ('non-str', dict(id=id, storage=('a', 'b', 'c'), vartype=('x', 'y', 'z')), (id, "('a', 'b', 'c')", "('x', 'y', 'z')"))]
    ...
    def test_validate_bad_field(self):
        ...
        notnames = (('1a', 'a.b', 'a-b', '&a', 'a++') + badch)
        tests = [('id', ()), ('storage', (('external', 'global') + notnames)), ('vartype', ())]
        seen = set()
