...
class IDTests(unittest.TestCase):
    VALID_ARGS = ('x/y/z/spam.c', 'func', 'eggs')
    VALID_KWARGS = dict(zip(ID._fields, VALID_ARGS))
    VALID_EXPECTED = VALID_ARGS

    def test_from_raw(self):
        tests = [('', None), (None, None), ('spam', (None, None, 'spam')), (('spam',), (None, None, 'spam')), (('x/y/z/spam.c', 'spam'), ('x/y/z/spam.c', None, 'spam')), (self.VALID_ARGS, self.VALID_EXPECTED), (self.VALID_KWARGS, self.VALID_EXPECTED)]
    ...
    def test_minimal(self):
        id = ID(filename=None, funcname=None, name='eggs')
        self.assertEqual(id, (None, None, 'eggs'))

    def test_init_typical_global(self):
        id = ID(filename='x/y/z/spam.c', funcname=None, name='eggs')
        self.assertEqual(id, ('x/y/z/spam.c', None, 'eggs'))

    def test_init_typical_local(self):
        id = ID(filename='x/y/z/spam.c', funcname='func', name='eggs')
        self.assertEqual(id, ('x/y/z/spam.c', 'func', 'eggs'))

    def test_init_all_missing(self):
        for value in ('', None):
            with self.subTest(repr(value)):
                id = ID(filename=value, funcname=value, name=value)
                self.assertEqual(id, (None, None, None))

    def test_init_all_coerced(self):
        tests = [('str subclass', dict(filename=PseudoStr('x/y/z/spam.c'), funcname=PseudoStr('func'), name=PseudoStr('eggs')), ('x/y/z/spam.c', 'func', 'eggs')), ('non-str', dict(filename=StrProxy('x/y/z/spam.c'), funcname=Object(), name=('a', 'b', 'c')), ('x/y/z/spam.c', '<object>', "('a', 'b', 'c')"))]
    ...
    def test_validate_bad_field(self):
        ...
        tests = [('filename', ()), ('funcname', notnames), ('name', notnames)]
        seen = set()
