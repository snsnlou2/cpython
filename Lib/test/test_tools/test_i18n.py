...
class Test_pygettext(unittest.TestCase):
    ...
    def test_funcdocstring(self):
        for doc in ('"""doc"""', "r'''doc'''", "R'doc'", 'u"doc"'):
            with self.subTest(doc):
                msgids = self.extract_docstrings_from_str(dedent(('                def foo(bar):\n                    %s\n                ' % doc)))
    ...
    def test_classdocstring(self):
        for doc in ('"""doc"""', "r'''doc'''", "R'doc'", 'u"doc"'):
            with self.subTest(doc):
                msgids = self.extract_docstrings_from_str(dedent(('                class C:\n                    %s\n                ' % doc)))
                self.assertIn('doc', msgids)
    ...
    def test_funcdocstring_annotated_args(self):
        ...
        msgids = self.extract_docstrings_from_str(dedent('        def foo(bar: str):\n            """doc"""\n        '))
        self.assertIn('doc', msgids)
    ...
    def test_funcdocstring_multiple_funcs(self):
        ...
        msgids = self.extract_docstrings_from_str(dedent('        def foo1(bar: tuple=()) -> str:\n            """doc1"""\n\n        def foo2(bar: List[1:2]) -> (lambda x: x):\n            """doc2"""\n\n        def foo3(bar: \'func\'=lambda x: x) -> {1: 2}:\n            """doc3"""\n        '))
    ...
    def test_classdocstring_early_colon(self):
        ...
        msgids = self.extract_docstrings_from_str(dedent('        class D(L[1:2], F({1: 2}), metaclass=M(lambda x: x)):\n            """doc"""\n        '))
        self.assertIn('doc', msgids)
