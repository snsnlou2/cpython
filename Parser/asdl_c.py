...
class PrototypeVisitor(EmitVisitor):
    ...
    def emit_function(self, name, ctype, args, attrs, union=True):
        ...
        for i in range(1, (len(args) + 1)):
            margs += (', a%d' % i)
        self.emit(('#define %s(%s) _Py_%s(%s)' % (name, margs, name, margs)), 0, reflow=False)
...
class MarshalPrototypeVisitor(PickleVisitor):
    ...
    def prototype(self, sum, name):
        ctype = get_c_type(name)
        self.emit(('static int marshal_write_%s(PyObject **, int *, %s);' % (name, ctype)), 0)
...
class ASTModuleVisitor(PickleVisitor):
    ...
    def addObj(self, name):
        self.emit(('if (PyModule_AddObject(m, "%s", state->%s_type) < 0) {' % (name, name)), 1)
        self.emit('return -1;', 2)
        self.emit('}', 1)
...
def generate_module_def(f, mod):
    ...
    with open(os.devnull, 'w') as devnull:
        ...
        visitor.visit(mod)
        visitor_list.add(visitor)
    state_strings = {'ast', '_fields', '__doc__', '__dict__', '__module__', '_attributes'}
...
def write_source(f, mod):
    ...
    v = ChainOfVisitors(PyTypesDeclareVisitor(f), PyTypesVisitor(f), Obj2ModPrototypeVisitor(f), FunctionVisitor(f), ObjVisitor(f), Obj2ModVisitor(f), ASTModuleVisitor(f), PartingShots(f))
    v.visit(mod)
