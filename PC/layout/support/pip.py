...
def get_pip_layout(ns):
    ...
    if (not pip_dir.is_dir()):
        ...
        if ns.include_pip_user:
            content = '\n'.join(('[{}]\nuser=yes'.format(n) for n in ['install', 'uninstall', 'freeze', 'list']))
...
def extract_pip_files(ns):
    ...
    output = subprocess.check_output([sys.executable, '-m', 'pip', '--no-color', 'install', 'pip', 'setuptools', '--upgrade', '--target', str(dest), '--no-index', '--no-compile', '--no-cache-dir', '-f', str(src), '--only-binary', ':all:'], env=env)
    try:
