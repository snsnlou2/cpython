...
def get_layout(ns):
    ...
    if ns.include_pip:
        for (dest, src) in get_pip_layout(ns):
            if ((not isinstance(src, tuple)) and ((src in EXCLUDE_FROM_LIB) or (src in EXCLUDE_FROM_PACKAGED_LIB))):
                continue
            (yield (dest, src))
    if ns.include_chm:
...
def _compile_one_py(src, dest, name, optimize, checked=True):
    ...
    if (dest is not None):
        dest = str(dest)
    mode = (py_compile.PycInvalidationMode.CHECKED_HASH if checked else py_compile.PycInvalidationMode.UNCHECKED_HASH)
    try:
        return Path(py_compile.compile(str(src), dest, str(name), doraise=True, optimize=optimize, invalidation_mode=mode))
    ...
    try:
        ...
        for (dest, src) in files:
            ...
            if isinstance(src, tuple):
                ...
                continue
            if (ns.precompile and (src in PY_FILES) and (src not in EXCLUDE_FROM_COMPILE) and (src.parent not in DATA_DIRS) and os.path.normcase(str(dest)).startswith(os.path.normcase('Lib'))):
                if ns.copy:
        ...
        if need_compile:
            for (dest, src) in need_compile:
                compiled = [_compile_one_py(src, None, dest, optimize=0), _compile_one_py(src, None, dest, optimize=1), _compile_one_py(src, None, dest, optimize=2)]
                for c in compiled:
                    if (not c):
                        continue
...
def main():
    ...
    parser.add_argument('-s', '--source', metavar='dir', help='The directory containing the repository root', type=Path, default=None)
    parser.add_argument('-b', '--build', metavar='dir', help='Specify the build directory', type=Path)
    parser.add_argument('--arch', metavar='architecture', help='Specify the target architecture', type=str, default=None)
    parser.add_argument('--doc-build', metavar='dir', help='Specify the docs build directory', type=Path, default=None)
    parser.add_argument('--copy', metavar='directory', help='The name of the directory to copy an extracted layout to', type=Path, default=None)
    parser.add_argument('--zip', metavar='file', help='The ZIP file to write all files to', type=Path, default=None)
    parser.add_argument('--catalog', metavar='file', help='The CDF file to write catalog entries to', type=Path, default=None)
    parser.add_argument('--log', metavar='file', help='Write all operations to the specified file', type=Path, default=None)
    parser.add_argument('-t', '--temp', metavar='file', help='A temporary working directory', type=Path, default=None)
    parser.add_argument('-d', '--debug', help='Include debug build', action='store_true')
    parser.add_argument('-p', '--precompile', help='Include .pyc files instead of .py', action='store_true')
    parser.add_argument('-z', '--zip-lib', help='Include library in a ZIP file', action='store_true')
    parser.add_argument('--flat-dlls', help='Does not create a DLLs directory', action='store_true')
    parser.add_argument('-a', '--include-all', help='Include all optional components', action='store_true')
    parser.add_argument('--include-cat', metavar='file', help='Specify the catalog file to include', type=Path, default=None)
